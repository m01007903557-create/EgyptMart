...
<head>
    ...
    function openmail(id, type) {
        ...
        setTimeout(function() {
            $.post("/enq-details.php", {id: id, type: type}, function(data) {
...
function sendQuoteDirect(rfqId, offerId, msgId) {
    // الرابط السحري الذي يذهب مباشرة إلى صفحة التفاصيل
   let magicLink = '/enq-details.php?id=' + msgId + '&type=inbox&offer_id=' + offerId;
    
    // رسالة الواتساب تحتوي على الرابط الصحيح
    let whatsappMsg = "📦 لديك عرض سعر جديد لطلبك\n\n";
    whatsappMsg += "للموافقة أو الرفض، يرجى الضغط على الرابط:\n";
    whatsappMsg += "https://egyptmart.shop" + magicLink;
    
    // فتح واتساب
    window.open('https://wa.me/' + buyerPhone + '?text=' + encodeURIComponent(whatsappMsg), '_blank');
}
