...
// =============================================
// 1. معالجة طلبات API (من Make.com أو أي تكامل خارجي)
// =============================================
...
if (strpos($contentType, 'application/json') !== false) {
    ...
    // ✅ البيانات القادمة من Make.com (تحتوي على source_detail و external_id)
    $rfq_data = [
        'br_u_id' => $input_data['user_id'] ?? 0,
        'source_channel' => $input_data['source_channel'] ?? SOURCE_API,
        'source_platform' => $input_data['source_platform'] ?? null,
        'source_detail' => $input_data['phone'] ?? $input_data['customer_phone'] ?? null,
        'external_id' => $input_data['external_id'] ?? $input_data['message_id'] ?? null,
        'br_pc_id' => $input_data['category_id'] ?? 0,
        'br_pd_name' => $input_data['product_name'] ?? $input_data['product'] ?? '',
        'br_requirement' => $input_data['message'] ?? $input_data['details'] ?? '',
...
// =============================================
// 2. التحقق من تسجيل الدخول للفورم العادي
// =============================================
...
// =============================================
// 3. معالجة الفورم العادي (منصة الموقع)
// =============================================
// ... باقي الكود الأصلي كما هو ...
// استمراراً مع الكلاس AddProduct والـ HTML
// =============================================
// دالة إرسال رسائل واتساب
// =============================================
function sendWhatsAppMessage($to, $message, $product_code = '') {
    // تكوين بيانات واتساب - قم بتعديلها حسب خدمتك
    $whatsapp_config = [
    ...
    // إضافة كود المنتج للرسالة إن وجد
...
// =============================================
// التحقق من وجود طلب من Make.com (API Mode)
// =============================================
...
if (strpos($content_type, 'application/json') !== false) {
    ...
    $input_data = $_POST;
    // التحقق من وجود بيانات Make.com
    if (isset($input_data['product_code']) || isset($input_data['message'])) {
        $is_api_request = true;
    }
}

// إذا كان طلب API من Make.com
if ($is_api_request) {
    // تسجيل الطلب للسجلات
    error_log("📥 استلام طلب من Make.com: " . print_r($input_data, true));
    
    // استخراج البيانات
    $product_code = $input_data['product_code'] ?? '';
    $message = $input_data['message'] ?? '';
    $supplier_phone = $input_data['supplier_phone'] ?? '+201234567890'; // رقم المورد الافتراضي
    
    // التحقق من البيانات المطلوبة
    if (empty($product_code) && empty($message)) {
        $api_response = [
            'status' => 'error',
            'message' => 'البيانات غير مكتملة. يرجى إرسال product_code و message'
        ];
        http_response_code(400);
    ...
    // إضافة طلب الشراء إلى قاعدة البيانات
    try {
        global $con;
        
        // جلب معلومات المستخدم (افتراضي)
        $uid = 1; // معرف المستخدم الافتراضي - يمكن تعديله حسب الحاجة
        
        // إضافة الطلب
        $insert_sql = "INSERT INTO buy_requirement 
                       (br_u_id, br_pd_name, br_requirement, br_status, br_posting_date, br_updated_date)
                       VALUES (?, ?, ?, '1', NOW(), NOW())";
        
        $stmt = mysqli_prepare($con, $insert_sql);
        $product_name = "طلب شراء كود: " . $product_code;
        $requirement_text = $message;
        
        mysqli_stmt_bind_param($stmt, 'iss', $uid, $product_name, $requirement_text);
        mysqli_stmt_execute($stmt);
        $br_id = mysqli_insert_id($con);
        mysqli_stmt_close($stmt);
        
        if ($br_id > 0) {
            // إرسال رسالة واتساب للمورد
            $whatsapp_result = sendWhatsAppMessage($supplier_phone, $message, $product_code);
            ...
            $api_response = [
                ...
                'whatsapp_sent' => $whatsapp_result['status'],
            ...
            $api_response = [
                'status' => 'error',
                'message' => '❌ فشل في إضافة طلب الشراء إلى قاعدة البيانات'
            ];
...
// =============================================
// الكود الأصلي للوضع العادي (عند استخدام الموقع)
// =============================================
...
// =============================================
// معالجة نموذج الإرسال (الوضع العادي)
// =============================================
$msg = $_SESSION['msg'] ?? '';
unset($_SESSION['msg']);
    ...
    function usePhoto(id) 
{
    ...
    var usr = '<?php echo $_SESSION['uid_indm'] ?? 0; ?>';
    var br_id = <?php echo isset($br_id) ? (int)$br_id : 0; ?>;
    
    if (imageBasket.length > 0) {
        id = imageBasket.pop();
    }
    
    console.log('📤 إرسال: id=' + id + ', usr=' + usr + ', br_id=' + br_id);
