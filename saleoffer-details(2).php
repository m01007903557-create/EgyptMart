...
<body>
    ...
    function openWaRfqOffer(offerId, offerTitle, supplierId) {
        console.log('📱 فتح Popup واتساب للعرض:', offerId, offerTitle);
        ...
        if (!document.getElementById('waModalOffer')) {
            createWaModalOffer();
        }

        document.getElementById('wa_offer_id').value = offerId;
        document.getElementById('wa_offer_title').value = offerTitle;
        document.getElementById('wa_supplier_id').value = supplierId;

        var displayElement = document.getElementById('wa_offer_title_display');
        if (displayElement) displayElement.textContent = offerTitle;

        document.getElementById('waModalOffer').style.display = 'block';
    }

    function createWaModalOffer() {
        var modalHTML = `
        <div id="waModalOffer" style="display:none;position:fixed;z-index:99999;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.6);direction:rtl;">
            <div style="background:#fff;width:90%;max-width:500px;margin:10% auto;padding:25px;border-radius:10px;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
                <h3 style="margin-top:0;color:#25D366;font-size:22px;"><i class="fa fa-whatsapp"></i> طلب سعر - عرض بيع</h3>
                <p style="color:#666;font-size:14px;margin-bottom:15px;">يرجى إدخال تفاصيل طلب السعر للعرض: <strong id="wa_offer_title_display" style="color:#25D366;"></strong></p>

                <form id="waFormOffer">
                    <input type="hidden" id="wa_offer_id" name="offer_id" value="0">
                    <input type="hidden" id="wa_offer_title" name="product_name" value="">
                    <input type="hidden" id="wa_supplier_id" name="supplier_id" value="0">
                    <input type="hidden" name="request_type" value="saleoffer">
                    <input type="hidden" name="product_id" value="0">
                    <input type="hidden" name="qty_to" value="1">

                    <div style="margin-bottom:12px;">
                        <label style="display:block;margin-bottom:4px;font-weight:bold;font-size:14px;">الكمية المطلوبة:</label>
                        <input type="number" id="wa_qty" name="qty_from" value="1" min="1" style="width:100%;padding:10px;border:2px solid #e0e0e0;border-radius:6px;box-sizing:border-box;font-size:14px;" required>
                    </div>

                    <div style="margin-bottom:12px;">
                        <label style="display:block;margin-bottom:4px;font-weight:bold;font-size:14px;">مدة التسليم المطلوبة (أيام):</label>
                        <input type="number" id="wa_delivery" name="delivery_days" value="7" min="1" style="width:100%;padding:10px;border:2px solid #e0e0e0;border-radius:6px;box-sizing:border-box;font-size:14px;">
                    </div>

                    <div style="margin-bottom:15px;">
                        <label style="display:block;margin-bottom:4px;font-weight:bold;font-size:14px;">ملاحظات إضافية:</label>
                        <textarea id="wa_notes" name="requirement_details" rows="3" style="width:100%;padding:10px;border:2px solid #e0e0e0;border-radius:6px;box-sizing:border-box;resize:vertical;font-size:14px;font-family:Arial;" placeholder="أدخل أي تفاصيل إضافية..."></textarea>
                    </div>

                    <div style="display:flex;gap:10px;">
                        <button type="submit" style="flex:1;background:#25D366;color:#fff;border:none;padding:12px 20px;border-radius:6px;cursor:pointer;font-size:16px;font-weight:bold;">📤 إرسال طلب السعر</button>
                        <button type="button" onclick="document.getElementById('waModalOffer').style.display='none'" style="flex:1;background:#e0e0e0;color:#333;border:none;padding:12px 20px;border-radius:6px;cursor:pointer;font-size:16px;">إلغاء</button>
                    </div>
                </form>
            </div>
        </div>`;
        ...
        var form = document.getElementById('waFormOffer');
        if (form) {
            form.removeEventListener('submit', submitWaFormOffer);
            form.addEventListener('submit', submitWaFormOffer);
        }
    }

    async function submitWaFormOffer(e) {
        e.preventDefault();
        var form = e.target;
        var offerId = document.getElementById('wa_offer_id').value;
        ...
        var qty = document.getElementById('wa_qty').value || 1;
        var delivery = document.getElementById('wa_delivery').value || 7;
        var notes = document.getElementById('wa_notes').value || '';

        formData.set('offer_id', offerId);
        formData.set('request_type', 'saleoffer');
        formData.set('qty_from', qty);
        formData.set('qty_to', qty);
        formData.set('delivery_days', delivery);
        formData.set('requirement_details', notes);
        formData.set('product_id', 0);
        ...
        try {
            var res = await fetch('/whatsapp_rfq_handler.php', { method: 'POST', body: formData });
            var data = await res.json();

            if (data.success) {
                alert('✅ تم إرسال طلب السعر بنجاح، سيتم التواصل معك قريباً.');
                if (data.whatsapp_url) window.open(data.whatsapp_url, '_blank');
                document.getElementById('waModalOffer').style.display = 'none';
                form.reset();
            } else {
    ...
    document.addEventListener('click', function(e) {
        var modal = document.getElementById('waModalOffer');
