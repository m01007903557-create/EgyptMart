...
// ✅ كود التصحيح - يوضع هنا بعد تعريف $uid
error_log("===== product-sel-cat.php =====");
error_log("uid: " . $uid);
error_log("membership_expired: " . (get_membership_expired() ? 'true' : 'false'));
error_log("user_type: " . getUserInfo($uid, 'user_type'));

// ✅ منع وصول مستخدمي JUNIOR (usr_mp_id = 9) إلى هذه الصفحة
$sql_check = "SELECT usr_mp_id FROM user WHERE usr_id = '$uid'";
$res_check = mysqli_query($con, $sql_check);
$row_check = mysqli_fetch_assoc($res_check);
$usr_mp_id = $row_check['usr_mp_id'];
...
if ((int)getUserInfo($uid, 'user_type') <= 1) {
    header("Location: sign-in.php");
    exit;
}

// ✅ تجاوز التحقق من صلاحية العضوية لخطة PROMO
$is_promo_plan = false;
$check_plan_sql = "SELECT mp_name FROM smembership_plan sp 
                   INNER JOIN user u ON u.usr_mp_id = sp.mp_id 
                   WHERE u.usr_id = '$uid' LIMIT 1";
$check_plan_res = mysqli_query($con, $check_plan_sql);
if ($check_plan_res && mysqli_num_rows($check_plan_res) > 0) {
    $plan_row = mysqli_fetch_object($check_plan_res);
    if (strtoupper($plan_row->mp_name) == 'PROMO') {
        $is_promo_plan = true;
    }
}

if (!$is_promo_plan && get_membership_expired() != true) {
    header("Location: membership_plans.php");
    exit;
}

// حذف الصور المؤقتة (الكود الأصلي)
...
$(document).ready(function(){
    var url ='http://arab-mart.com/server/php/';
...
function showNextBtn(id) {
    if (!id || id == '') {
        console.log('showNextBtn: id فارغ');
        return;
    }
    console.log('showNextBtn تم استدعاؤها مع id: ' + id);
    
    // ✅ تعيين pd_subcat_id
    $("#pd_subcat_id").val(id);
    
    // ✅ جلب اسم التصنيف الفرعي وعرضه في السطر الأفقي
    $.post("ajax-file/showSelectCategory.php", {id: id}, function(data) {
        var parts = data.split('|');
        // parts[1] هو اسم التصنيف الفرعي
        if (parts[1] && parts[1] != '') {
            $("#sc").html(' >> ' + parts[1]);
        } else {
            $("#sc").html('');
...
function beowswcat()
{
    ...
    <div class="w56b f1 p2b p14 blr">
        <div class="mt5">
            <h1 style="margin-top:-5px !important;padding-bottom:5px;">Select Product / Service Category</h1>
            
            <div class="ap1">
                <a href="javascript:searchcat()" id="scs" class="f1 ap2 tabclose">Search Categories</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <a id="bcs" href="javascript:beowswcat()" style="margin-left: 26px;" class="f1 ap2 bc">Browse Categories</a>
                                ...
                                <div id="browseCateContent" style="padding: 5px;">
                                    ...
                                    <div id="selectListCate" class="selectListCate">
                                        <div style="width:100%;" id="multiSelectList" class="clearfix multiSelectList">
                                            <?php
                                            $sql_mc = "SELECT * FROM product_category WHERE pc_status='1' AND pc_parent_id='0' ORDER BY pc_name";
                                            $res_mc = mysqli_query($con, $sql_mc);
                                            ?>
                                            <?php 
                                            $people = ['JUNIOR','SENIOR','SPONSER']; 
                                            if (isset($icon['mst_name']) && in_array(strtok($icon['mst_name'], " "), $people)) {
                                                $selected_member = 'class="mybs"';
                                            } else {
                                                $selected_member = '';
                                            } 
                                            ?>
                                            <div style="width:32%;float:left;margin-right:0px;overflow-x:auto">
                                                <select name="mc_id" tabindex="1" class="column" size="10" id="mc_id" style="height:214px;width: 280px;" onChange="showCategory(this.value)">
                                                    <?php while ($row_mc = mysqli_fetch_object($res_mc)) {
                                                        $selected = '';
                                                        if (isset($_GET['select']) && $_GET['select'] == 'bs') {
                                                            if (ucfirst($row_mc->pc_name) == 'Business Services') {
                                                                $selected = 'selected="selected"';
                                                            }
                                                        }
                                                    ?>
                                                        <option value="<?php echo $row_mc->pc_id; ?>" <?php echo $selected; ?> <?php if (ucfirst($row_mc->pc_name) == 'Business Services') { echo $selected_member; } ?>>
                                                            <?php echo ucfirst($row_mc->pc_name); ?>
                                                        </option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                            <div style="width:33%;float:left;overflow-x:auto">
                                                <select name="cat_id" tabindex="2" class="column" size="10" id="cat_id" style="height: 214px; width: 280px; display:none;" onChange="showsubcat(this.value)"></select>
                                            </div>
                                            <div style="width:33%;float:left;overflow-x:auto">
                                                <select name="subcat_id" tabindex="3" class="column" size="10" id="subcat_id" style="height: 214px; width: 250px; display:none" onchange="showNextBtn(this.value);"></select>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    // ✅ تعيين pd_subcat_id عند اختيار نتيجة البحث
    if (data && data != '') {
        showNextBtn(data);
    }
});
