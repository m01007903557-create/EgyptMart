...
// إذا كانت الصفحة الحالية تخص مورداً مختلفاً عن آخر مورد تم تخزين منتجاته، افرغ الجلسة
$current_mfr = $cid ?? ''; // معرف المورد الحالي من الرابط
$last_mfr = $_SESSION['last_mfr_for_products'] ?? '';

if ($last_mfr != '' && $last_mfr != $current_mfr) {
    // إفراغ معرفات المنتجات من الجلسة
    for ($i = 1; $i <= 30; $i++) {
        unset($_SESSION["id$i"]);
...
// 3. ثم باقي الكود (التحقق من تسجيل الدخول، جلب البيانات، إلخ)
...
echo '<pre style="direction:ltr; text-align:left; background:#f0f0f0; padding:10px;">';
...
// ✅ التحقق من أن المنتجات المخزنة تخص المورد الحالي
$current_mfr_id = $row->usr_id ?? 0;
$session_mfr_id = $_SESSION['last_mfr_id'] ?? 0;

// إذا كان المورد قد تغير أو كانت الجلسة فارغة، افرغ المعرفات
if ($session_mfr_id != 0 && $session_mfr_id != $current_mfr_id) {
    for ($i = 1; $i <= 30; $i++) {
        unset($_SESSION["id$i"]);
        unset($_SESSION["image$i"]);
    }
}
// سجل آخر مورد تم زيارته
$_SESSION['last_mfr_id'] = $current_mfr_id;
...
// 🔍 للتصحيح - أزله بعد التأكد
echo "<!-- IDs in session: " . implode(',', array_filter($ids)) . " -->";
echo "<!-- Current MFR ID: $current_mfr_id -->";
...
echo "<pre>IDs found: "; print_r(array_filter($ids)); echo "</pre>";

// جلب المنتجات المختارة (من نفس المورد فقط)
$sel_product = [];
if ($id_list != '0') {
    $sel_pro = "select * from products where pd_uid = '$current_mfr_id' and pd_id IN ($id_list)";
...
// عرض المنتجات أو رسالة خطأ
if (empty($sel_product)) {
    echo '<p style="color: red; text-align: center; padding: 20px;"> ⚠️  لا يمكنك تجميع منتجات أكثر من مورد في نفس الوقت. يرجى العودة إلى صفحة المنتجات السابقة وإكمال طلبك، أو إغلاق الصفحة الحالية والبدء من جديد </p>';
    
    
} else {
    // عرض المنتجات (كود الـ HTML الخاص بالصور)
}
?>
