...
$sql_btype="select * from business_type_arabyos where bsntyp_id in(".$row->bnsprof_businesstype.")";
...
$noempsql=mysqli_query($con,"select * from employee_range where emprange_status='1' AND emprange_id='".$row->bnsprof_comemp."'");
...
if($row->bnsprof_designation!=''){
	...
	$res_desig=mysqli_query($con,$sql_desig);
	
if(mysqli_num_rows($res_desig)>0){ 
