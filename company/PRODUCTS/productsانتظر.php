...
// التحقق من اتصال قاعدة البيانات فورًا بعد تعريفه
...
// === جلب بيانات الشركة بشكل آمن ===
if (!isset($row) || $row === null || !isset($row->usr_id)) {
...
// ... باقي الكود
    ...
    // تعيين القيم الافتراضية لجميع الخصائص المستخدمة
    $row->usr_id = 0;
...
$c = $_GET['c'] ?? '';  // أو أي قيمة افتراضية مناسبة
...
$product_name = '';
if (isset($row_pd->pd_title)) {
    $product_name = $row_pd->pd_title;
} elseif (isset($row_pd_h->pd_title)) {
    $product_name = $row_pd_h->pd_title;
} else {
    $product_name = 'أحد المنتجات';
}

// جلب بيانات المشتري (الاسم، الجوال، المحافظة)
$user_name = '';
$user_phone = '';
$user_city = '';
...
if (isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != '') {
    ...
    $user_query = mysqli_query($con, "SELECT fname, lname, mobile1, city FROM user WHERE usr_id = '$user_id'");
    if ($user_data = mysqli_fetch_assoc($user_query)) {
        $user_name = trim(($user_data['fname'] ?? '') . ' ' . ($user_data['lname'] ?? ''));
        $raw_phone = $user_data['mobile1'] ?? '';
        // إضافة صفر إذا كان مفقودًا
...
?>
<div class="product_number">
    <a href="javascript:void(0);" onclick="openQuantityPopup(
        '<?php echo addslashes($product_name); ?>',
        '<?php echo addslashes($user_name); ?>',
        '<?php echo $user_phone; ?>',
        '<?php echo addslashes($user_city); ?>'
...
<script>
function openQuantityPopup(productName, userName, userPhone, userCity) {
    // إنشاء النافذة المنبثقة
    var popupHtml = `
    <div id="customPopup" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:9999; display:flex; justify-content:center; align-items:center;">
        <div style="background:#fff; width:90%; max-width:400px; border-radius:12px; padding:20px; direction:rtl; font-family:Tahoma, sans-serif;">
            <h3 style="text-align:center; margin-top:0;">طلب عرض سعر</h3>
            <div style="background:#f5f5f5; padding:10px; border-radius:8px; margin:10px 0;">
                <div>🛒 المنتج: <strong>${productName}</strong></div>
                <div>👤 المشتري: ${userName || 'زائر غير مسجل'}</div>
                <div>📍 المحافظة: ${userCity || 'غير مسجلة'}</div>
                <div>📞 الجوال: ${userPhone || 'غير مسجل'}</div>
            </div>
            <div style="margin-bottom:10px;">
                <label>الكمية (الحد الأدنى):</label>
                <input type="number" id="qtyFrom" placeholder="مثال: 10" style="width:100%; padding:8px; margin-top:5px; border:1px solid #ccc; border-radius:6px;">
            </div>
            <div style="margin-bottom:20px;">
                <label>الكمية (الحد الأقصى):</label>
                <input type="number" id="qtyTo" placeholder="مثال: 100" style="width:100%; padding:8px; margin-top:5px; border:1px solid #ccc; border-radius:6px;">
            </div>
            <div style="display:flex; gap:10px;">
                <button onclick="sendWhatsAppMessage()" style="flex:1; background:#25D366; color:white; border:none; padding:10px; border-radius:6px; cursor:pointer;">إرسال الطلب</button>
                <button onclick="closeCustomPopup()" style="flex:1; background:#ccc; border:none; padding:10px; border-radius:6px; cursor:pointer;">إلغاء</button>
            </div>
        </div>
    </div>
    `;
    ...
    if (document.getElementById('customPopup')) document.getElementById('customPopup').remove();
    document.body.insertAdjacentHTML('beforeend', popupHtml);

    // تعريف دوال الإرسال والإغلاق
    window.sendWhatsAppMessage = function() {
        var qtyFrom = document.getElementById('qtyFrom').value.trim();
        var qtyTo = document.getElementById('qtyTo').value.trim();
        
        if (qtyFrom === '' || qtyTo === '') {
            alert('الرجاء إدخال الكمية التقريبية (من وإلى)');
            return;
        }
        ...
        message += "📊 الكمية المطلوبة: من " + qtyFrom + " إلى " + qtyTo + " وحدة\n";
        message += "━━━━━━━━━━━━━━━━━━━━━\n";
        message += "👤 بيانات المشتري:\n";
        message += "   • الاسم: " + (userName || 'زائر غير مسجل') + "\n";
        message += "   • المحافظة: " + (userCity || 'غير مسجلة') + "\n";
        if (userPhone) message += "   • الجوال: " + userPhone + "\n";
        message += "━━━━━━━━━━━━━━━━━━━━━\n";
        message += "🔄 الرجاء التواصل مع المشتري لتقديم عرض السعر المناسب";
        
        // فتح واتساب
        var whatsappUrl = "https://api.whatsapp.com/send?phone=<?php echo $platform_phone; ?>&text=" + encodeURIComponent(message);
