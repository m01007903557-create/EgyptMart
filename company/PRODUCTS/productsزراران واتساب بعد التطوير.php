...
?>
<div class="product_number">
    <a href="javascript:void(0);" onclick="sendWhatsAppRequest();" style="cursor:pointer;">
...
function sendWhatsAppRequest() {
    // جلب اسم المنتج من الصفحة باستخدام المحدد الصحيح
    var productName = '';
    
    // المحدد الصحيح لاسم المنتج
    var productElement = document.querySelector('.product_title, .product_title_2, .product_title.product_title_2');
    
    if (productElement) {
        productName = productElement.innerText.trim();
    }
    
    // إذا لم يتم العثور، حاول مرة أخرى
    if (!productName) {
        var elements = document.querySelectorAll('[class*="product_title"]');
        for (var i = 0; i < elements.length; i++) {
            if (elements[i].innerText.trim().length > 0) {
                productName = elements[i].innerText.trim();
                break;
            }
        }
    }
    
    // إذا كان لا يزال فارغاً
    if (!productName) {
        productName = 'أحد المنتجات';
    }
    
    // بناء الرسالة
    var message = "📦 طلب عرض سعر للمنتج: " + productName + "\n";
    message += "━━━━━━━━━━━━━━━━\n";
    message += "👤 اسم المشتري: " + (userData.name || 'زائر غير مسجل') + "\n";
    if (userData.phone) {
        message += "📞 رقم الهاتف: " + userData.phone + "\n";
    }
    if (userData.email) {
        message += "📧 البريد الإلكتروني: " + userData.email + "\n";
    }
    message += "━━━━━━━━━━━━━━━━\n";
