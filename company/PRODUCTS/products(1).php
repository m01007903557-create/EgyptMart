...
// لا حاجة لفحص $con هنا، common.php يتولى ذلك
...
if ($flag == 'whsuccess') { ?>
    <div style="text-align: center; color: green;">تم إرسال الطلب بنجاح</div>
<?php }

// ... باقي الكود الأصلي (استعلامات المنتجات، إلخ)
...
// استخدام $row->usr_id الآن موجود دائمًا
...
echo "usr_id للشركة: " . ($row->usr_id ?? 'غير معرف') . "<br>";

$test_query = "select count(*) as total from products where pd_uid='" . $row->usr_id . "' and pd_status='1'";
$test_res = mysqli_query($con, $test_query);
$test_row = mysqli_fetch_assoc($test_res);
echo "عدد المنتجات الفعالة (pd_status=1): " . ($test_row['total'] ?? 0) . "<br>";

echo "الاستعلام المستخدم: " . htmlspecialchars($sq1s_totle) . "<br>";
echo "</div>";
...
$sq1_totle = "select count(*) as totle from products where pd_uid='" . $company->usr_id . "' and pd_status='1' and (pd_hot='0' OR pd_hot=' ')";
