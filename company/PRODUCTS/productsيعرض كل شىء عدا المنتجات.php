...
// ============================================================
$_SESSION['last_page'] = "company/products.php?c=" . $c;      // التصنيف الرئيسي (قد لا يستخدم مباشرة)
...
// ============================================================
// 2. متغيرات إضافية (للتجميل أو التوافق)
// ============================================================
$flag = $_GET['flag'] ?? '';
if ($flag == 'whsuccess') { ?>
...
// ============================================================
// 3. تعريف متغيرات الصفحة (بدون الاعتماد على شركة محددة)
// ============================================================
$class = "grids_list";
...
// ============================================================
// 4. استعلامات المنتجات بناءً على pd_subcat_id فقط
// ============================================================

// للمنتجات الساخنة (pd_hot = 1)
$sq1s_totle = "SELECT COUNT(*) as totle FROM products WHERE pd_subcat_id = '$subcat_id' AND pd_status = '1' AND pd_hot = '1'";
$ress_totle = mysqli_query($con, $sq1s_totle);
$rows_totle = mysqli_fetch_object($ress_totle);
$totalitem = ($rows_totle && $rows_totle->totle) ? ceil($rows_totle->totle / $limit) : 0;

// للمنتجات العادية (pd_hot = 0)
$sq1_totle = "SELECT COUNT(*) as totle FROM products WHERE pd_subcat_id = '$subcat_id' AND pd_status = '1' AND (pd_hot = '0' OR pd_hot = ' ')";
$res_totle = mysqli_query($con, $sq1_totle);
$row_totle = mysqli_fetch_object($res_totle);
$totalitems = ($row_totle && $row_totle->totle) ? ceil($row_totle->totle / $limit) : 0;

$prev = ($page > 1) ? $page - 1 : 1;
$next = ($page < $totalitems) ? $page + 1 : 1;

// ============================================================
// 5. استعلامات جلب المنتجات الفعلية (للعرض)
// ============================================================
...
// ============================================================
// 6. تشخيص بسيط (يمكن حذفه لاحقًا)
// ============================================================
echo "<div style='background:#f0f0f0; padding:10px; margin:10px; direction:ltr; text-align:left;'>";
echo "🔍 تشخيص المنتجات:<br>";
echo "subcat_id (sc): $subcat_id<br>";
echo "عدد المنتجات الساخنة: " . ($rows_totle->totle ?? 0) . "<br>";
echo "عدد المنتجات العادية: " . ($row_totle->totle ?? 0) . "<br>";
echo "الاستعلام الساخن: " . htmlspecialchars($sql_pd_h) . "<br>";
echo "</div>";

// ============================================================
// 7. كائن $row افتراضي لتجنب الأخطاء في right.php أو القالب
// ============================================================
$row = new stdClass();
$row->usr_id = 0;
...
// ============================================================
// 8. الآن يمكنك متابعة عرض المنتجات باستخدام $res_pd_h و $res_pd
//    كما هو موجود في الكود الأصلي (حلقات while)
// ============================================================
// مثال:
...
// لاحظ: الكود الأصلي بعد هذا السطر قد يحتوي على استعلامات أخرى (مثل $sql_pd_h و $sql_pd)
// وقد تكون مكررة. تأكد من استخدام المتغيرات التي عرفناها أعلاه وتجنب تكرار الاستعلامات.
?>
                    ...
                    <div id="view1" style="position: relative;">
                        ...
                        <ul class="hot-product">
                            <li class="ac-bdrb lc-bbw0 <?php echo $class; ?>">
                                ...
                                <?php
                                $sql_pd_h = "select * from products where pd_uid='" . $company->usr_id . "' and pd_status='1' and pd_hot='1' LIMIT " . $limit . " OFFSET " . $start . "";
                                $res_pd_h = mysqli_query($con,$sql_pd_h);
                                if (mysqli_num_rows($res_pd_h) > 0) {
                                    $j = 1;
                                    while ($row_pd_h = mysqli_fetch_object($res_pd_h)) {
                    ...
                    <div id="view2" style="position: relative;">
                        ...
                        <ul class="hot-product">
                            <li class="ac-bdrb lc-bbw0 <?php echo $class; ?>">
                                ...
                                <link href="css/colorbox.css" type="text/css" rel="stylesheet">
                                <?php
                             //   $sql_pd = "select * from products where pd_uid='" . $company->usr_id . "' and pd_status='1' and pd_hot='0'  LIMIT " . $limit . " OFFSET " . $start . "";
							 $sql_pd = "select * from products where pd_uid='" . $company->usr_id . "' and pd_status='1' and (pd_hot='0' OR pd_hot=' ')  LIMIT " . $limit . " OFFSET " . $start . "";
								//echo $sql_pd;
                                $res_pd = mysqli_query($con,$sql_pd);
                                if (mysqli_num_rows($res_pd) > 0) {
                                    $j = 1;
...
<?php 
$platform_phone = "201030029097";

// جلب بيانات المستخدم المسجل بالكامل (بما فيها المحافظة)
$user_name = '';
$user_phone = '';
$user_email = '';
$user_city = '';
...
if (isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != '') {
    ...
    $user_query = mysqli_query($con, "SELECT fname, lname, mobile1, email, city, governorate FROM user WHERE usr_id = '$user_id'");
    if ($user_data = mysqli_fetch_assoc($user_query)) {
        $user_name = trim(($user_data['fname'] ?? '') . ' ' . ($user_data['lname'] ?? ''));
        $raw_phone = $user_data['mobile1'] ?? '';
        $user_phone = (!empty($raw_phone) && substr($raw_phone, 0, 1) != '0') ? '0' . $raw_phone : $raw_phone;
        $user_email = $user_data['email'] ?? '';
        $user_city = $user_data['city'] ?? $user_data['governorate'] ?? '';
    }
}

// تخزين البيانات في متغيرات JavaScript
...
<div class="product_number">
    <a href="javascript:void(0);" onclick="openQuantityPopup();" style="cursor:pointer;">
...
<script>
// بيانات المستخدم
var userData = <?php echo $user_data_json; ?>;
var platformPhone = "<?php echo $platform_phone; ?>";
...
function openQuantityPopup() {
    ...
    <div id="whatsappPopup" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:10000; display:flex; justify-content:center; align-items:center;">
        <div style="background:#fff; width:90%; max-width:400px; border-radius:10px; padding:20px; direction:rtl; font-family:inherit;">
            ...
            <div style="background:#f5f5f5; padding:10px; border-radius:5px; margin-bottom:15px; font-size:13px;">
                <div>👤 الاسم: ` + (userData.name || 'زائر') + `</div>
                <div>📍 المحافظة: ` + (userData.city || 'غير مسجلة') + `</div>
                <div>📞 الجوال: ` + (userData.phone || 'غير مسجل') + `</div>
            </div>
            ...
            <div style="display:flex; gap:10px;">
                <button onclick="sendWhatsAppRequest()" style="flex:1; background:#25D366; color:#fff; border:none; padding:10px; border-radius:5px; cursor:pointer;">إرسال</button>
    ...
    if (document.getElementById('whatsappPopup')) {
...
function sendWhatsAppRequest() {
    ...
    if (qtyFrom === '') {
        alert('الرجاء إدخال الحد الأدنى للكمية');
        return;
    }
    if (qtyTo === '') {
        alert('الرجاء إدخال الحد الأقصى للكمية');
        return;
    }
    ...
    // جلب اسم المنتج
    var productName = '';
    var productElem = document.querySelector('.product_title, .product_title_2, h1, h2');
    if (productElem) productName = productElem.innerText.trim();
    if (!productName) productName = 'المنتج';
    
    var message = "طلب عرض سعر للمنتج: " + productName + "\n";
    message += "الكمية المطلوبة: من " + qtyFrom + " إلى " + qtyTo + "\n";
    message += "━━━━━━━━━━━━━\n";
    message += "الاسم: " + (userData.name || 'زائر') + "\n";
    message += "المحافظة: " + (userData.city || 'غير مسجلة') + "\n";
    if (userData.phone) message += "الجوال: " + userData.phone + "\n";
