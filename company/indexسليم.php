...
// ✅ تصحيح: عرض بيانات الشركة
$c = $_GET['c'] ?? '';
echo "<!-- Debug: c = " . htmlspecialchars($c) . " -->";

$sql = "SELECT bnsprof_id, bnsprof_compname, MD5(bnsprof_id) as md5_id FROM business_profile LIMIT 5";
$res = mysqli_query($con, $sql);
while ($row = mysqli_fetch_assoc($res)) {
    echo "<!-- ID: " . $row['bnsprof_id'] . " -> MD5: " . $row['md5_id'] . " -->";
}
...
// ============================================================
// 1. تعريف المتغيرات الأساسية والتأكد من وجودها
// ============================================================
...
if (!empty($c)) {
    ...
    if ($siteNameCheck) {
        // الكود الذي تريد تنفيذه عند صحة c
    }
}

// ============================================================
// 3. جلب بيانات الشركة بشكل آمن (هذا هو الجزء الحاسم)
// ============================================================
$bnsprof_uid = $_SESSION['uid_indm'] ?? 0;
$row = null; // تعريف $row مسبقًا بقيمة null
...
// إذا فشل جلب البيانات أو لم يكن هناك bnsprof_uid، قم بإنشاء كائن افتراضي لمنع الأخطاء
if ($row === null) {
    $row = new stdClass();
}

// تعيين القيم الافتراضية لجميع الخصائص التي قد تُستخدم في الصفحة
$row->usr_id = $row->usr_id ?? 0;
$row->bnsprof_comp_url = $row->bnsprof_comp_url ?? '';
$row->name_prefix = $row->name_prefix ?? '';
$row->fname = $row->fname ?? '';
$row->lname = $row->lname ?? '';
$row->bnsprof_address1 = $row->bnsprof_address1 ?? '';
$row->bnsprof_address2 = $row->bnsprof_address2 ?? '';
$row->bnsprof_city = $row->bnsprof_city ?? 0;
$row->country = $row->country ?? 0;
$row->country_ph_code = $row->country_ph_code ?? '';
$row->bnsprof_phcode1 = $row->bnsprof_phcode1 ?? '';
$row->bnsprof_ph1 = $row->bnsprof_ph1 ?? '';
$row->bnsprof_state = $row->bnsprof_state ?? 0;
