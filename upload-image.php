 ...
 * Description: رفع صورة مؤقتة لـ "من نحن" (About Us)
...
try {
    ...
    $check_sql = "SELECT COUNT(*) as count FROM temp_about_us WHERE tmabs_usrid = ?";
    ...
    if ($image_count >= 1) {
        throw new RuntimeException("You can only upload one temporary image");
    }
    ...
    $insert_sql = "INSERT INTO temp_about_us (tmabs_usrid, tmabs_images) VALUES (?, ?)";
    ...
    echo "1|Temporary image uploaded successfully|" . $fileName;

} catch (RuntimeException $e) {
    error_log("Upload Temp About Us Image Error: " . $e->getMessage() . " | User ID: $uid");
    http_response_code(400);
    echo "0|" . $e->getMessage();
} catch (Exception $e) {
    error_log("Unexpected Upload Temp About Us Image Error: " . $e->getMessage() . " | User ID: $uid");
