...
$sql_mcat="select * from product_category where md5(pc_id)='".$token."' and pc_status ='1'";
...
/* هذا الكود يؤثر فقط على category.php */
#leftsection {
...
/* إصلاح العمود الأيمن في index.php لم يتأثر */


/* إصلاح الصور الى اليمين نهائى */
#leftsection {
    width: 250px !important;
    float: left !important;
}
...
$sql_cat="select pc_id,pc_name from product_category pc where pc_parent_id='".$row_mcat->pc_id."' and pc_status='1'";
//echo $sql_cat;
$res_cat=mysqli_query($con, $sql_cat);
while($row_cat=mysqli_fetch_object($res_cat)){
  
    
$sql_cmt_cnt="SELECT count(*) FROM products,business_profile,plan_member_id WHERE business_profile.bnsprof_uid = products.pd_uid and  plan_member_id.b_id =business_profile.bnsprof_id and pd_subcat_id in(select pc_id from product_category where pc_parent_id='".$row_cat->pc_id."' and pc_status='1') ".$sql_pd_ck." AND plan_member_id.expiry_date > " . time() . "  and pd_status='1' group by pd_uid ";



//  $sql_cmt_cnt="SELECT count(*) FROM products WHERE pd_subcat_id in(select pc_id from product_category where pc_parent_id='".$row_cat->pc_id."' and pc_status='1') ".$sql_pd_ck." and pd_status='1' group by pd_uid";
  //echo $sql_cmt_cnt;
  ...
  if($num_comp>0){
    ...
    $sql_cat1 = "select pc_id,pc_name from product_category pc where pc_parent_id='".$row_cat->pc_id."' and pc_status='1'";
    ...
    <div class="blank_bg">
      <div class="welcome_desc">
        <div class="course_demo">
          ...
          $sql_so =  "select * from products p JOIN product_category pc ON p.pd_subcat_id=pc.pc_id JOIN measurement_unit mu ON p.pd_unit=mu.mu_id JOIN country ON p.pd_currency=country.cn_id JOIN business_profile bp ON p.pd_uid=bp.bnsprof_uid WHERE (pd_pck_dets='1' OR pd_lp_slider='1' OR pd_so_slider='1') AND pc.pc_parent_id in (select distinct pc_id from product_category where pc_parent_id = '".$row_mcat->pc_id."') and pc.pc_status='1' ".$sql_pd_ck."";
            ...
            <div class="row">
              ...
              while ($row_so = mysqli_fetch_object($res_so)) {
                ...
                if ($row_so->bnsprof_city) {
                                    if (isset($_COOKIE['loc_id'])) {
                                        $row_cityname = mysqli_fetch_object(mysqli_query($con,"select ct_name from city where ct_id='" . $row_so->bnsprof_city . "' limit 1"));
                                        $cn_city = $row_cityname->ct_name;
                                        $row_statename = mysqli_fetch_object(mysqli_query($con,"select state_name from states where state_id='" . $row_so->bnsprof_state . "' limit 1"));
                                       $cn_state = $row_statename->state_name;
                                        $cn_name = $cn_city ." - " . $cn_state;
                                    } else {
                                        $row_cityname = mysqli_fetch_object(mysqli_query($con,"select ct_cn_id from city where ct_id='" . $row_so->bnsprof_city . "' limit 1"));
                                        $row_countryname = mysqli_fetch_object(mysqli_query($con,"select cn_name from country where cn_id='" . $row_cityname->ct_cn_id . "' limit 1"));
