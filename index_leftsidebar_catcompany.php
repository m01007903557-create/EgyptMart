...
<style>
.ar-flags .checkbox { margin: 5px 0; padding: 2px 0; }
...
.ar-flags .checkbox input { margin: 0; }
</style>
...
<?php else: ?>
    <?php
    // عرض المحافظات النشطة للبلد المختار والتصنيف المحدد
    $sql_states = "SELECT DISTINCT s.state_id, s.state_name, c.cn_flag
                   FROM states s
                   JOIN country c ON c.cn_id = s.state_cn_id
                   WHERE s.state_cn_id = '" . mysqli_real_escape_string($con, $selected_country) . "'
                   AND EXISTS (
                       SELECT 1 
                       FROM products p
                       JOIN business_profile bp ON bp.bnsprof_uid = p.pd_uid
                       JOIN plan_member_id pm ON pm.b_id = bp.bnsprof_id
                       WHERE bp.bnsprof_state = s.state_id
                       AND (p.pd_subcat_id IN (SELECT pc_id FROM product_category WHERE pc_parent_id = $pc_id OR pc_id = $pc_id))
                       AND pm.expiry_date > $current_time
                       AND p.pd_status = '1'
                   )
    ...
    ?>
        ...
        <section class="ar-flags">
            <form action="catcompany.php" method="post">
                <input type="hidden" name="rctyp" value="Suppliers">
                <input type="hidden" name="pc_id" value="<?php echo $pc_id; ?>">
                <?php while($row = mysqli_fetch_object($result_states)): ?>
                    <div class="checkbox">
                        <label>
                            <input type="checkbox" name="state_id[]" value="<?php echo $row->state_id; ?>">
                            <?php if($row->cn_flag): ?>
                                <img src="images/country_flag/<?php echo $row->cn_flag; ?>" height="20" width="20">
                            <?php endif; ?>
                            <span><?php echo $row->state_name; ?></span>
                        </label>
                    </div>
                <?php endwhile; ?>
                <div class="form-group text-center" style="margin-top:10px;">
