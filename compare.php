 ...
 * File: compare.php
 * Version: PHP 8.3
 * Description: صفحة مقارنة المنتجات - تعرض المنتجات المختارة للمقارنة مع إمكانية إرسال استفسارات جماعية
 */

// بدء المخزن المؤقت
ob_start();

// إعداد التخزين المؤقت للجلسة
...
<head>
    <title>مقارنة المنتجات | <?php echo htmlspecialchars(getSiteTitle() ?? ''); ?></title>
    ...
    function delprod(id) {
        if (confirm("هل أنت متأكد من حذف هذا المنتج من المقارنة؟")) {
            ...
            var removeid = id.toString();
            
            // تنظيف سلسلة المعرفات
            existingids = existingids.replace(',' + removeid, '')
                                     .replace(removeid + ',', '')
                                     .replace(removeid, '');
            
            $('#prod_block' + removeid).fadeOut(500, function() {
                $(this).remove();
            ...
            createCookie("productids", existingids);
        }
    }
...
<body>
    ...
    <div class="hm1 bbc">
        ...
        <div class="comparemainblock">
            <!------------------ محتوى المقارنة -------------------------->
            <?php 
            if (file_exists('compare_middle_content.php')) {
                include_once 'compare_middle_content.php';
            } else {
                echo '<div class="alert alert-warning" style="margin:20px; text-align:center;">';
                echo 'لا يوجد محتوى للمقارنة حالياً';
                echo '</div>';
            }
            ?>
        </div>
        ...
        <div class="col-md-8" style="text-align:center; display:none; margin:0 auto; float:none;" id="thankYouBlock">
            <span>شكراً لاستفسارك، المورد سوف يرد عليك عند وجود إمكانية لذلك</span>
        </div>
