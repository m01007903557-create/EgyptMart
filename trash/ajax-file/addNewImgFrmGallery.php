...
if($tbl=='sale_offer_edit')
{
	...
	$newFileName="so-".rand(10,9999).substr($row_ph->ph_fileName,7);
...
else if($tbl=='temp_product_image')
{
	...
	$newFileName="prd-".rand(10,9999).substr($row_ph->ph_fileName,7);
...
{       
	...
	if(copy($pathS, $pathD))
	{
		...
		$sql_pd1="select * from products where pd_id='".$pd_id."'";
	$res_pd1=mysqli_query($con, $sql_pd1);
	$row_pd1=mysqli_fetch_object($res_pd1);
	if($_POST['typ']=='product'){
	$img = $row_pd1->pd_image;
		...
		}elseif($_POST['typ']=='logo'){
		$img = $row_pd1->pd_imagelogo;
...
else if($tbl=='temp_buyrequirement_image')
{
	...
	$newFileName="br-".rand(10,9999).substr($row_ph->ph_fileName,7);
