...
font-size:14px;
padding: 1px;
position:absolute;right:34px;top:144px;
}
.img_down
{
height:10px;margin: -16px 0px 6px 171px;
}
.sup_liy{
margin-left: 37px;margin-top:8px; float:left; width: 37%;
}
...
font-size:14px;
padding: 1px;
position:absolute;right:34px;top:82px;
}
.img_down
{
height:10px;margin: -16px 0px 6px 171px;
...
.sup_liy{
margin-left: -9px;margin-top:8px; float:left;  width: 37%;
}
}
...
font-size:14px;
padding: 1px;
position:absolute;right:34px;top:130px;
}
}
.checkbox-inline {
...
if($_POST['page'])
{
    ...
    $cur_page = $page;
    $page -= 1;
    $per_page = 40;//20; // Per page records
    $previous_btn = true;
    $next_btn = true;
    $first_btn = true;
    $last_btn = true;
...
//unset($_COOKIE['loc_id']);
    ...
    */
    ...
    $sql_pcat="select m.pc_id,m.pc_name,c.pc_id,c.pc_sort_name from product_category_arabyos m,product_category_arabyos c where m.pc_id=c.pc_parent_id and md5(c.pc_id)='".$pc_id."'";
    $res_pcat=mysql_query($sql_pcat);
    ...
    $sql_check1="select pc_id from product_category_arabyos where  md5(pc_{$parent}id)='".$pc_id."'";
    ...
    $sql_prd="select * from products,measurement_unit,country, business_profile, plan_member_id, smembership_plan_arabyos
...
$SubCategoryArray = "";
$html = "";
$sql_cmt_cnt1 = "select DISTINCT pd_subcat_id  from products,measurement_unit,country, business_profile, plan_member_id, smembership_plan_arabyos
...
if (count($pc_id_arr) == 0) {
    ...
    $ParentCategoryArray = "";
    $sql_dd_cmnuParent = "select	DISTINCT pc_parent_id	from product_category_arabyos where pc_id in ('$SubCategoryArray') ";
    ...
    $MasterCategoryArray = "";
    $sql_dd_cmnuMaster = "select	DISTINCT pc_parent_id	from product_category_arabyos where pc_id in ('$ParentCategoryArray') ";
    ...
    $sql_dd_cmnuAgainMaster = "select	DISTINCT pc_id	, pc_image ,	pc_name 	from product_category_arabyos where pc_id in ('$MasterCategoryArray') order by pc_name asc /*limit $first ,14*/";
...
else
{
    ...
    <div class="col-md-3 col-sm-3 col-xs-12 col-lg-2 prc-left-side" >
        ...
        <h3 class="togle_style" style="font-size:19px;cursor: pointer;font-weight:700;" onclick="toggle_menu();"><span class="fa fa-list"></span>&nbsp; MY MARKETS&nbsp;<i id="uparrow" class="fa fa-angle-up" style="font-size:18px;display:none;"></i><i id="downarrow" class="fa fa-angle-down" style="font-size:18px;"></i></h3>
        <div style="padding-left:0px;padding-right:10px;" class="left-side-bar-sale-offer">
            ...
            <div id='cssmenu' class="" style="float:left;width:200px;display: none">
                <ul>
                    ...
                    <?php
                    $sql_dd_mnu="select pc_id,pc_name,pc_image from product_category_arabyos where product_category_arabyos.pc_parent_id = '0' and product_category_arabyos.pc_status = '1' ".$sql_order;
                    $res_dd_mnu=mysqli_query($con, $sql_dd_mnu);
                    while ($row_dd_mnu = mysql_fetch_object($res_dd_cmnuAgainMaster))//while($row_dd_mnu=mysqli_fetch_object($res_dd_mnu))
                    {
                        ?>
...
//echo file_get_contents( 'https://arabyos.com/ajax_get_leftmenu_again.php');
    ...
    <div class="col-md-9 col-sm-9 col-lg-10 col-xs-12 prc-right-side" >
        <div  id="final_result" class="col-md-9 col-sm-9 col-xs-12 col-lg-9 new_final">
            <div class="middle-part">
                ...
                <button type="button" class="btn btn-default border-radius-0  txt-bold bold-xs btn-white text-capitalize " style="border-top:2px solid #ff7519 !important;border:0px;font-weight:600;">Products</button>

                <button type="button" class="btn btn-default border-radius-0  txt-bold bold-xs btn-white text-capitalize <?php echo $pc_id; ?>" style="background-color: #F5F7FA !important;"><a target="_blank" href="https://arabyos.com/catcompany.php?token=<?php echo rand(1000,9999).$pc_id;?>" style="color:#000;font-weight:600;">Suppliers</a></button>
            ...
            <div class="middle-control-part row" style="margin-left:0px;">
                ...
                <div class="col-md-10 col-xs-12">
                    ...
                    $mtqury = "select sp.mp_id, sip.mst_icon, sp.mst_name from smembership_plan_arabyos sp join smembership_icon_plan sip ON sp.mp_id = sip.mp_id where sp.mp_status != '0'";
					...
					<button class="btn btn-xs btn-default border-radius-0 " onclick="$('input[name=mst_type]').each(function(){this.checked = false; }); filter_member()" style="padding:0 5px 0 5px">Cancel</button>
    ...
    <div class="col-md-5" style="margin-top:8px">
                    ...
                    <label class="checkbox-inline min_order1" style="padding-left: 0px;padding-right: 15px;">Supplier Location:</label>

                    <span id="showcnt" style="">
                        All Countries & States &nbsp;<i class="fa fa fa-sort-desc"></i>
                    </span>
                </div>
                <div class="col-md-3 min_quan" style=" display:inline-block;">
               
                    <label for="min_order" class="checkbox-inline min_order">Min Qty <input id="min_order" type="text" value="" style="width: 50px"/></label>
			  ...
			  <form name="getcitydata" id="getcitydata" method="post" action="search.php?<?php if($_GET['rctyp']!=''){ echo "&rctyp=".$_GET['rctyp']; } ?><?php if($_GET['keywords']!=''){ echo "&keywords=".$_GET['keywords']; } ?>">
						  ...
						  <input type="search" class="" name="scity" id="scity" value="<?php echo $_POST['scity']?$_POST['scity']:'';?>" placeholder="Search City" />
							<!--<a href="#"><i class="fa fa-search " style="font-size:14px;"></i></a>-->
							<button type="submit" onclick="filter_member();return false;" class="scity_btn"><i class="fa fa-search fa_search"></i></button>
...
$country = "select * from states where state_cn_id in (".$_COOKIE['loc_id'].") AND state_status='1'";
$country = "select DISTINCT states.state_id, states.state_name from products,measurement_unit,country,states, business_profile, plan_member_id, smembership_plan_arabyos
where mu_id=pd_unit and pd_currency=cn_id and state_cn_id in (".$_COOKIE['loc_id'].") and states.state_id=business_profile.bnsprof_state ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1'  ANd smembership_plan.mp_id = plan_member_id.p_id and pd_image!='' and plan_member_id.expiry_date > ". time() ." and pd_subcat_id in('$ids') ORDER BY state_id/*FIELD(p_id,'5','4','3','15') */";

$cc=mysql_query($country);

while($data1=mysql_fetch_array($cc)){

$stateID = $data1['state_id'];
$ifstatebuisness = mysql_num_rows(mysql_query("select * from business_profile where bnsprof_state = ".$stateID));
if($ifstatebuisness == 0){continue;}

?>
<span class="outer_c"><input type="checkbox" name="state_sel" value="<?php echo $data1['state_id'];?>"><span style="color:blue;">
<?php
echo $data1['state_name'];?></span><i class="fa fa-angle-down cnt_state" id="<?php echo $data1['state_id'];?>" style="font-size: 15px;margin-left: 5px;cursor: pointer;"></i></span>
...
// below is from index_ls_countries.php - webxtor @ 18 June 2018 // see product query :109
//$sql_prd="select * from products,measurement_unit,country, business_profile, plan_member_id, smembership_plan_arabyos
 //where mu_id=pd_unit and pd_currency=cn_id ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1'  ANd smembership_plan.mp_id = plan_member_id.p_id and pd_image!='' and plan_member_id.expiry_date > ". time() ." and pd_subcat_id in('$ids') ORDER BY FIELD(p_id,'5','4','3','15') LIMIT ".$start.", ".$per_page;
...
$country = "select DISTINCT /*user.country*/ country.cn_id as country, country.cn_name from products,measurement_unit,country /*INNER JOIN `user` ON user.country = country.cn_id*/, business_profile, plan_member_id, smembership_plan_arabyos
where mu_id=pd_unit and pd_currency=cn_id ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1'  ANd smembership_plan.mp_id = plan_member_id.p_id and pd_image!='' and plan_member_id.expiry_date > ". time() ." and pd_subcat_id in('$ids') ORDER BY FIELD(p_id,'5','4','3','15') /*LIMIT ".$start.", ".$per_page."*/";

$country = "select DISTINCT pd_uid from products,measurement_unit, business_profile, plan_member_id, smembership_plan_arabyos
where mu_id=pd_unit ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1'  ANd smembership_plan.mp_id = plan_member_id.p_id and pd_image!='' and plan_member_id.expiry_date > ". time() ." and pd_subcat_id in('$ids') ORDER BY FIELD(p_id,'5','4','3','15') /*LIMIT ".$start.", ".$per_page."*/";

//$country="SELECT DISTINCT user.country, country.cn_name FROM `country` INNER JOIN `user` ON user.country = country.cn_id";

$cc=mysql_query($country);
 	$country_buy=array();
   while( $row11=mysql_fetch_array($cc, MYSQL_ASSOC))
		{
...
<span class="outer_c"><input type="checkbox" name="country_sel" value="<?php echo $rowk['cn_id'];//$data1['country'];?>"><span style="color:blue;">

<?php

echo $rowk['cn_name'];//$data1['cn_name']/*.','.$data1['country']*/;?></span><i class="fa fa-angle-down cnt_state" id="<?php echo $rowk['cn_id'];//$data1['country'];?>" style="font-size: 15px;margin-left: 5px;cursor: pointer;"></i></span>
...
<button class="btn btn-xs btn-default border-radius-0 "  style="padding:0 5px 0 5px" onclick="filter_member()">OK</button>
            ...
            <div id="header_load_sub" style="text-align: center;display: none" ><img src="http://arabyos.com/loadingp.gif" style="height: 125px"/></div>
                    ...
                    <ul class="als-wrapper" style="width: 101.1%">
                        ...
                        if (mysql_num_rows($recObj) > 0) {
                            ...
                            while ($row_prd = mysql_fetch_object($recObj)) {
                                ...
                                $newtext = substr($row_prd->pd_title, 0, "30");//12
                                ...
                                <li class="als-item"
                                    ...
                                    <a href="company/products.php?c=<?php echo rand(1000, 9999) . md5($row_prd->bnsprof_id); ?>&sc=<?php echo rand(10000, 99999) . $row_prd->pd_subcat_id; ?>#<?php echo $row_prd->pd_id; ?>"
                                       style="text-decoration:none;color:#000" target="_blank">

                                        <div style="height:150px;" id="img-div"><span class="img-helper" style="display: inline-block; height: 100%; vertical-align: middle;"></span><img src="upload/myproduct/<?php echo $row_prd->pd_image; ?>"
                                             alt="<?php echo ucwords(substr($row_prd->pd_title, 0, 28)); ?>"
                                             title="<?php echo ucwords($row_prd->pd_title); ?>"/><!--thumb--></div>

                                        <div style="">
                     <div class="utext" style=""><b><?php echo substr($newtext,0,36); ?></div></b>
						...
						<div style="text-align:center;margin-top:-8px;">
                                        ...
                                        <div style="height:10%;margin-top:1%; font-size:10px;">MOQ : <span
                                                    style="color:red; font-weight: 600; font-size:13px !important;"><?php echo $row_prd->pd_min_order_qty; ?>
                                                &nbsp;</span><span style="color: #9ca1ac;"><?php echo $row_prd->mu_name; ?></span></div>

                                        <div style="line-height: 0.8;margin-top:1%; font-size:10px;"><?php echo $row_prd->cn_currency; ?>
                                            &nbsp;<span
                                                    style="color:red; font-weight: 600; font-size:13px !important;"><?php echo $row_prd->pd_fob_price ?></span> / <span style="color: #9ca1ac;"><?php echo $row_prd->mu_name; ?></span>
                                        </div>

                                        <div style="font-weight:bold;font-size: 10px;padding: 4px 0;"><?php echo $row_prd->mst_name; ?> </div>
										</div>
                                    </a>

                                    <p class="cnt-phone"><span class="cnt-phone-inner"><img src="images/mobile.png" width="25px">&nbsp;&nbsp;<a class="a_tel" href="tel:+<?php echo $row_contact->country_ph_code; ?>-<?php echo $row_contact->mobile1; ?>"><?php echo $row_contact->country_ph_code; ?>
                                                -<?php echo $row_contact->mobile1; ?></a></p>


                                    <p class="cnt_supplier"><span class="cnt_supplier_inner"><i class="fa fa-envelope"
                                                                                                aria-hidden="true"></i>&nbsp;&nbsp;<a
                                                    href="ajax-file/quotationRequest.php?id=<?php echo rand(1000, 9999) . md5($row_prd->bnsprof_id); ?>&pid=<?php echo $row_prd->pd_id; ?>&c=<?php echo $row_contact->usr_id; ?>&vform=1"
                                                    id="btn_ajax<?php echo $row_prd->pd_id; ?>"
                                                    rel="product-send-inquiry" class="inquiry_but" style="color:#fff;"
                                                    `>Contact Supplier</a></span></p>
                                            ...
                                            $(document).ready(function () {
                                                ...
                                                $("#btn_ajax" +<?php echo $row_prd->pd_id; ?>).click(function (event) {
                                                    ...
                                                    if (uid_ind == '') {
                                                        ...
                                                        $("#btn_ajax" +<?php echo $row_prd->pd_id; ?>).colorbox({
                                                            width: "62%",
                                                            height: "89%"
                                                        });
                                                    }
                            ...
                            <li class="als-item"
                                style="border:1px solid #484891;margin-top:1%;margin-left:1.5%;border-radius:4px;width:97%;height:20px;color:#F00;height: 350px;">
...
$rowsperpage = $per_page;//$xml_atts['totalResultsReturned']
...
if (isset($cur_page) && is_numeric($cur_page)) {
   // cast var as int
...
?>
<li class="page-item"><span style="color:black; background:none;border:0;">Go to page <input id="goToPageNum" type="text" value="" style="width: 50px"/><button class="btn btn-xs btn-default border-radius-0 " onclick="goToPage()" style="padding:0 5px 0 5px">Go</button></span></li>
<?php } //if ($totalpages ?>
</ul>
</div>
</nav>
