...
$sql="select * from auction,purchased_auction where pauc_auc_id=auc_id and auc_status='1' and pauc_usr_id='".$_SESSION['uid_indm']."' order by pauc_purchase_date desc LIMIT ".$start.", ".$per_page;

$recObj=mysqli_query($con, $sql) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from auction,purchased_auction where pauc_auc_id=auc_id and auc_status='1' and pauc_usr_id='".$_SESSION['uid_indm']."'"; // Total records
...
<span class="pagenavigation">
			...
			// FOR ENABLING THE FIRST BUTTON
				if ($first_btn && $cur_page > 1) {	?>
					<a href="javascript:purchasedAuctions('1')"><img id="firstmail" src="images/firsten.gif"></a>
			...
			?>&nbsp;<?php
             ...
             // FOR ENABLING THE PREVIOUS BUTTON
	            if ($previous_btn && $cur_page > 1){
                 $pre = $cur_page - 1;
            ?><a href="javascript:purchasedAuctions('<?php echo $pre; ?>')"><img id="prevmail" src="images/prven.gif"></a><?php	
			...
			// TO ENABLE THE NEXT BUTTON
                if($next_btn && $cur_page < $no_of_paginations){
  	                $nex = $cur_page + 1;
   	        ?><a href="javascript:purchasedAuctions('<?php echo $nex; ?>')"><img id="nextmail" src="images/nxten.gif"></a><?php
                }else if ($next_btn){
			...
			?><a href="javascript:purchasedAuctions('<?php echo $no_of_paginations; ?>')"><img id="lastmail" src="images/lastenv.gif"></a>
...
<td valign="TOP">
	<div class="mp5 blhd" style="border-right: 1px solid #E7EAEE;">
    	<p>
        	<a onclick="detailPurAuctions(<?php echo $row->pauc_id; ?>)" style="cursor:pointer;"><?php echo $row->auc_heading; ?></a>
		...
		<div style="padding:0 0 5px 0;font-size:11px;color:#727272;"><b>Purchased on:</b> <?php echo date("d M, Y",strtotime($row->pauc_purchase_date)); ?></div> <?php /*echo stripslashes($row->br_requirement);*/ ?>
        <div class="blvd"><a onclick="detailPurAuctions(<?php echo $row->pauc_id; ?>)" style="cursor:pointer;">View complete details</a></div>
        <div class="blvd"></div>
	</div>
</td>
...
<div class="bulletImage fleft"><a id="apprvDel1" class="apprvDel" style="cursor:pointer;" onClick="delAuction(<?php echo $row->auc_id; ?>,'op');">Delete</a></div>
...
<?php	}else{	?>
    ...
    <div style="clear:both;">
		<div class="pbl-sr" style="background-position:0px -35px; min-height:90px;color:#F00;" align="center"><br><b style="line-height:50px;">You do not have any Purchased Auctions.</b><br><br></div>
