...
$sql_inbox="select * from tender,user,business_profile where tnd_usr_id=usr_id and usr_id=bnsprof_uid and tnd_usr_id='".$_SESSION['uid_indm']."' and tnd_status='1' order by tnd_updated_date desc LIMIT ".$start.", ".$per_page;

$recObj=mysqli_query($con, $sql_inbox) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from tender,user,business_profile where tnd_usr_id=usr_id and usr_id=bnsprof_uid and tnd_usr_id='".$_SESSION['uid_indm']."' and tnd_status='1'"; // Total records
...
<td valign="TOP">
	<div class="mp5 blhd" style="border-right: 1px solid #E7EAEE;">
    	<p>
        	<a onclick="detailTender(<?php echo $row->tnd_id; ?>)" style="cursor:pointer;"><?php echo $row->tnd_heading; ?></a>
		...
		<div style="padding:0 0 5px 0;font-size:11px;color:#727272;"><b>Posted on:</b> <?php echo date("d M, Y",strtotime($row->tnd_updated_date)); ?></div> <?php echo stripslashes($row->tnd_detail); ?>
        <div class="blvd"><a onclick="detailTender(<?php echo $row->tnd_id; ?>)" style="cursor:pointer;">View complete details</a></div>
	</div>
</td>
...
<div class="bulletImage"><a id="apprvDel1" class="apprvDel" style="cursor:pointer;" onClick="delTender(<?php echo $row->tnd_id; ?>);">Delete</a></div>
</td>
	</tr>
<?php	}	?>
</tbody></table></div>
