...
$sql="select * from buy_requirement,purchased_buy_requirement where pbr_br_id=br_id and br_display_status='1' and br_status='1' and pbr_usr_id='".$_SESSION['uid_indm']."' order by pbr_purchase_date desc LIMIT ".$start.", ".$per_page;

$recObj=mysqli_query($con, $sql) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from buy_requirement,purchased_buy_requirement where pbr_br_id=br_id and br_display_status='1' and br_status='1' and pbr_usr_id='".$_SESSION['uid_indm']."'"; // Total records
...
<span class="pagenavigation">
			...
			// FOR ENABLING THE FIRST BUTTON
				if ($first_btn && $cur_page > 1) {	?>
					<a href="javascript:purchasedBuyleads('1')"><img id="firstmail" src="images/firsten.gif"></a>
			...
			?>&nbsp;<?php
             ...
             // FOR ENABLING THE PREVIOUS BUTTON
	            if ($previous_btn && $cur_page > 1){
                 $pre = $cur_page - 1;
            ?><a href="javascript:purchasedBuyleads('<?php echo $pre; ?>')"><img id="prevmail" src="images/prven.gif"></a><?php	
			...
			// TO ENABLE THE NEXT BUTTON
                if($next_btn && $cur_page < $no_of_paginations){
  	                $nex = $cur_page + 1;
   	        ?><a href="javascript:purchasedBuyleads('<?php echo $nex; ?>')"><img id="nextmail" src="images/nxten.gif"></a><?php
                }else if ($next_btn){
			...
			?><a href="javascript:purchasedBuyleads('<?php echo $no_of_paginations; ?>')"><img id="lastmail" src="images/lastenv.gif"></a>
...
<tr>
<td class="pbl_top_mBuy" height="24">Buy Leads Details</td>
...
;">
    	<p>
        	<a onclick="detailPurBuyleads(<?php echo $row->pbr_id; ?>)" style="cursor:pointer;"><?php echo $row->br_pd_name; ?></a>
		...
		<div style="padding:0 0 5px 0;font-size:11px;color:#727272;"><b>Purchased on:</b> <?php echo date("d M, Y",strtotime($row->pbr_purchase_date)); ?></div> <?php echo stripslashes($row->br_requirement); ?>
        <div class="blvd"><a onclick="detailPurBuyleads(<?php echo $row->pbr_id; ?>)" style="cursor:pointer;">View complete details</a></div>
        <div class="blvd"></div>
	</div>
</td>
...
<div class="bulletImage fleft"><a id="apprvDel1" class="apprvDel" style="cursor:pointer;" onClick="delRequirement(<?php echo $row->br_id; ?>,'op');">Delete</a></div>
<div class="bulletImage fleft"><a class="ajax" rel="nofollow" href="sendLeadEnquiry-form.php?id=<?php echo rand(1000,9999).md5($row->br_u_id); ?>&headline=<?php echo urlencode($row->br_pd_name); ?>">Send Enquiry</a></div>
</td>
	</tr>
<?php	}	?>
</tbody></table></div>
...
<?php	}else{	?>
    ...
    <div style="clear:both;">
		<div class="pbl-sr" style="background-position:0px -35px; min-height:90px;color:#F00;" align="center"><br><b style="line-height:50px;">You do not have any Purchased Buy Leads.</b><br><br></div>
