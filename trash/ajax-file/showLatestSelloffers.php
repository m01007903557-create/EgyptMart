...
$sql_so="select * from sale_offer,user,business_profile where so_usr_id=usr_id and usr_id=bnsprof_uid and so_approval_status='1' ".$sql_so_ck." and so_status='1' and DATE_ADD(so_approval_date,INTERVAL so_validity DAY)>=now() order by so_approval_date desc LIMIT ".$start.", ".$per_page;
$recObj=mysqli_query($con, $sql_so) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from sale_offer,user,business_profile where so_usr_id=usr_id and usr_id=bnsprof_uid and so_approval_status='1' ".$sql_so_ck." and so_status='1' and DATE_ADD(so_approval_date,INTERVAL so_validity DAY)>=now()"; // Total records
...
?>
<div class="bx a1 p2">
<div class="bc1 f3" style="padding-left:5px;"><p class="flr fz2 ttc">Total Sell Offers - <span class="c6 bo"><?php echo $count; ?></span> </p>Latest Sell Offers</div>
...
<?php
	...
	{
?>
<li><img class="lm mr1" src="upload/sale_offer/<?php echo $row_so->so_pic; ?>" alt="<?php echo $row_so->so_service; ?>" align="left" height="68" width="68"><a href="saleoffer-details.php?id=<?php echo rand(1000,9999).md5($row_so->so_id); ?>" class="bo" style="font-size:15px"><?php echo ucwords($row_so->so_service); ?></a>
		<p class="p1 lnh"><?php echo $row_so->so_description; ?></p>
		<p class="p1">
		<?php
        	//echo get_country_name($row_so->country); 
			if($row_so->so_preferred_buyer_location=='any')
			{
				?>
                <span class="c7">Location:</span> <?php echo get_country_name($row_so->country); ?>
                &nbsp;&nbsp;<img src="images/country_flag/<?php echo get_country_flag($row_so->country); ?>" alt="" class="w4" align="top" height="15" width="23">
                <span style="float:right"><span class="c7">Trade Scope:</span> (Foreign & Domestic)</span>
                
				<?php
			}
    		else if($row_so->so_preferred_buyer_location=='abroad')
			{
				?>
                <span class="c7">Location:</span> <?php echo get_country_name($row_so->country); ?>
                &nbsp;&nbsp;<img src="images/country_flag/<?php echo get_country_flag($row_so->country); ?>" alt="" class="w4" align="top" height="15" width="23">
			...
			else if($row_so->so_preferred_buyer_location=='domestic')
			{
				?>
                <span class="c7">Location:</span> <?php echo get_country_name($row_so->country); ?>
                &nbsp;&nbsp;<img src="images/country_flag/<?php echo get_country_flag($row_so->country); ?>" alt="" class="w4" align="top" height="15" width="23">
			...
			else if($row_so->so_preferred_buyer_location=='my_city' && $row_so->bnsprof_city!='0')
			{
				?>
                <span class="c7">Location:</span> <?php echo get_city_name($row_so->bnsprof_city);	?>
      ...
      <?php if($count>$per_page){ ?>
			...
			// FOR ENABLING THE FIRST BUTTON
				if ($first_btn && $cur_page > 1) {	?>
					<a href="javascript:showSaleoffers('1')"><img id="firstmail" src="images/firsten.gif"></a>
			...
			?>&nbsp;<?php
             ...
             // FOR ENABLING THE PREVIOUS BUTTON
	            if ($previous_btn && $cur_page > 1){
                 $pre = $cur_page - 1;
            ?><a href="javascript:showSaleoffers('<?php echo $pre; ?>')"><img id="prevmail" src="images/prven.gif"></a><?php	
			...
			// TO ENABLE THE NEXT BUTTON
                if($next_btn && $cur_page < $no_of_paginations){
  	                $nex = $cur_page + 1;
   	        ?><a href="javascript:showSaleoffers('<?php echo $nex; ?>')"><img id="nextmail" src="images/nxten.gif"></a><?php
                }else if ($next_btn){
			...
			?><a href="javascript:showSaleoffers('<?php echo $no_of_paginations; ?>')"><img id="lastmail" src="images/lastenv.gif"></a>
