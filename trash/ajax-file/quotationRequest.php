...
//echo "<pre>"; print_r( $userArrayRow_Result); echo "</pre>";
...
if($_GET['vform'] == 1){
$nxt_id .= " AND products.pd_uid = ".$p_row->pd_uid;
}
$nxt_id .= " AND  products.pd_id <".$_GET['pid']." ORDER BY products.pd_id DESC LIMIT 1";
...
AND products.pd_image !=  '' ";
if($_GET['vform'] == 1){
$pre_id .= " AND products.pd_uid = ".$p_row->pd_uid;
}
$pre_id .= " AND  products.pd_id >".$_GET['pid']."
LIMIT 1";
}
//echo $pre_id; //die;
$run_querypre=mysql_query($pre_id) or die(mysql_error());
$run_pre=mysql_fetch_object($run_querypre);
//echo "<pre>"; print_r($run_pre); echo "</pre>";
...
<?php if($run_pre!=''){ 
if($_GET['keywords']!=""){?>
  <a data-prev=""   class="ajax"href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo rand(1000, 9999) . md5($data1['bnsprof_id']); ?>&pid=<?php echo $run_pre->pd_id; ?>&keywords=<?php echo $_GET['keywords'];?>&geo=<?php echo $geo_id;?>&conty=<?php echo $con_id;?>"><img src="https://arabyos.com/company/images/2.png" style="height: 61px; float: left; margin-top: 44px; cursor: pointer;"  /></a>
  <?php }
  else{ 
  if($_GET['vform'] == 1)
	$url_id = $_GET['id'];
  else {
	$url_id = rand(1000, 9999) . md5($data1['bnsprof_id']);
  }
  ?>
  
   
   <a data-prev=""  class="ajax" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo $url_id; ?>&pid=<?php echo $run_pre->pd_id; ?>&geo=<?php echo $geo_id;?>&conty=<?php echo $con_id;?><?php echo (($_GET['vform'] == 1)?'&vform=1':''); ?>" <?php echo ($_GET['vform'] == 1)?'style="display:none;"':''; ?>><img src="https://arabyos.com/company/images/2.png" style="height: 61px; float: left; margin-top: 44px; cursor: pointer;"  /></a>
  
  <?php }} ?>
     <!--****************** end left arrow *********************-->

        <img height="163" width="250"src="https://arabyos.com/upload/myproduct/<?php echo $p_row->pd_image ?>" >
        
     <!--******************  right arrow *********************-->
    <?php if($run_nxt!=''){ 
	if($_GET['keywords']!=""){?>

 <a data-next="" id=""  class="ajax" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']);?>&pid=<?php echo $run_nxt->pd_id; ?>&keywords=<?php echo $_GET['keywords'];?>&geo=<?php echo $geo_id;?>&conty=<?php echo $con_id;?>"><img src="https://arabyos.com/company/images/play.png" style="height: 61px; float: right; margin-top: 44px; cursor: pointer;"  /></a>
 <?php }
  else{ 
  ...
  if($_GET['vform'] == 1)
	...
	$url_id = rand(1000, 9999) . md5($data['bnsprof_id']);
  }
  ?>
  <a data-next=""  class="ajax" id=""   href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo $url_id;?>&pid=<?php echo $run_nxt->pd_id; ?>&geo=<?php echo $geo_id;?>&conty=<?php echo $con_id;?><?php echo (($_GET['vform'] == 1)?'&vform=1':''); ?>" <?php echo ($_GET['vform'] == 1)?'style="display:none;"':''; ?>><img src="https://arabyos.com/company/images/play.png" style="height: 61px; float: right; margin-top: 44px; cursor: pointer;"  /></a>
  <?php }} ?>
     <!--****************** end right arrow *********************-->
