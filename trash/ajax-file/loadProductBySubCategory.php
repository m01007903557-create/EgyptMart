...
<?php
$sql_prd="select * from products,measurement_unit,country, business_profile, plan_member_id  where mu_id=pd_unit and pd_currency=cn_id ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1' and plan_member_id.expiry_date > ". time() ." and pd_image!='' and md5(pd_subcat_id)='".$pc_id."'  ORDER BY FIELD(p_id,'5','4','3','15') ";

$res_prd=mysql_query($sql_prd);
?>

<div style="border:1px solid #F5ECFF;border-radius:5px;padding-left:10px;">
<h4 style="color:#36006C"><a href="index.php" style="text-decoration:none;color:#36006C">Home</a>&nbsp;&raquo;&nbsp;<a href="dir.php#main_cat_<?php echo $row_pcat[0];?>" style="text-decoration:none;color:#36006C"><?php echo ucwords($row_pcat[1]); ?></a>&nbsp;&raquo;&nbsp;<a href="products.php?c=<?php echo md5($row_pcat[2]);?>" style="text-decoration:none;color:#36006C"><?php echo ucwords($row_pcat[3]); ?></a>&nbsp;&raquo;&nbsp;<?php echo ucwords($row_pcat[4]); ?></h4>
</div>
...
<div class="als-container" id="product_slider" style="border:1px solid #F5ECFF;border-radius:5px;">
  ...
  <div class="als-viewport" align="center">
	...
	if(mysql_num_rows($res_prd)>0)
	{
	while($row_prd=mysql_fetch_object($res_prd))
	{
		 ...
		 if(isset($_COOKIE['loc_id']))
	{
      ...
      <span style="color:red;"><?php echo $cn_name; ?></span>
      </div>
       <hr />
      <hr />
<div style="height:10%;margin-top:16%; font-size:11px;">MOQ: <span style="color:red; font-weight: 600; font-size:15px;"><?php echo $row_prd->pd_min_order_qty; ?>&nbsp;</span><?php echo $row_prd->mu_name; ?></div>
