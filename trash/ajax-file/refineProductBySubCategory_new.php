...
$start = $page * $per_page;
    ...
    */
    ...
    if(isset($_POST['is_sub'])&&$_POST['is_sub']=="true") {
        ...
        $sql_check1="select pc_id from product_category_arabyos where  md5(pc_parent_id)='".$pc_id."'";
...
/**/
 ...
 where c2.cn_id=pd_currency AND bnsprof_city=ct_id AND mu_id=pd_unit and /*pd_currency=cn_id*/ cn_id=ct_cn_id ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1'  ANd smembership_plan.mp_id = plan_member_id.p_id and pd_image!='' and plan_member_id.expiry_date > ". time() ." ".$sql_sub_cat." ". $member ." ".$minorder." $country $sql_pd_city $sql_pd_state ORDER BY FIELD(p_id,'5','4','3','15') LIMIT ".$start.", ".$per_page;
   ...
   // echo $sql_prd;exit;
    ...
    /* -----Total count--- */
    //$query_pag_num = "SELECT count(*) AS count from products,measurement_unit,country, business_profile, plan_member_id where mu_id=pd_unit and pd_currency=cn_id ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1' and pd_image!='' and plan_member_id.expiry_date > ". time() ." ".$sql_sub_cat." ". $member." ".$minorder; // Total records
	$query_pag_num="select count(*) AS count from products,measurement_unit,country, business_profile, plan_member_id, smembership_plan,city
 where bnsprof_city=ct_id AND mu_id=pd_unit and /*pd_currency=cn_id*/ cn_id=ct_cn_id ".$sql_pd_ck." and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1'  ANd smembership_plan.mp_id = plan_member_id.p_id and pd_image!='' and plan_member_id.expiry_date > ". time() ." ".$sql_sub_cat." ". $member ." ".$minorder." $country $sql_pd_city $sql_pd_state";
