...
$sql_expired="select * from sale_offer where so_usr_id='".$_SESSION['uid_indm']."' and so_approval_status='1' and so_status='1' and DATE_ADD(so_approval_date,INTERVAL so_validity DAY)<now() order by so_updated_date desc LIMIT ".$start.", ".$per_page;

$recObj=mysqli_query($con, $sql_expired) or die('MySql Error' . mysql_error());
...
<div id="expired_offer">
			...
			// FOR ENABLING THE FIRST BUTTON
				if ($first_btn && $cur_page > 1) {	?>
					<a href="javascript:showExpired('1')"><img id="firstmail" src="images/firsten.gif"></a>
			...
			?>&nbsp;<?php
             ...
             // FOR ENABLING THE PREVIOUS BUTTON
	            if ($previous_btn && $cur_page > 1){
                 $pre = $cur_page - 1;
            ?><a href="javascript:showExpired('<?php echo $pre; ?>')"><img id="prevmail" src="images/prven.gif"></a><?php	
			...
			// TO ENABLE THE NEXT BUTTON
                if($next_btn && $cur_page < $no_of_paginations){
  	                $nex = $cur_page + 1;
   	        ?><a href="javascript:showExpired('<?php echo $nex; ?>')"><img id="nextmail" src="images/nxten.gif"></a><?php
                }else if ($next_btn){
			...
			?><a href="javascript:showExpired('<?php echo $no_of_paginations; ?>')"><img id="lastmail" src="images/lastenv.gif"></a>
			...
			{
			?>
            <tr><td colspan="4" align="center" style="vertical-align:middle;color:#F00;padding-top:10px; padding-bottom:10px;">No expired Sale Offer till date</td></tr>
            <?php	}	?>
			</tbody></table>
