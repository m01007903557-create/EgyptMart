...
$blockedUser="";
if(getBlockedUserList($_SESSION['uid_indm'])!='')
{
	$blockedUser=" and usr_id not in(".getBlockedUserList($_SESSION['uid_indm']).")";
}
$sql="select * from message,user,business_profile where usr_id=bnsprof_uid and if(msg_to='".$_SESSION['uid_indm']."', msg_from=usr_id,msg_to=usr_id) and msg_id in(select max(msg_id) from message where msg_to='".$_SESSION['uid_indm']."' or msg_from='".$_SESSION['uid_indm']."' group by if(msg_to='".$_SESSION['uid_indm']."', msg_from,msg_to)) and usr_id!='".$_SESSION['uid_indm']."' ".$blockedUser." order by msg_date desc LIMIT ".$start.", ".$per_page;
...
/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from message,user,business_profile where usr_id=bnsprof_uid and if(msg_to='".$_SESSION['uid_indm']."', msg_from=usr_id,msg_to=usr_id) and msg_id in(select max(msg_id) from message where msg_to='".$_SESSION['uid_indm']."' or msg_from='".$_SESSION['uid_indm']."' group by if(msg_to='".$_SESSION['uid_indm']."', msg_from,msg_to)) and usr_id!='".$_SESSION['uid_indm']."' ".$blockedUser; // Total records
	...
	<span style="display: block;" class="pagenav" id="pagenav">
       	<span class="pagenav" id="pagenav">
			...
			// FOR ENABLING THE FIRST BUTTON
				if ($first_btn && $cur_page > 1) {	?>
					<a href="javascript:showAddressBook('1')"><img id="firstmail" src="images/firsten.gif"></a>
			...
			?>&nbsp;<?php
             ...
             // FOR ENABLING THE PREVIOUS BUTTON
	            if ($previous_btn && $cur_page > 1){
                 $pre = $cur_page - 1;
            ?><a href="javascript:showAddressBook('<?php echo $pre; ?>')"><img id="prevmail" src="images/prven.gif"></a><?php	
			...
			// TO ENABLE THE NEXT BUTTON
                if($next_btn && $cur_page < $no_of_paginations){
  	                $nex = $cur_page + 1;
   	        ?><a href="javascript:showAddressBook('<?php echo $nex; ?>')"><img id="nextmail" src="images/nxten.gif"></a><?php
                }else if ($next_btn){
			...
			?><a href="javascript:showAddressBook('<?php echo $no_of_paginations; ?>')"><img id="lastmail" src="images/lastenv.gif"></a>
		...
		<div class="ab2">
            ...
            <div class="ab4">
                ...
                <tbody>
                	<tr>
                    	<td width="414"><a onclick="detailContact(<?php echo $row->usr_id; ?>,<?php echo $row->msg_id; ?>,<?php echo $cur_page; ?>)" style="cursor:pointer;"><strong><?php echo $row->name_prefix." ".$row->fname." ".$row->lname; ?></strong> (<?php echo $row->email; ?>)<br><span><?php echo $row->bnsprof_compname; ?> &nbsp;</span></a></td>
                        <td width="80">
							<?php echo get_country_name($row->country); ?><!--<img src="images/gh_flag_s.png" title="Ghana" class="abm5" border="0" height="13" width="20">-->
                        </td>
                        <td width="100">
                        
                    <ul id="ulmy_text_input1" class="starRating webwidget_rating_simple">
                    <?php
						$sql_r="select * from review_rating where rr_from_usr='".$_SESSION['uid_indm']."' and rr_to_usr='".$row->usr_id."'";
						$res_r=mysqli_query($con, $sql_r);
						$row_r=mysqli_fetch_object($res_r);
						$activeStar=$row_r->rr_rating;

					?>
                    
                    <?php for($star=1;$star<=$activeStar;$star++){ ?>
                    	<li class="starActive"><span><?php echo $star; ?></span></li>
                    <?php } ?>
                    <?php for(;$star<=5;$star++){ ?>
                        <li class="starDactive"><span><?php echo $star; ?></span></li>
                    <?php } ?>
                    	<!--<li style="background-image:url(images/sth.gif)"><span>1</span></li>
                        <li style="background-image:url(images/sth.gif)"><span>2</span></li>
                        <li style="background-image:url(images/nst.gif)"><span>3</span></li>
                        <li style="background-image:url(images/nst.gif)"><span>4</span></li>
                        <li style="background-image:url(images/nst.gif)"><span>5</span></li>-->
                    </ul>
                    <div style="clear:both;display:none"></div>
                    </td>
                    <td width="120"><?php echo date("d M Y",strtotime($row->msg_date)); ?></td>
                    </tr>
                </tbody>
                </table>
			</div>
            ...
            <div class="ab4">
                ...
                <tbody>
                	<tr>
                    <td align="center"><font color="#FF0000">No Contacts listed.</font></td>
                    </tr>
                </tbody>
                </table>
            </div>
