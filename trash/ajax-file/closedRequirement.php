...
$sql_inbox="select * from buy_requirement where br_approval_status!='2' and br_display_status='0' and br_status='1' and br_u_id='".$_SESSION['uid_indm']."' order by br_updated_date desc LIMIT ".$start.", ".$per_page;

$recObj=mysqli_query($con, $sql_inbox) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from buy_requirement where br_approval_status!='2' and br_display_status='0' and br_status='1' and br_u_id='".$_SESSION['uid_indm']."'"; // Total records
...
<span class="pagenavigation">
			...
			// FOR ENABLING THE FIRST BUTTON
				if ($first_btn && $cur_page > 1) {	?>
					<a href="javascript:closedRequirement('1')"><img id="firstmail" src="images/firsten.gif"></a>
			...
			?>&nbsp;<?php
             ...
             // FOR ENABLING THE PREVIOUS BUTTON
	            if ($previous_btn && $cur_page > 1){
                 $pre = $cur_page - 1;
            ?><a href="javascript:closedRequirement('<?php echo $pre; ?>')"><img id="prevmail" src="images/prven.gif"></a><?php	
			...
			// TO ENABLE THE NEXT BUTTON
                if($next_btn && $cur_page < $no_of_paginations){
  	                $nex = $cur_page + 1;
   	        ?><a href="javascript:closedRequirement('<?php echo $nex; ?>')"><img id="nextmail" src="images/nxten.gif"></a><?php
                }else if ($next_btn){
			...
			?><a href="javascript:closedRequirement('<?php echo $no_of_paginations; ?>')"><img id="lastmail" src="images/lastenv.gif"></a>
...
<div class="bulletImage"><a id="apprvDel1" class="apprvDel" style="cursor:pointer;" onClick="repostRequirement(<?php echo $row->br_id; ?>,'cl');">Repost</a></div>
<div class="bulletImage"><a id="apprvDel1" class="apprvDel" style="cursor:pointer;" onClick="delRequirement(<?php echo $row->br_id; ?>,'cl');">Delete</a></div>
</td>
	</tr>
<?php	}	?>
</tbody></table></div>
...
<?php	}else{	?>
	<div style="font-size:12px;color:#b60000;padding: 10px 0 10px 0;" align="center">There are no Closed Requirements to manage.</div>    
<?php	}	?>
