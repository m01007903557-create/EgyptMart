...
/*else if(strlen($pd_pck_dets)>2000)
{
	$data[0]="0";
	$data[1]='<font color="#CC0000">Packaging Details cannot have more than 2000 characters</font>';
	$valid=false;
}*/
else
{
...
if($valid==true)
{	
	$tmpimgsql=mysql_query("select * from temp_product_image where tpi_usr_id='".$uid."'");
	...
	$sql="insert into products
		set 
			...
			pd_desc='".$pd_desc."',
			pd_image='".$tmpimagerow->tpi_image."',
            pd_imagelogo='".$tmpimagerow->tpi_logo."',
			pd_date=now(),
			...
			//mail('tezibufir@zdfpost.net', 'My Subject', $sql);
	...
	//print_r(mysql_fetch_object($res12));die;
