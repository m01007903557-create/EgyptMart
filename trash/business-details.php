...
{	
	...
	function update()
	{
		...
		$sqlchk="select * from business_profile_arabyos where bnsprof_uid='".$this->userd."'";
		$reschk=mysql_query($sqlchk);
		if(mysql_num_rows($reschk)>0)
		{
			$sql="update business_profile_arabyos set		
		...
		{
			$sql="insert into business_profile_arabyos
				set
...
if(isset($_SESSION['bnsprof_yoe'])){ $bnsprof_yoe=$_SESSION['bnsprof_yoe'];	unset($_SESSION['bnsprof_yoe']); }else{ $bnsprof_yoe=""; }
if(isset($_SESSION['bnsprof_comemp'])){	$bnsprof_comemp=$_SESSION['bnsprof_comemp']; unset($_SESSION['bnsprof_comemp']); }else{ $bnsprof_comemp=""; }
if(isset($_SESSION['bnsprof_turnover'])){ $bnsprof_turnover=$_SESSION['bnsprof_turnover']; unset($_SESSION['bnsprof_turnover']); }else{ $bnsprof_turnover=""; }
if(isset($_SESSION['bnsprof_owntype'])){ $bnsprof_owntype=$_SESSION['bnsprof_owntype'];	unset($_SESSION['bnsprof_owntype']); }else{ $bnsprof_owntype=""; }

if(isset($_POST['btnUpdate'])){
	
 	$ecms->company_name=trim(addslashes($_POST['company_name']));
	$ecms->website_alt=trim(addslashes($_POST['website_alt'])); 
	$ecms->bnsprof_businesstype=$_POST['bnsprof_businesstype'];
	$ecms->bnsprof_yoe=trim(addslashes($_POST['bnsprof_yoe']));
	$ecms->bnsprof_comemp=trim(addslashes($_POST['bnsprof_comemp']));
	$ecms->bnsprof_turnover=trim(addslashes($_POST['bnsprof_turnover']));
	$ecms->bnsprof_owntype=trim(addslashes($_POST['bnsprof_owntype']));
...
$sql="select * from business_profile_arabyos where bnsprof_uid='".$uid."'"; 
$res=mysql_query($sql);
...
<div class="inner_wrapper">
	...
	<style type="text/css">.max{color:#fa5901; font-size:11px; margin-top:5px} .s_u{width:144px} .frm_a{width:95%; border:1px solid #e0f0fd; padding:10px}.label {color: #000 !important; }</style>
	<div> 
	...
	<?php include 'includes/business-panel.php';?>
		...
		<div id="re_link" class="utab" style="height:39px;"><span style="font-size: 14px;" class="f1">Complete your Business Details to attract genuine buyers.</span></div>
 ...
 <div style="text-align:left;width:389px;padding:1% 1% 1% 5%;display:block;margin-left:87px;" class="" id="updatemessage"><?php echo $msg; ?></div>
		...
		<tbody><tr>
			<td class="label" style="text-align:right" width="135">Company Name</td>
			<td><div id="a1" class="tbp cona" style="display:none"><div class="t1a">We suggest that you use a genuine and complete company name such as <b>'Perfect Systems Ltd.'</b> Buyers and Suppliers prefer to interact with companies with complete and accurate company name.</div></div>
   <input maxlength="60" name="company_name" id="company_name" value="<?php echo ucwords(user_info($uid,'bnsprof_compname'));?>" class="a_f rf" onFocus="javascript:ntt('comp_name','a1');" onBlur="nhid('a1')" tabindex="1"></td>
		</tr>

		<tr>
			<td class="label pt7" valign="top">Company Logo</td>
			<td>
  <!--<iframe name="iform_middle" framespacing="0" marginwidth="0" marginheight="0" src="upload-companylogo.php" scrolling="no" width="322" frameborder="0" height="114"></iframe>-->
  <script type="text/javascript">
...
function DelTempImage(imid)
{
   ...
   if(cnf==true)
   {
	...
	jQuery(document).ready(function(){
			//var url ='https://www.arabyos.com/server/php/';
...
<script type="text/javascript">list_photo()</script>
 ...
 <div class="upload_div">
	...
	<span class="file_input">Add image</span>
		...
		<tr>
			<td class="label">Alternate Website</td>
			<td><div id="a3" class="tbp cona" style="display:none"><div class="t1a">If you have a website other than <?php echo getWebSiteName(); ?> catalog, please enter its URL here.<br><b>Example: www.yourwebsite.com</b></div></div> 
<input class="a_f rf" maxlength="80" name="website_alt" id="website_alt"  value="<?php echo user_info($uid,'bnsprof_website_alt')?>" tabindex="3">
			<span id="wb" class="em" style="display:none"></span><span id="wb1" class="em" style="display:none"></span>
			</td>
		</tr> 

		<tr>
			<td class="label pt7" valign="top">Business Type</td>
			<td>
			<table style="margin:0px;" cellpadding="0" cellspacing="0" width="70%">
			<tbody>
            <?php
			$c=1;
            $bstypsql=mysql_query("select * from business_type_arabyos where bsntyp_status='1'");
			while($bstyprow=mysql_fetch_object($bstypsql)){
			if($c==1)
			{
				echo "<tr>";	
			}
            ...
            <?php echo $bstyprow->bsntyp_title; ?>
            </td>            
            <?php 
			if($c==3)
			{ 
			...
			<tr>
			<td class="label" width="138">Year of Establishment</td>
			<td><div id="a11" class="tbp cona" style="display:none"><div class="t1a">Please enter year in (YYYY format), <b>Example: 1998</b>  </div></div>
      <input maxlength="60" class="a_f rf" name="bnsprof_yoe" id="bnsprof_yoe" value="<?php echo user_info($uid,'bnsprof_yoe');?>">
		...
		<tr>
            ...
            <?php
			$c=1;
            $noempsql=mysql_query("select * from employee_range_arabyos where emprange_status='1'");
			while($noemprow=mysql_fetch_object($noempsql))
			{
			?>
 <option value="<?php echo $noemprow->emprange_id;?>" <?php if(user_info($uid,'bnsprof_comemp') == $noemprow->emprange_id){ ?> selected="selected" <?php } ?> >
		...
		<tr>
			<td class="label">Revenue Sales Turnover</td>
			<td>
            ...
            <?php
			$c=1;
            $turnoversql=mysql_query("select * from revenue_turnover_arabyos where revturnover_status='1'");
			while($turnoverow=mysql_fetch_object($turnoversql))
			{
			?>
		...
		<tr>
            ...
            <?php
			$c=1;
            $owntypesql=mysql_query("select * from ownership_type_arabyos where owntyp_status='1'");
			while($owntyperow=mysql_fetch_object($owntypesql))
			{
			?>
		...
		<tr>
			...
			<td align="left">
				...
				<input name="btnUpdate" id="btnUpdate" value="Update Details" class="saps mt5" tabindex="31" type="submit">
                </td><td> <span id="pf_save" style="display:none;margin-left:15px;margin-top:6px;">
                <img src="images/loading.gif" alt="" border="0" width="16" height="11"></span> </td></tr></tbody></table>
			</td>
		</tr>
		</tbody></table>
		<div class="clb">&nbsp;</div>
