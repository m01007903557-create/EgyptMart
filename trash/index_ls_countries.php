...
//echo $sql_pd_ck; die;
	...
	//echo $view_country2; 	
			...
			if(isset($_COOKIE['loc_id'])){
				$view_country2 = "select business_profile.bnsprof_state from products INNER JOIN business_profile ON business_profile.bnsprof_uid = products.pd_uid  where ((pd_title LIKE '%".$keywords."%') or (bnsprof_compname LIKE '%".$keywords."%')) and ((pd_preferred_buyer_location =  'domestic' AND products.pd_currency =  '".$_COOKIE['loc_id']."') OR (pd_preferred_buyer_location =  'any' AND products.pd_currency =  '".$_COOKIE['loc_id']."') OR (pd_preferred_buyer_location =  'my_city' AND products.pd_currency =  '".$_COOKIE['loc_id']."')) group by products.pd_uid HAVING business_profile.bnsprof_state >0";
	//echo $view_country2; 	
			}else{
					$view_country2 = "select business_profile.bnsprof_state from products INNER JOIN business_profile ON business_profile.bnsprof_uid = products.pd_uid where ((pd_title LIKE '%".$keywords."%') or (bnsprof_compname LIKE '%".$keywords."%')) and ((pd_preferred_buyer_location =  'domestic' AND products.pd_currency =  '".$location_geo_country[0]."') OR (pd_preferred_buyer_location =  'any' AND products.pd_currency =  '".$location_geo_country[0]."') OR (pd_preferred_buyer_location =  'my_city' AND products.pd_currency =  '".$location_geo_country[0]."' )) group by products.pd_uid HAVING business_profile.bnsprof_state >0";
			}
		} 
		 //else if($_GET['rctyp'] == 'Products') {
		else {
			$view_country2 = "select business_profile.bnsprof_state from products,business_profile,plan_member_id  where pd_image!=''  and (products.pd_title LIKE '%".$_GET['keywords']."%' or bnsprof_compname LIKE '%".$_GET['keywords']."%') and business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id =business_profile.bnsprof_id AND plan_member_id.expiry_date > " . time() . " and pd_status='1' ".$sql_pd_ck." group by products.pd_uid";
			//echo $view_country2;
		}
...
?>
		...
		if($_GET['keywords']){ $keywordsval = $_GET['keywords'];}	?>   
          <section class="ar-flags">
            <form action="search.php?rctyp=<?php echo $rctypval;?>&keywords=<?php echo $keywordsval;?>"  method="post"  >
               ...
               //  echo $view_country;
       if($_GET['rctyp'] == 'buy_lead'){ 
			// $view_country = "select country,br_estimate_qty_unit,mu_id,bnsprof_uid,br_u_id,b_id,bnsprof_id,cn_id,expiry_date,br_display_status,br_approval_status from buy_requirement,measurement_unit,country,business_profile,user,plan_member_id where br_estimate_qty_unit=mu_id and business_profile.bnsprof_uid = buy_requirement.br_u_id and plan_member_id.b_id =business_profile.bnsprof_id and country=cn_id AND plan_member_id.expiry_date > " . time() .$keywords_string."  and br_approval_status='1' ".$sql_br_ck."  group by br_u_id";
		    $view_country = "select country,br_estimate_qty_unit,mu_id,bnsprof_uid,br_u_id,bnsprof_id,cn_id,br_display_status,br_approval_status from buy_requirement,measurement_unit,country,business_profile,user where br_estimate_qty_unit=mu_id and business_profile.bnsprof_uid = buy_requirement.br_u_id  ".$keywords_string."  and br_approval_status='1' ".$sql_br_ck."  group by br_u_id";
	//echo $view_country; 	
		} 
		else if($_GET['rctyp'] == 'tender'){
			$tender_keywords_string = generateTenderSearchString($keywords);
	...
	$view_country = "select * from tender,product_category_arabyos,user,business_profile,plan_member_id where tnd_pc_id=pc_id and tnd_usr_id=usr_id and usr_id=bnsprof_uid and business_profile.bnsprof_uid = tender.tnd_usr_id and plan_member_id.b_id =business_profile.bnsprof_id AND plan_member_id.expiry_date > " . time() . "  AND (".$tender_keywords_string.") and tnd_due_date>='".date('Y-m-d')."' AND tnd_approval_status = '1' ".$tenderCondition." group by tender.tnd_currency ORDER BY tnd_id desc ";//and TO_DAYS(tnd_docSaleEnd_date)>=TO_DAYS(now()) 
		}else if($_GET['rctyp'] == 'Suppliers'){
			if(isset($_COOKIE['loc_id'])){
			
				if($_GET['keywords'] != ""){
					$view_country = "select * from products,measurement_unit,country,business_profile,user,plan_member_id where usr_id = pd_uid AND bnsprof_uid = pd_uid AND mu_id=pd_unit AND ((pd_title LIKE '%".$keywords."%') or (bnsprof_compname LIKE '%".$keywords."%')) and business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id =business_profile.bnsprof_id and pd_currency=cn_id and ((pd_preferred_buyer_location =  'domestic' AND user.country =  '".$_COOKIE['loc_id']."') OR (pd_preferred_buyer_location =  'any' AND user.country =  '".$_COOKIE['loc_id']."') OR (pd_preferred_buyer_location =  'my_city' AND user.country =  '".$_COOKIE['loc_id']."')) AND plan_member_id.expiry_date > " . time() . " and pd_status='1' and pd_image!='' group by pd_currency";
				}else{
					$view_country = "select * from products,measurement_unit,country,business_profile,user,plan_member_id where usr_id = pd_uid AND bnsprof_uid = pd_uid AND mu_id=pd_unit AND business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id =business_profile.bnsprof_id and user.country=cn_id and ((pd_preferred_buyer_location =  'domestic' AND user.country =  '".$_COOKIE['loc_id']."') OR (pd_preferred_buyer_location =  'any' AND user.country =  '".$_COOKIE['loc_id']."') OR (pd_preferred_buyer_location =  'my_city' AND user.country =  '".$_COOKIE['loc_id']."')) AND plan_member_id.expiry_date > " . time() . " and pd_status='1' and pd_image!='' group by user.country";
				}
			}else{
				if($_GET['keywords'] != ""){
					$view_country = "select * from products,measurement_unit,country,business_profile,plan_member_id where mu_id=pd_unit and ((pd_title LIKE '%".$_GET['keywords']."%') or (bnsprof_compname LIKE '%".$_GET['keywords']."%')) and business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id =business_profile.bnsprof_id  and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' AND plan_member_id.expiry_date > " . time() . " and pd_image!='' group by pd_id";
				}else{
					$view_country = "select * from products,measurement_unit,country,business_profile,user,plan_member_id where usr_id = pd_uid and mu_id=pd_unit and business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id =business_profile.bnsprof_id  and user.country=cn_id ".$sql_pd_ck." and pd_status='1' AND plan_member_id.expiry_date > " . time() . " and pd_image!='' group by user.country";
				}
			}

	  }else { 
		  if($_GET['idd']!=""){
			  $view_country="select * from products,measurement_unit,country,business_profile,plan_member_id where mu_id=pd_unit and pd_subcat_id = ".$_GET['idd']." and business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id =business_profile.bnsprof_id and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' AND plan_member_id.expiry_date > " . time() . " and  pd_image!='' group by pd_currency ";
		 }else{
			$view_country = "select * from products,measurement_unit,country,business_profile,plan_member_id where mu_id=pd_unit and (pd_title LIKE '%".$_GET['keywords']."%' or bnsprof_compname LIKE '%" . $_GET['keywords'] . "%') and business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id =business_profile.bnsprof_id  and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' AND plan_member_id.expiry_date > " . time() . " and pd_image!='' group by pd_id";
			...
			// echo '<pre>';print_r($_POST['country_id']);exit;
  ...
  ?>
              <div class="checkbox" style="text-align:left;">
                ...
                $sql = mysql_query("select measurement_unit.*,country.*,business_profile.*,products.*,user.usr_id,user.email,user.fname,user.website,user.country,user.image,user.country_ph_code,user.profileImage, MATCH (pd_title) AGAINST ('beans' IN BOOLEAN MODE) AS title_relevance from products,measurement_unit,country,business_profile,user,plan_member_id where 
user.usr_id = bnsprof_uid
 and bnsprof_uid=pd_uid
 and b_id = bnsprof_id 
and mu_id=pd_unit 
and (pd_title LIKE '%".$_GET['keywords']."%' or bnsprof_compname LIKE '%".$_GET['keywords']."%') 
