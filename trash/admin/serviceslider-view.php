...
class sliderviewlist{
	...
	function deleterecord($adid){
		global $con;
	    $chquesql="select * from prodservice_slider where adv_id ='".$adid."' ";
		$chqueres=mysqli_query($con, $chquesql);
		$chquerow=mysqli_fetch_array( $chqueres);
		$path="../upload/service_slider/".$chquerow['adv_img'] ;
		unlink($path);
		$sql_del="delete from prodservice_slider where adv_id='".$adid."'";
	...
	function deletelink($id){
		if($_SERVER['QUERY_STRING']==""){
			...
			$dellink="serviceslider-view.php?".$_SERVER['QUERY_STRING']."&action=del&fid=".$id;
		}
		return $dellink;
	}
}
...
/********************delete record*********************/
	if(isset($_GET['action']) && $_GET['action']=="del"){
		...
		//header("location:productslider-view.php?".rtrim($_SERVER['QUERY_STRING'],"&action=del&id=".$_GET['id']));
		}
/***********************************************/

$al->limit=$p->setlimit(20);
$al->setsql("select * from prodservice_slider where adv_type='2' order by adv_updated_date desc");
	...
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb">
			<li>
				...
				<li>
					<a href="productslider-view.php">Manage Product Slider</a>
				</li>
...
<button class="btn btn-xs btn-danger" name="btnDelete" type="submit" onClick="return confirm('Are you sure to delete the record?')" ><i class="icon-trash bigger-120"></i>Delete</button>
 <button type="button" class="btn btn-xs btn-success" onClick="window.location='serviceslider-add.php' "><i class="icon-pencil align-top bigger-120"></i>Add Slider</button>
 </div>
...
$count=mysqli_num_rows($recObj);
	if($count >0)
	{
        ...
        <tr>
			...
			<td style="text-align:center;"><img src="../upload/service_slider/<?php echo $row->adv_img; ?>" width="200px;" height="150px;"/></td>
			<td style="text-align:center">
			<?php echo '<strong>Link:</strong> '.$row->adv_link;?>
			<?php echo '<br/><br/><strong>Heading:</strong> '.$row->adv_title;?>
			<?php 
			  echo '<br/><strong>MOQ data:</strong> '.$row->adv_description;
			 ...
			 echo '<br/><br/><strong>Fob price:</strong> '.$currency_symbols[$row->adv_currency].$row->adv_price;
			  echo '<br/><br/><strong>MOQ unit:</strong> '.$row->adv_piece;
			     echo '<br/><strong>Unit type:</strong> '.$row->unit_type;
			 echo '<br/><br/><strong>Title:</strong> '.$row->adv_title;
			  echo '<br/><br/><strong>Membership Icon:</strong> <img src="../images/'.$row->adv_icon.'" alt="">';
			?>
			</td>
          ...
          <script>
			...
			{
				$.post("ajax-file/productslider-change-status.php", {stat:stat,id:id},
         		function(data){
				location.reload();		
        		});
			}
            ...
            <td align="center">
     <a href="serviceslider-edit.php?aid=<?php echo $row->adv_id; ?>" title="edit"><img alt="edit" src="images/edit.jpg" border="0"></a>
