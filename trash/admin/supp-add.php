...
class addAdvertisement{
	...
	var $adv_country;
	var $adv_global;
	function __construct($imgsize, $adv_img, $adv_link,$adv_country,$adv_global)
	{		
	...
	{	
	   global $con;
		if($_FILES["adv_img"]["name"] != "")
		{
			...
			{
				$this->adv_imagewidth=substr($this->imgsize,0,2);
				$this->adv_imageheight=substr($this->imgsize,3,2);
				...
				$imgSImage->save("../upload/supplier_logo/".$this->adv_img);	
				
				//$ds = move_uploaded_file($_FILES["adv_img"]["tmp_name"], "../upload/supplier_logo/".$this->adv_img) or die('error');
															
				$sql="insert into supplier_logo
						set			
...
{ 	
	...
	$adn=new addAdvertisement(addslashes(trim($_POST['imgsize'])), trim($_FILES['adv_img']['name']), addslashes(trim($_POST['adv_link'])),$_POST['adv_country'],$_POST['adv_global']);
	if($adn->valid())
	{	
		$adn->add();		
	}
...
<div class="main-content">
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb">
			...
			<li>
				<a href="supp-view.php">Supplier Logo</a>
			</li>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<select name="imgsize" id="imgsize" class="chosen-select">
			        <option value="90x90" <?php if($imgsize=="90x90"){ ?> selected="selected"<?php } ?>>90 x 90</option>
				</select>
		</div>
	</div>
    <div class="form-group">
