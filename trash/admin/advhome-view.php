...
/********************delete record*********************/
	if(isset($_GET['action']) && $_GET['action']=="del"){
		...
		//header("location:advhome-view.php?".rtrim($_SERVER['QUERY_STRING'],"&action=del&id=".$_GET['id']));
		}
/***********************************************/
...
<button class="btn btn-xs btn-danger" name="btnDelete" type="submit" onClick="return confirm('Are you sure to delete the record?')" ><i class="icon-trash bigger-120"></i>Delete</button>
 <button type="button" class="btn btn-xs btn-success" onClick="window.location='advhome-add.php' "><i class="icon-pencil align-top bigger-120"></i>Add Advertisement</button>
 </div>
...
$count=mysqli_num_rows($recObj);
	if($count >0)
	{
        ...
        <tr>
			...
			<td style="text-align:center;"><img src="../upload/advertisementhome/<?php echo $row->adv_img; ?>" width="200px;" height="150px;"/></td>
          ...
          <script>
			...
			{
				$.post("ajax-files/advhome-change-status.php", {stat:stat,id:id},
         		function(data){
				location.reload();		
        		});
			}
            ...
            <td align="center">
     <a href="advhome-edit.php?aid=<?php echo $row->adv_id; ?>" title="edit"><img alt="edit" src="images/edit.jpg" border="0"></a>
