...
class AdminLoginlist{
	...
	function deleterecord($adid){
		global $con;
		mysqli_query($con, "update contact_us set cu_status= 0 where cu_id='".$adid."'");
	}
...
<?php $j=0;
	...
	if($count >0)
	{
		...
		<tr>
			<?php } ?>
	        <td class="center"><label><input name="cb[]"  class="ace" type="checkbox" value="<?php echo $row->cu_id; ?>" /><span class="lbl"></span></label></td>
			<td><?php echo date('d/m/Y', strtotime($row->cu_updated_date)); ?></td>
			<td><?php echo ucfirst($row->cu_fname)." ".ucfirst($row->cu_lname); ?></td>
			<td><a href="mailto:<?php echo $row->cu_email ?>"><?php echo $row->cu_email ?></a></td>
			
<td><?php echo $row->cu_contactnumber; ?></td>
<td><?php echo ucfirst($row->cu_country)."-".ucfirst($row->cu_state); ?></td>
<td style="text-align:center"><a href="contact-details.php?fid=<?php echo $row->cu_id ?>" title="Details"><img src="images/details.png"></a><?php
			if($row->replied == 1){ ?>
			<i class="icon-reply icon-only"></i>
			<?php } ?></td>
			<td style="text-align:center"><a href="<?php echo $al->deletelink($row->cu_id)?>" title="Delete" onclick="return confirm('Are you sure to delete the record?')"><img src="images/delete.jpg" border="0"></a></td>
		</tr>
        <?php $j++; } }?>
</tbody>
</table>
