...
class sliderviewlist{
	...
	function deleterecord($adid){
		global $con;
	    $chquesql="select * from supplier_logo where adv_id ='".$adid."' ";
		$chqueres=mysqli_query($con, $chquesql);
		$chquerow=mysqli_fetch_array( $chqueres);
		$path="../upload/supplier_logo/".$chquerow['adv_img'] ;
		unlink($path);
		$sql_del="delete from supplier_logo where adv_id='".$adid."'";
	...
	function deletelink($id){
		if($_SERVER['QUERY_STRING']==""){
			...
			$dellink="supp-view.php?".$_SERVER['QUERY_STRING']."&action=del&fid=".$id;
		}
		return $dellink;
	}
}
...
/********************delete record*********************/
	if(isset($_GET['action']) && $_GET['action']=="del"){
		...
		//header("location:supp-view.php?".rtrim($_SERVER['QUERY_STRING'],"&action=del&id=".$_GET['id']));
		}
/***********************************************/
	...
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb">
			<li>
				...
				<li>
					<a href="supp-view.php">Manage Supplier Logo</a>
				</li>
...
<button class="btn btn-xs btn-danger" name="btnDelete" type="submit" onClick="return confirm('Are you sure to delete the record?')" ><i class="icon-trash bigger-120"></i>Delete</button>
 <button type="button" class="btn btn-xs btn-success" onClick="window.location='supp-add.php' "><i class="icon-pencil align-top bigger-120"></i>Add Logo</button>
 </div>
...
$count=mysqli_num_rows($recObj);
	if($count >0)
	{
        ...
        <tr>
			...
			<td style="text-align:center;"><img src="../upload/supplier_logo/<?php echo $row->adv_img; ?>"/></td>
          ...
          <script>
			...
			{
				$.post("ajax-file/supp-change-status.php", {stat:stat,id:id},
         		function(data){
				location.reload();		
        		});
			}
            ...
            <td align="center">
     <a href="supp-edit.php?aid=<?php echo $row->adv_id; ?>" title="edit"><img alt="edit" src="images/edit.jpg" border="0"></a>
