...
class editProduct{
	...
	function detailsObj(){
		global $con;
		$sql="select * from message where msg_id=".$this->msg_id;
	...
	function valid(){
		...
		else if($this->msg_message == "" || $this->msg_message == " ")
		{
			$this->msg= '<div class="alert alert-danger"><i class="icon-remove"></i> Please enter Message Content</div>';
			$valid=false;
		}
		return $valid;
	}
...
if(isset($_POST['btnUpdate'])){
	...
	$ob->msg_subject=addslashes(trim($_POST['msg_subject']));
	$ob->msg_message=addslashes(trim($_POST['msg_message']));
	...
	header("location:message-edit.php?fid=".$ob->msg_id);
}
...
<form class="form-horizontal" action="" id="cmp_edit" name="cmp_edit" method="post" enctype="multipart/form-data" onSubmit="return myvalid();">
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<label style="padding-top:4px;"><?php echo date("d-M-Y (h:iA)",strtotime($row->msg_date)); ?></label>
		</div>
	</div>
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<label style="padding-top:4px;"><?php echo getUserInfo($row->msg_from,'name_prefix')." ".getUserInfo($row->msg_from,'fname')." ".getUserInfo($row->msg_from,'lname'); ?></label>
		</div>
	</div>
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<label style="padding-top:4px;"><?php echo getUserInfo($row->msg_to,'name_prefix')." ".getUserInfo($row->msg_to,'fname')." ".getUserInfo($row->msg_to,'lname'); ?></label>
		</div>
	</div>
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<input name="msg_subject" id="msg_subject" class="col-xs-10 col-sm-6" type="text" value="<?php echo $row->msg_subject; ?>" />
    ...
    <div class="form-group">
		...
		<div class="col-sm-8">
        <textarea id="msg_message" name="msg_message" class="autosize-transition form-control" style="overflow: hidden; word-wrap: break-word; resize: horizontal; height: 69px;"><?php echo $row->msg_message; ?></textarea>
		</div>
	</div>
