...
check_user_login();
	...
	function detailsObj(){
		global $con;
		$sql="select * from yahoo_slider_arabyos where adv_id=".$this->adv_id;
	...
	{			
	   global $con;
		if($_FILES["adv_img"]["name"] != '')
		{
			...
			{	
				$sqlImg="select * from yahoo_slider_arabyos where adv_id='".$this->adv_id."'";
				$resImg=mysqli_query($con, $sqlImg);
				$rowImg=mysqli_fetch_object($resImg);
				
//				$pathS="../upload/logo/small/".$rowImg->ht_logo;	
				$pathB="../upload/yahoo_slider/".$rowImg->adv_img;
...
//				$ds = move_uploaded_file($_FILES["adv_img"]["tmp_name"], "../upload/yahoo_slider/".$this->adv_img) or die('error');
				...
				{*/	
					...
					$sql="update yahoo_slider_arabyos
							set
...
if(isset($_POST['btnUpdate'])){
    ...
    $ob->adv_global=addslashes(trim($_POST['adv_global']));	
	...
	header("location:yahooslider-edit.php?aid=".$ob->adv_id);
}
?>
...
<form class="form-horizontal" action="" id="cmp_edit" name="cmp_edit" method="post" enctype="multipart/form-data" onSubmit="return myvalid();">
    ...
    <div class="form-group">
		...
		<div class="col-sm-8">
		    ...
		    <?php
			$sqlcnty="select * from country where cn_status=1 order by cn_name";
			...
			{
				$selected="";
				echo "CID=>".$countryid=$row_cntry->cn_id;
			...
			?>
                <?php if($singcntryid==-1) { ?>
				<option value="-1" <?php echo $selected;?>>Global</option>
                <?php } ?>
	...
	<span id="contmsg" style="color:#cc0000;"></span>
		...
		<div class="col-sm-9">
		<textarea name="adv_description" id="adv_description" rows="10" cols="60" onkeyup="countChar('adv_description','22','Words');" onkeyup="countChar('adv_description','22','Words');" required><?php echo $row->adv_description; ?></textarea>
		<div style="clear"></div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<img src="../upload/yahoo_slider/<?php  echo $row->adv_img;?>" />
		</div>
	</div>
    <div class="form-group">
