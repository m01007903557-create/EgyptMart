...
/********************approve record*********************/
	if(isset($_GET['action']) && $_GET['action']=="appr"){
		...
		$get_user="select * from user,tender,country,states,city where tnd_usr_id = usr_id and country = cn_id and state_id = ct_state and state_cn_id = cn_id  and tnd_id = ".$ten_id; 
		...
		$get_product = "SELECT * FROM tender WHERE tnd_id = '$ten_id'";
		...
		$unitsql=mysqli_query($con, "select * from measurement_unit where mu_status='1' AND mu_id = ".$sproduct->tnd_qty_mu_id);
		while($unitrow=mysqli_fetch_object($unitsql)){
			$product_type = $unitrow->mu_name;
		}
		//echo "<pre>"; print_r($sproduct); exit;
		$product_title = $spname = $sproduct->tnd_heading;
		/*Put Your Email Adress Here*/
			$subject = "Tender Approval From ".get_page_settings(4);
			$from_name = get_page_settings(4);
			$from_email = get_adminemail();
			$message = $suname."<br /><br />"."Your Tender <b>".$spname."</b> is approved";
			...
			if(mail($to, $subject, $comment, $headers)){
				//exit;
					//header("location:tender-view.php");	
					header('Location:../tender-email.php?tnd_id='.$ten_id);	
				}
		...
		<script type="text/javascript">
			$(document).ready(function(){
				var oTable1 = $('#sample-table-2').dataTable({				 
				 "ajax":{
					url :"tender-view-response.php", // json datasource
