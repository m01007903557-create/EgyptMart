...
// initilize all variable
	...
	while( $row = mysqli_fetch_object($queryRecords) ) {			
			...
			$res[2] = '<a href="'.$row->website.'" target="_blank">'.$row->website.'</a>';
			$res[3] = '<a href="user-details.php?token='.rand(1000,9999).md5($row->usr_id).'" target="_blank">'.ucwords($row->name_prefix." ".$row->lname." ".$row->fname).'</a>';
			$res[4] =  '+'.$row->country_ph_code.' '.$row->mobile1;
			$res[5] =  $row->email;
			$res[6] = $row->cn_name;
			$res[7] = $row->state_name;
			$res[8] = $row->ct_name;
			...
			$res[11] = '<a href="company-details.php?token='.md5($row->bnsprof_id).'"><img src="images/details.png" /></a>';
