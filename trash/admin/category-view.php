...
check_user_login();
$varcat="";
$mycatsql="select * from product_category_arabyos where pc_status='1' and pc_parent_id='0' order by pc_id";
...
class productlist{
	...
	function deleterecord($adid){
		global $con;
		mysqli_query($con, "delete from product_category_arabyos where pc_id='".$adid."'");
	}
	function deletelink($id){
		...
		if($_SERVER['QUERY_STRING']==""){
			...
			$plink="category-view.php?".$_SERVER['QUERY_STRING']."&action=paid&did=".$id;
		}
		return $plink;
	}
}
...
$al->setsql("select * from product_category_arabyos where pc_parent_id IN ($varcat5) and pc_parent_id!=0 and pc_status='1' order by pc_id desc");
...
<button class="btn btn-xs btn-danger" name="btnDelete" type="submit" onClick="return confirm('Are you sure to delete the record?')" ><i class="icon-trash bigger-120"></i>Delete</button>
 <button type="button" class="btn btn-xs btn-success" onClick="window.location='category-add.php' "><i class="icon-pencil align-top bigger-120"></i>Add Category</button>
 </div>
...
<?php $j=0;
   ...
   {			
		while($row=mysqli_fetch_object($recObj)){ 
           ...
           $maincatsql="select * from product_category_arabyos where pc_id='".$row->pc_parent_id."' and pc_status='1' order by pc_id";	
			$maincatres=mysqli_query($con, $maincatsql);
			$maincatrw=mysqli_fetch_array( $maincatres);
        ...
        <tr>
            ...
            <td><?php echo $row->pc_name; ?></td>
      		<td><?php  echo $maincatrw['pc_name'];?></td>   
      		<td><?php  echo $row->pc_sort_name; ?></td>    
            <td align="center">
            <a href="category-edit.php?token=<?php echo rand(1000,9999).md5($row->pc_id);?>" title="Edit">
            <img alt="edit" src="images/edit.jpg"></a>
            <a href="<?php echo $al->deletelink($row->pc_id)?>" title="delete" onclick="return confirm('Are you sure to delete the record?')">
