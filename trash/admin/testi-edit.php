...
include "../common.php";
	...
	function __construct($testi_id)
	{
          $this->testi_id=$testi_id;     
	}
	function detailsObj(){
		global $con;
		$sql="select * from testimonials where md5(testi_id)='".$this->testi_id."'";
	...
	if($this->testi_image != '')
	{
        ...
        if(in_array($ext,$validEXT)) {
			...
			$lstImg = mysqli_fetch_object(mysqli_query($con, "select testi_image from testimonials where md5(testi_id) = '".$this->testi_id."'"));
			if($lstImg->testi_image != 'noimage.png')
			{
				unlink("../upload/testimonial_img/".$lstImg->testi_image);
			}
            ...
            $imgSImage->save("../upload/testimonial_img/" . $image);
				  ...
				  $sql="update testimonials set
						...
						where md5(testi_id) = '".$this->testi_id."'";				
	
				 mysqli_query($con, $sql) or die(mysql_error());	
		 }
        else
        {
            $this->msg = '<div class="alert alert-danger"><i class="icon-remove"></i> Please Upload A Image With Valid Extention</div>';
        }
		...
		$this->msg='<div class="alert alert-success"><i class="icon-ok"></i> Testimonials Updated successfully.</div>';	
	}
}
...
{	
	...
	$_SESSION['msg']=$ob->msg;
	header("location:testi-edit.php?token=".$_GET['token']);
}
?>
...
<div class="main-content">
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb">
			...
			<li class="active">Testimonial Edit</li>
		</ul><!-- .breadcrumb -->
		<!-- #nav-search -->
	</div>
...
<em style="display:block;margin:5px;">Fields with <span style="color:#F00">*</span> are required.</em>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<select id="testi_type" name="testi_type" class="chosen-select">
               	<option value="buyer" <?php if($row->testi_type=='buyer'){?> selected="selected"<?php }?>>Buyer</option>
               	<option value="supplier" <?php if($row->testi_type=='supplier'){?>selected="selected"<?php }?>>Supplier</option>
            </select>
		</div>
	</div>
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<input name="testi_name" id="testi_name" class="col-xs-10 col-sm-5" type="text" value="<?php echo $row->testi_name; ?>" />
		</div>
	</div>
   	...
   	<div class="form-group">
		...
		<div class="col-sm-9">
			<textarea type="text" name="testi_details" id="testi_details" class="col-xs-10 col-sm-7"><?php echo $row->testi_details;?></textarea>
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input name="testi_business_name" id="testi_business_name" class="col-xs-10 col-sm-5" type="text" value="<?php echo $row->testi_business_name; ?>" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input name="testi_email" id="testi_email" class="col-xs-10 col-sm-5" type="text" value="<?php echo $row->testi_email; ?>" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input name="testi_mobile" id="testi_mobile" class="col-xs-10 col-sm-5" type="text" value="<?php echo $row->testi_mobile; ?>" />
		</div>
	</div>
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<select id="testi_cn_id" name="testi_cn_id" class="chosen-select">
				...
				?>
                <option value="<?php echo $cn_row->cn_id?>" <?php if($row->testi_cn_id==$cn_row->cn_id){?> selected="selected"<?php }?>><?php echo $cn_row->cn_name;?></option>
                <?php }?>
    		</select>
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<img src="../upload/testimonial_img/<?php echo $row->testi_image; ?>" />
		</div>
	</div>
    <div class="form-group">
