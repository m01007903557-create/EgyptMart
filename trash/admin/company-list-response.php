...
//include_once '../lib/function.php';////webxtor: 2021Feb06
...
$email_link="http://".$_SERVER['SERVER_NAME']."/verifyUser.php?token=".rand(1000,9999).md5($_SESSION['uid_indm']);
$to = stripslashes(user_info($_SESSION['uid_indm'],'email')); 
$subject = "عشرة الاف مشترى وبائع فى انتظارك فى مصر والمنطقة العربية ";
...
// initilize all variable
	...
	if( !empty($params['search']['value']) ) {   
		...
		$where .=" OR country.cn_name LIKE '%".$params['search']['value']."%' ";
		$where .=" OR date LIKE '%".$params['search']['value']."%' ";
		$where .=" OR mst_name LIKE '%".$params['search']['value']."%' ";//webxtor
		//$where .=" OR date LIKE '%".$params['search']['value']."%' ";
		$where .=" OR email LIKE '%".$params['search']['value']."%' )";
		$search_kw = strtolower(trim($params['search']['value']));
		if ($search_kw == 'active') {
			$where =" AND expiry_date > " . time();
		} elseif ($search_kw == 'inactive') {
			$where =" AND ( expiry_date IS NULL OR expiry_date < " . time() . ') ';
		} elseif ($search_kw == 'verified') {
			$where =" AND usr_emailVerify = '1' ";
		} elseif ($search_kw == 'verify') {
			$where =" AND usr_emailVerify = '0' ";
		}
	}
	...
	$sql = "select * from country,business_profile bf JOIN user u ON u.usr_id = bf.bnsprof_uid JOIN smembership_plan ON mp_id=usr_mp_id LEFT JOIN plan_member_id pm ON pm.b_id = bf.bnsprof_id LEFT JOIN states s ON bnsprof_state=s.state_id LEFT JOIN city c on bnsprof_city=ct_id where country=cn_id and bnsprof_uid=usr_id and status='1' ";
	
	$sql = "select *from business_profile bf 
JOIN user u ON u.usr_id = bf.bnsprof_uid 
LEFT JOIN country ON country=cn_id 
LEFT JOIN states s ON bnsprof_state=s.state_id 
LEFT JOIN city c on bnsprof_city=ct_id 
LEFT JOIN plan_member_id pm ON pm.b_id = bf.bnsprof_id
LEFT JOIN smembership_plan ON mp_id=usr_mp_id
 WHERE status='1' "; // webxtor
	
	//$sqlTot .= "select count(*) as count from country,business_profile bf JOIN user u ON u.usr_id = bf.bnsprof_uid JOIN smembership_plan ON mp_id=usr_mp_id LEFT JOIN plan_member_id pm ON pm.b_id = bf.bnsprof_id where country=cn_id and bnsprof_uid=usr_id and status='1' ";
	$sqlTot  .= "select count(*) as count from business_profile bf 
JOIN user u ON u.usr_id = bf.bnsprof_uid 
LEFT JOIN country ON country=cn_id 
LEFT JOIN plan_member_id pm ON pm.b_id = bf.bnsprof_id
LEFT JOIN smembership_plan ON mp_id=usr_mp_id
 WHERE status='1'  "; // webxtor
	...
	while( $row = mysqli_fetch_object($queryRecords) ) {			
			...
			$res[11] = (($row->expiry_date != '' && date('d F Y',$row->expiry_date) == '09 September 9999')?'Permanent':((($row->expiry_date != '')?(date("d F Y", $row->expiry_date)):'')).' '. ((date("Y-m-d", $row->expiry_date) > date("Y-m-d"))?'Active':'Inactive'));// . ', '. $row->expiry_date;
