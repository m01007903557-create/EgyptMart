...
/********************delete record*********************/
	if($_GET['action']=="del"){
		...
		//header("location:user-list.php?".rtrim($_SERVER['QUERY_STRING'],"&action=del&id=".$_GET['id']));
		}
/*************************************************/

$al->limit=$p->setlimit(10);
$al->setsql("select * from user,country,business_profile where bnsprof_uid=usr_id and country=cn_id and status='1' order by usr_id desc");
 ...
 <script type="text/javascript">
		...
		<script type="text/javascript">
			jQuery(function($) {
				...
				"ajax":{
					url :"user-list-response.php", // json datasource
