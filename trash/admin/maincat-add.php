...
check_user_login();
	...
	function __construct($pc_name,$ci_image)
	{	
		...
		if($this->ci_image != '')
	{
		...
		$ext = end(explode('.',$this->ci_image)); 
        $validEXT = array('jpg','png','jpeg', 'gif', 'pdf');

        if(in_array($ext,$validEXT))
		{
            $tempFile = $_FILES['ci_image']['tmp_name'];
            $ci_banner = $_FILES['ci_banner']['tmp_name'];
           //print_r($_FILES['ci_banner']);die();
            ...
            $image = 'CATIMG-' . rand(0,9999) .$this->ci_image;
            $banner = 'CATIMG-' . rand(0,9999) .$_FILES['ci_banner']['name'];
            //die();
            

             $imgSImage->resize(70,70);
             $cibanner->resize(1000,450);
...
if(isset($_POST['btnAdd']))
{
	...
	$adn=new addproduct(addslashes(trim($_POST['pc_name'])), $_FILES['ci_image']['name']);
...
{	
	...
	else if(ci_image.value=='' || ci_image.value == null)
	{
...
<form class="form-horizontal" action="" id="cmp_edit" name="cmp_edit" method="post" enctype="multipart/form-data" onsubmit="return myvalid();">
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<input name="pc_name" id="pc_name" class="col-xs-10 col-sm-5" type="text" value="<?php echo $pc_name; ?>" />
		</div>
	</div>
    
    <div class="form-group">
		<label class="col-sm-3 control-label no-padding-right" for="form-field-2">Category Image </label>
		<div class="col-sm-9">
			<div class="ace-file-input" style="width:400px;"><input name="ci_image" id="id-input-file-2" type="file"></div>
	        (Preferred width & height is 70px & 70px respectively)
		</div>
	</div>


<div class="form-group">
		<label class="col-sm-3 control-label no-padding-right" for="form-field-2">Add Category page Banner</label>
		<div class="col-sm-9">
