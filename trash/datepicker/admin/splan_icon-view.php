...
class listCat{
	...
	function deleterecord($adid){
		...
		mysql_query("update smembership_icon_plan set mp_status = '0' where mp_id='".$adid."'");
	}
	function deletelink($id){
		if($_SERVER['QUERY_STRING']==""){
			...
			$dellink="splan_icon-view.php?".$_SERVER['QUERY_STRING']."&action=del&fid=".$id;
		}
		return $dellink;
	}
}
...
/********************delete record*********************/
	if($_GET['action']=="del"){
		...
		//header("location:splan_icon-view.php?".rtrim($_SERVER['QUERY_STRING'],"&action=del&id=".$_GET['id']));
		}
/***********************************************/

$al->limit=$p->setlimit(20);
$al->setsql("select * from smembership_icon_plan where mp_status = '1'");
$totalitems=$al->totalrecord();
$limit=$al->limit;
$al->start=$p->setstart($page,$limit,$totalitems);
$adjacents=1;
$targetpage = "splan_icon-view.php";
...
{ 
	...
	{		
		mysql_query("update smembership_icon_plan set mp_status = 0 where mp_id='".$cb."'");		
	}
	header("Location:splan_icon-view.php");
}
?>
...
<button class="btn btn-xs btn-danger" name="btnDelete" type="submit" onClick="return confirm('Are you sure to delete the record?')" ><i class="icon-trash bigger-120"></i>Delete</button>
 <button type="button" class="btn btn-xs btn-success" onClick="window.location='splan_icon-add.php' "><i class="icon-pencil align-top bigger-120"></i>Add Plan</button>
 </div>
...
$count=mysql_num_rows($recObj);
	if($count >0)
	{
        ...
        <tr <?php if($j % 2 == 1) { ?> class="row-clr" <?php } ?> >
            ...
            <a href="splan_icon-edit.php?fid=<?php echo $row->mp_id ?>" title="Edit"><img alt="edit" src="images/edit.jpg" border="0"></a>
            <a href="<?php echo $al->deletelink($row->mp_id)?>" title="Delete" onclick="return confirm('Are you sure to delete the plan?')"><img alt="Delete" src="images/delete.jpg" border="0"></a>
            </td>
        </tr>
        <?php $j++; } }?>
</tbody>
