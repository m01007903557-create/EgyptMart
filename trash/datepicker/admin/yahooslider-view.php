...
class sliderviewlist{
	...
	function deleterecord($adid){
		global $con;
	    $chquesql="select * from yahoo_slider_arabyos where adv_id ='".$adid."' ";
		$chqueres=mysqli_query($con, $chquesql);
		$chquerow=mysqli_fetch_array( $chqueres);
		$path="../upload/yahoo_slider/".$chquerow['adv_img'] ;
		unlink($path);
		$sql_del="delete from yahoo_slider_arabyos where adv_id='".$adid."'";
...
$al->setsql("select * from yahoo_slider_arabyos order by adv_updated_date desc");
