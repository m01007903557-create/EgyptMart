...
{	
	...
	function __construct($cn_name, $cn_currency, $cn_ph, $cn_flag)
	{		
...
{
	$adn=new addCountry(addslashes(trim($_POST['cn_name'])), addslashes(trim($_POST['cn_currency'])), addslashes(trim($_POST['cn_ph'])), trim($_FILES['cn_flag']['name']));
	$adn->add();		
...
function updCountry(id)
{
	...
	{
		alert('OK');
		$.post("checkOldCountry.php", {cn_id:cn_id,cn_name:cn_name.value,cn_currency:cn_currency.value,cn_ph:cn_ph.value}, function(data){
			if(data==1)
			{
				alert("Records already exist. Please try with different data.");	
			}
			...
			{
				$.post("updCountry.php", {cn_id:cn_id,cn_name:cn_name.value,cn_currency:cn_currency.value,cn_ph:cn_ph.value}, function(data){	
...
<table id="sample-table-2" class="table table-striped table-bordered table-hover">
	<tr>
		<TD style="border:0px;" align="center"><!--<span><button class="btn btn-xs btn-success" onclick="ShowaddCountry()" type="button"><i class="icon-plus-sign"></i><b>ADD COUNTRY/CURRENCY</b></button></span>-->
           <a href="#modal-form" role="button" data-toggle="modal" class="btn btn-xs btn-success"><i class="icon-plus-sign"></i><b>ADD COUNTRY/CURRENCY</b></a>
        </TD>
	</tr>
</table>
