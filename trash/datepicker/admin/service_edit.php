...
include "../common.php";
	...
	function __construct($ser_id){
		$this->ser_id=$ser_id;
	}
	
	function detailsObj(){
		global $con;
		$sql="select * from services where md5(ser_id)='".$this->ser_id."'";
	...
	if($this->ser_image != '')
	{
        ...
        if (in_array($ext,$validEXT)) {
			...
			$lstImg = mysqli_fetch_object(mysqli_query($con, "select ser_image from services where md5(ser_id) = '".$this->ser_image."'"));
			unlink("../image/".$lstImg->ser_image);
...
if(isset($_POST['btnAdd']))
{
	...
	$ob->ser_content=trim(addslashes($_POST['ser_content']));	
	$ob->ser_heading=trim(addslashes($_POST['ser_heading']));
	$ob->ser_image=$_FILES['ser_image']['name'];
	...
	header("location:service_edit.php?token=".$_GET['token']);
}
?>
...
<td  style="width:678px">
        ...
        <div class="formItem leftLabel" style="padding-bottom:10px; padding-top:10px;">
			...
			<div class="formInputBox" style="width:387px;height:auto;">
                      <textarea name="ser_heading" id="ser_heading"class="reg_txtfld" style="height: 300px" ><?php echo $row->ser_heading;?></textarea> 
			</div>
		</div>
		 ...
		 <div class="formItem leftLabel" style="padding-bottom:10px; padding-top:10px;">
			...
			<div class="formInputBox" style="width:387px;height:auto;">
                       <textarea name="ser_content" id="ser_content" class="reg_txtfld"  style="height: 300px" ><?php echo $row->ser_content;?></textarea> 
			</div>
		</div>
