...
class editProduct{
	...
	function __construct($spc_id){
		$this->spc_id=$spc_id;
	}
	function detailsObj(){
		global $con;
		$sql="select * from servicepage_content where spc_id='".$this->spc_id."'";
	...
	{		
		global $con;
			$sql="update servicepage_content
				set	
					spc_heading ='".$this->spc_heading."',
					spc_updated_date=now()
				where spc_id='".$this->spc_id."'";							
...
if(isset($_POST['btnUpdate'])){
		...
		$ob->spc_heading=addslashes(trim($_POST['spc_heading']));
	...
	header("location:service_page_edit.php?sid=".$ob->spc_id);
}
?>
...
<div  class="row buttons"><input type="button" name="btnUpdate" id="btnUpdate"  onclick=location.href='service_page_view.php' value="Back" class="x2-button" style="margin-right:10px;margin-top:5px;"> </div>		
...
<td  style="width:678px">
			...
			<div class="formInputBox" style="width:387px;height:auto;">
                      <textarea name="spc_heading" id="spc_heading"class="reg_txtfld" style="height: 300px" ><?php echo $row->spc_heading;?></textarea> 
			</div>
		</div>
