...
class addSubCategory{
	...
	function __construct($mcat_id,$cat_id,$scat_name,$scat_sort_name)
	{
...
if(isset($_SESSION['mcat_id'])){ $mcat_id=$_SESSION['mcat_id']; unset($_SESSION['mcat_id']); } else { $mcat_id=""; }
if(isset($_SESSION['cat_id'])){ $cat_id=$_SESSION['cat_id']; unset($_SESSION['cat_id']); } else { $cat_id=""; }
if(isset($_SESSION['scat_name'])){ $scat_name=$_SESSION['scat_name']; unset($_SESSION['scat_name']); } else { $scat_name=""; }
if(isset($_SESSION['scat_sort_name'])){ $scat_sort_name=$_SESSION['scat_sort_name']; unset($_SESSION['scat_sort_name']); } else { $scat_sort_name=""; }

if(isset($_POST['btnAdd']))
{
	
	$adn=new addSubCategory(addslashes(trim($_POST['mcat_id'])), addslashes(trim($_POST['cat_id'])), addslashes(trim($_POST['scat_name'])), addslashes(trim($_POST['scat_sort_name'])));

	$_SESSION['mcat_id']=addslashes(trim($_POST['mcat_id']));
	$_SESSION['cat_id']=addslashes(trim($_POST['cat_id']));
    $_SESSION['scat_name']=addslashes(trim($_POST['scat_name']));
    $_SESSION['scat_sort_name']=addslashes(trim($_POST['scat_sort_name']));
...
<form class="form-horizontal" action="" id="cmp_edit" name="cmp_edit" method="post" enctype="multipart/form-data" onsubmit="return myvalid();">
    ...
    <div class="form-group">
		...
		<div class="col-sm-9">
			<select name="mcat_id" id="mcat_id" class="chosen-select" onchange="showCategory();">
                ...
                <?php 
                $mcatsql = mysqli_query($con, "select * from product_category where pc_parent_id = '0' and pc_status = '1'");
                while($mcatrow = mysqli_fetch_object($mcatsql)){
                ?>
               <option value="<?php echo $mcatrow->pc_id; ?>"><?php echo $mcatrow->pc_name;?></option>
               <?php }?>
           </select>
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input name="scat_name" id="scat_name" class="col-xs-10 col-sm-5" type="text" value="<?php echo $scat_name; ?>" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input name="scat_sort_name" id="scat_sort_name" class="col-xs-10 col-sm-5" type="text" value="<?php echo $scat_sort_name; ?>" />
		</div>
	</div>
