...
function scatAddDel(id)
{
	if($('#scat_'+id).attr('checked')) {
		$.post("ajax-file/addTempBuyleadAlertCat.php",{id:id},    function(data){	showList()	});
	} else {
		$.post("ajax-file/delTempBuyleadAlertCat.php",{id:id},    function(data){	showList()	});
	}
}
function showList()
{
	$.post("ajax-file/showTempBuyleadAlertCat.php",{},    function(data){	$("#div1").html(data);	});
}
function remove(id)
{
	$.post("ajax-file/delTempBuyleadAlertCat.php",{id:id},    function(data){	showList()	});
...
<div style="background-color:#FFFFFF; height:670px" id="dvh2">
	<table border="0" cellpadding="0" cellspacing="0" width="100%" >
    	<tbody>
        	<tr>
            <td bgcolor="#FAF4FF"><div class="myta">Manage Your Buy Lead Product Preference</div></td>
            </tr>
		</tbody>
	</table>
...
function produtcustomcategory()
{
   ...
   $.post("ajax-file/produtcustomcategory.php",{'keywordsFilter':keywordsFilter,'type':'addTempBuyleadAlertCat'},function(data){ $('#div1').append(data);})
}
 ...
 <b style="font-size:13px;"><font color="#E95801">Enter product keywords to find a category</font></b></div>
  ...
  <tr> <td>
  <input role="textbox" class="txt ui-placeholder-input ui-autocomplete-input" name="keywordsFilter" id="keywordsFilter" type="text" maxlength="60" size="33" ></td>
 ...
 <input name="confirm1" id="addLeadCatBtn" value="Submit Categories" onclick="addAlertCategory();" type="button">
