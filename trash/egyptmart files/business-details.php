...
<div class="inner_wrapper">
	...
	<h1 style="font size: 18px; weight;bold; title="Business Profile ">إملأ بيانات بروفايل الشركة  </h1>
	</div>
	<?php include 'includes/business-panel.php';?>
		
		<div id="re_link" class="utab" style="height:39px;"><span style="font-size: 20px;" class="f1"title="Complete your Business Details to attract genuine buyers.">أكمل معلومات الشركة حتى تجتذب مشتريين حقيقيين </span></div>
...
function DelTempImage(imid)
{
   ...
   if(cnf==true)
   {
	...
	jQuery(document).ready(function(){
			//var url ='https://www.egyptmart.online/server/php/';
...
<script type="text/javascript">list_photo()</script>
 ...
 <div class="upload_div"title=" حمل لوجو - صورة الشعار التجارى - للشركة ">
	...
	<span class="file_input"title="Add image"> حمل لوجو الشركة </span>
		...
		<tr>
			<td class="label"title=" Alternate Website ">أكتب الموقع الألكترونى الأصلى للشركة </td>
...
<input class="a_f rf" maxlength="80" name="website_alt" id="website_alt"  value="<?php echo user_info($uid,'bnsprof_website_alt')?>" tabindex="3">
		...
		<tr>
			<td class="label pt7" valign="top"title="Business Type">إختار أنوع نشاط الشركة التى تعمل بها</td>
			<td>
			<table style="margin:0px;" cellpadding="0" cellspacing="0" width="70%">
			<tbody>
            <?php
			$c=1;
            $bstypsql=mysql_query("select * from business_type where bsntyp_status='1'");
...
>إختار العدد التقريبى للموظفين</td>
			<td>
            ...
            <?php
			$c=1;
            $noempsql=mysql_query("select * from employee_range where emprange_status='1'");
 ...
 <option value="<?php echo $noemprow->emprange_id;?>" <?php if(user_info($uid,'bnsprof_comemp') == $noemprow->emprange_id){ ?> selected="selected" <?php } ?> >
		...
		<tr>
			<td class="label"title=" Revenue Sales Turnover ">إختار العائد السنوى التقريبى </td>
			<td>
            ...
            <?php
			$c=1;
            $turnoversql=mysql_query("select * from revenue_turnover where revturnover_status='1'");
		...
		<tr>
			<td class="label"title="Ownership Type"> إختار نوع ملكيـة الشركة  </td>
			<td>
            ...
            <?php
			$c=1;
            $owntypesql=mysql_query("select * from ownership_type where owntyp_status='1'");
		...
		<tr>
			<td>&nbsp;</td>
			<td align="left">
				<table><tbody><tr><td width="118px;"title="إحفظ - التغييرات - للنشر - مع بيانات الشركة ">
