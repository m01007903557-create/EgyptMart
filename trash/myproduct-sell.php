...
if(isset($_POST['btnUpdate']))
{
	...
	$reschk=mysqli_query($con, "SELECT * FROM product_sell WHERE pdby_uid='$usid' ORDER BY pdby_id DESC");
	if(mysqli_num_rows($reschk)>0){
		mysqli_query($con, "DELETE FROM product_sell WHERE pdby_uid='".$usid."'");
		 foreach($pdby_title as $val){
			if($val!=""){
				$key=$val;
				//$sql_key="select * from buy_requirement join product_category on product_category.pc_id=buy_requirement.br_pc_id where br_pd_name   like '%".$key."%' and pc_status='1'";
			...
			$sql="insert into product_sell set pdby_uid='".$usid."', pdby_title='".$val."' ";
			...
			if($val!="")
			{
				...
				if(mysql_num_rows($r) == 0){		
					$SQL_BUY_ALERT="insert  into buylead_alert_category SET 
													  ...
													  bac_pc_id=".$key_cat_id.",
													  bac_updated_date=now()";
					$r=mysql_query($SQL_BUY_ALERT) or die('Error in query while updating');
				}
...
$aid="";
$pname="";
$res=mysqli_query($con, "select * from product_sell where pdby_uid='".$uid."' order by pdby_id desc");
...
<div class="inner_wrapper">
	...
	<h1> بروفايل الشركة - إملأ بيانات </h1>
	</div>
	<?php include 'includes/business-panel.php';?>
	<div id="re_link" class="utab"><span style="font-size: 14px;" class="f1"title="أكتب الأصناف المعتاده التى تبيعها شركتك لتتلقى إشعارات طلبات شراء لها فى بريدك  ">Add Products / Services you usually Buy & Get Sell Offer Alerts to your mailbox. </span></div>
	...
	<div class="frm_a clb" style="background-color:#FAF4FF">
		<table align="left" border="0" cellpadding="4" cellspacing="0" width="100%">			
			...
			<div class="f1 m5" id="hidetbl"><a class="f_l" onclick="add_more(<?php echo $uid;?>); " id="ad_more1" style="cursor:pointer;"title=" + Add More ">إضافة المزيد  + </a></div>
			</td>
			</tr>
			<tr>
			<td>&nbsp;</td>
			<td align="left">
				...
				<input name="btnUpdate" title="Update Details"value="إحفظ التغييرات" class="saps mt5" tabindex="31" type="submit"></td><td> <span id="pf_save" style="display:none;margin-left:15px;margin-top:6px;"><img src="https://my.imimg.com/gifs-new/loading.gif" alt="" border="0" width="16" height="11"></span> </td></tr></tbody></table>
			</td>
			</tr>
			</tbody></table>
			<div class="clb">&nbsp;</div>
