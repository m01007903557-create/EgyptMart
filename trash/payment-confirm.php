...
$credits = $row->mp_credits;
$plan_id = $row->mp_id;

/*$sql_pg=mysqli_query($con, "select * from payment_gateway where md5(id)='".$pg."'");
$row_pg=mysqli_fetch_object($sql_pg);*/
...
<div class="m_pkgdtl"> 	
	 <h2>Order Summary</h2>
		<div class="m_pkgdtl-inner">
			...
			<tbody>
            <tr>
            <td class="m_pkgdetails"><?php echo $plan; ?> - <?php echo $credits; ?> Credits</td>
            <td class="m_pkgdetails" width="10">:</td>
            <td class="m_pkgdetails m_tdpl"><span class="WebRupee"><?php echo getCurrencySymbol(); ?></span> <?php echo $amount; ?></td>
            </tr>
            <?php if(getServiceTaxRate()!='0' && getServiceTaxRate()!=''){	
			$tax_rate=getServiceTaxRate();
			?>
            <tr>
            <td class="m_stax">Service Tax (<?php echo $tax_rate; ?>%)</td>
            <td class="m_stax" width="10">:</td>
            <td class="m_stax m_tdpl"><span class="WebRupee"><?php echo getCurrencySymbol(); ?></span> <?php echo number_format(($amount*$tax_rate)/100,2); ?></td>
            </tr>
            
			<tr>
            <td class="m_pricefnl">Total Amount Payable</td>
            <td class="m_pricefnl" width="10">:</td>
            <td class="m_pricefnl m_tdpl"><span class="WebRupee"><?php echo getCurrencySymbol(); ?>&nbsp;</span> <?php $tot=$amount+(($amount*$tax_rate)/100); echo number_format($tot,2); ?></td>
            </tr>
			<?php }
			else
			{
            ...
            <td class="m_pricefnl">Total Amount Payable</td>
            <td class="m_pricefnl" width="10">:</td>
            <td class="m_pricefnl m_tdpl"><span class="WebRupee"><?php echo getCurrencySymbol(); ?>&nbsp;</span> <?php $tot=$amount; echo number_format($tot,2); ?></td>
            </tr>
            <?php } ?>
			</tbody></table>
	...
	<input type="hidden" id="mp" name="mp" value="<?php echo $plan_id; ?>" />

<div class="m_ca2ndstep cpa1">
 	<div class="m_crtop">
		        
        <div class="m_fst2s cpa1 m_crtophd">
		  <p class=" m_2s cpa1"></p>Confirm Payment
		</div>
