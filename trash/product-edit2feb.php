...
class Editcat{
	...
	function valid()
	{
		...
		if($this->mcat_id =='')
		{
			$this->msg='<font color="#CC0000">Please choose the Product Main Category here.</font>';
			$valid=false;
		}
		else if($this->cat_id == "")
		{
			$this->msg='<font color="#CC0000">Please choose the Product Category here.</font>';
			$valid=false;
		}
		else if($this->pd_subcat_id == "")
		{
			$this->msg='<font color="#CC0000">Please choose the Product subcategory here</font>';
			$valid=false;
		}
		else if($this->pd_title == "")
		{
			$this->msg='<font color="#CC0000">Please enter the Product Name.</font>';
			$valid=false;
		}
		else if($this->pd_title != "")
		{		
		...
		{
			$this->msg='<font color="#CC0000">Please enter FOB Price.</font>';
			$valid=false;
		}
	...
	function update()
	{
		...
		$this->msg= '<font color="green">Product updated successfully!</font>';	
  }	
}
...
$.get("ajax-file/showProductImagelogo.php", {id:id},	function(data){
		$("#img_disp_logo").html('');												 
		$("#img_disp_logo").html('<img src="'+data+'" alt="" style="width:125px;height:125px;margin-top:1px;margin-left:1px;"/>');
	});
}
function usePhoto()
{          
...
console.log(data);
		 jQuery('#cboxOverlay').remove();
    jQuery('#colorbox').remove();
		$("#img_disp_logo").html('<img src="images/loader.gif" alt="Uploading...." style="width:125px;height:125px;margin-top:1px;margin-left:1px;"/>');
...
//	$.post("ajax-file/chkSubcat.php",{cid:cid},  function(data){	  $('#sbcat').html(data);	}); 
	...
	function additionaldet_update(pid)
	{
		...
		$.get("ajax-file/additionaldet-edit.php",{pid:pid,pd_hot:pd_hot,pd_payment:pd_payment,pd_pod:pd_pod,pd_pn_capct:pd_pn_capct,pd_dlv_time:pd_dlv_time,pd_pck_dets:pd_pck_dets},
		function(data){
		var e=data.split("||");
		if(e[1]==0)
		{
...
<?php include 'includes/header_menu.php';?> 
        ...
        <div class="w56b f1 p2b p14 blr"><h1>Manage Products</h1>
	...
	<!--you are here:start-->
		...
		<div>&nbsp;</div>	
		<div class="tab_2">
			<span id="t1" class="tab-sel f1 fw"><a onclick="showmedit();">Edit Product</a></span>
			<span id="t3" class="tab_p1 f1"><a onclick="showadditional();">Additional Details</a></span>
	...
	<a name="ba"></a>
	<div class="p-irt" id="bapre" style="display:none;">
	<p class="bulb">Well written product information boosts your product visibility in search results</p>
	<ul>
  ...
  jQuery('#cboxOverlay').remove();
    jQuery('#colorbox').remove();
            ...
            <a class="ajax" href="show_productedit_img.php?pid=<?php echo $pdrow->pd_id;?>" style="text-decoration:none;">Edit Image</a>
                </div>
        <p id="img_form" style="margin-top: 2px;"></p>
		<div id="img_disp_logo" class="cssie1 mover">
        ...
        <img src="upload/myproduct/<?php if($pdrow->pd_imagelogo==''){ echo "add-image.gif"; }else{ echo $imgonelogo[0];} ?>" title="Add Brand Logo" style="width:125px;height:125px;margin-top:1px;margin-left:1px;" title="ADD Logo,Brand,Discount,Sign ">
		</div>
	...
	<link rel="stylesheet" type="text/css" href="uploadifive/uploadifive.css">
            ...
            <a class="ajaxa" href="show_product_edit_logo.php?pid=<?php echo $pdrow->pd_id;?>" style="text-decoration:none;" title="ADD Logo,Brand,Discount,Sign ">Edit Brand Logo</a>
		...
		<div class="fside f1" style="width:82%;">
            ...
            <div class="fs1 f1" style="width:100%">
    <?php
	...
	?>
            ...
            <p><span style="line-height: 12px;">*</span> Main Category</p>
            ...
            <span>Please choose the Product Main Category here.</span>
            </div>
            <div class="fs1 f1"></div>
            <div class="fs1 f1">
            <p><span style="line-height: 12px;">*</span> Category</p>
            <?php
    ...
    <option value="<?php echo $catrwk['pc_id']?>" <?php if($pdrow->pc_parent_id==$catrwk['pc_id']) { ?> selected="selected"<?php } ?>><?php echo $catrwk['pc_name']?></option>
            ...
            <span>Please choose the Product Category here.</span>
            </div>
...
<option value="<?php echo $subcatrwk['pc_id']?>" <?php if($pdrow->pd_subcat_id==$subcatrwk['pc_id']) { ?> selected="selected"<?php } ?>><?php echo $subcatrwk['pc_name']?></option>
            ...
            <div class="fs1 f1">
			<p><span>*</span> Minimum Order Quantity</p>
            ...
            <span>Please choose the Measurement Unit here.</span>
			</div>
            
			<div class="fs1 f1">
			<p><span>*</span> FOB Price</p>

			From : <input name="pd_fob_price" id="pd_fob_price" maxlength="60" class="a_f pf1" type="text" value="<?php echo $pdrow->pd_fob_price;?>">
            To : <input name="pd_fob_price2" id="pd_fob_price2" maxlength="60" class="a_f pf1" type="text" value="<?php echo $pdrow->pd_fob_price2;?>">
            
			
			<span>Please enter the FOB Price here.</span>
			</div>
	
			<div class="fs2 f1" style="margin-top:4px;">
			<p>&nbsp;</p>
            ...
            <span>Please choose Currency</span>
			</div>
            
			<div style="clear:both;line-height: 21px;*line-height: 10px;">&nbsp;</div>
			<div class="fs1 f1" style="margin-top:5px;">
			<p><span style="line-height: 12px;">*</span> Location Prefrences</p>
            
            <div style="vertical-align:middle">
            <input type="radio" id="pd_preferred_buyer_location_1" name="pd_preferred_buyer_location" value="abroad" <?php if($pdrow->pd_preferred_buyer_location=='abroad'){ ?> checked="checked"<?php } ?> /><label style="top:0px;">Abroad Only</label>
            </div>
            <div>
		         <input type="radio" id="pd_preferred_buyer_location_2" name="pd_preferred_buyer_location" value="any" <?php if($pdrow->pd_preferred_buyer_location=='any'){ ?> checked="checked"<?php } ?>/><label style="top:0px;">Abroad + Domestic</label>
			</div>
            <div>
	             <input type="radio" id="pd_preferred_buyer_location_3" name="pd_preferred_buyer_location" value="domestic" <?php if($pdrow->pd_preferred_buyer_location=='domestic'){ ?> checked="checked"<?php } ?>/><label style="top:0px;">Domestic Only</label>
    		</div>
            <div>
                 <input type="radio" id="pd_preferred_buyer_location_4" name="pd_preferred_buyer_location" value="my_city" <?php if($pdrow->pd_preferred_buyer_location=='my_city'){ ?> checked="checked"<?php } ?>/><label style="top:0px;">My City Only</label>
            </div>
			...
			<div class="edtbut">
				<span id="save_basic"><input name="btnUpdate" class="saps mt12" value="Save" type="submit"></span> 
	      ...
	      <div class="pad f1 mr">
	        <ul>
	          ...
	          <li class="f1 pct tc1">
	            ...
	            Hot
	            <input name="pd_hot" value="0" id="pd_hot" <?php if($pdrow->pd_hot=='0'){?> checked="checked"<?php } ?> onclick="return makelive(0)" type="radio" />
	            Default</li>
	          ...
	          <li class="f1 pct">
	            <input class="a_f adt" name="pd_pod" id="pd_pod" maxlength="100" value="<?php echo $pdrow->pd_pod; ?>" type="text" />
	            </li>
	          <li class="f1 tc adtp">Production Capacity</li>
	          <li class="f1 pct">
	            <input name="pd_pn_capct" id="pd_pn_capct" maxlength="100" value="<?php echo $pdrow->pd_pn_capct; ?>" class="a_f adt" type="text" />
	            </li>
	          <li class="f1 tc adtp">Delivery Time</li>
	          <li class="f1 pct">
	            <input class="a_f adt" name="pd_dlv_time" id="pd_dlv_time" maxlength="100" value="<?php echo $pdrow->pd_dlv_time; ?>" type="text" />
	            </li>
	          <li class="f1 tc adtp">Packing Details </li>
	          <li class="f1 pct">
	            <textarea rows="5" class="a_f" name="pd_pck_dets" id="pd_pck_dets" onkeyup="packdetcount();">
	          ...
	          <li class="f1 tc adtp">Attach PDF Brochure</li>
...
<iframe src="upload-prd-doc.php?pid=<?php echo $pdrow->pd_id;?>" border="0" framespacing="0" allowtransparency="true" scrolling="no" width="269" frameborder="0" height="30"> 
	          ...
	          <li>
	            <input name="updateaddi" class="saps awt mt12 m5" value="Save Details" type="button" onclick="additionaldet_update(<?php echo $pdrow->pd_id;?>)"/>
	...
	<li class="f1 tc">FOB Price</li><li class="f1 pct" id="view_fob_price">12000 INR</li>
	...
	<div id="addef" class="p-irt" style="">
	  ...
	  <div style="display:none;" id="adfrmjj">
	      ...
	      <div class="pad f1 mr">
	        <ul>
	          ...
	          <li class="f1 pct pbi tc1">
	            ...
	            Other </li>
	          <input name="in_pc_item_payment_terms" id="in_pc_item_payment_terms" value="L/C (Letter of Credit),D/A,T/T (Bank Transfer),Other" type="hidden" />
	          <li class="f1 tc adtp">FOB Price</li>
	          <li class="f1 pct">
		...
		<option value="Piece/Pieces">Piece/Pieces</option-->
	          ...
	          <li class="f1 tc adtp">Attach PDF Brochure</li>
	          <li class="f1 pct1 mt5">
	            <iframe src="images/upload-prd-doc.htm" border="0" framespacing="0" allowtransparency="true" scrolling="no" width="269" frameborder="0" height="30"> </iframe>
	            <span class="f2" id="indecator_gif0" style="left:37px;position:relative;top:-32px;"></span> (.pdf type attachment only) </li>
	          <input name="item_doc" value="" id="myfile_doc_form0" type="hidden" />
	          <li class="f1 tc adtp"> </li>
	          <li>
	            <input name="updateaddi" class="saps awt mt12 m5" value="Save Details" type="submit" />
	            <span style="display:none;" id="adsaving"> <img alt="" src="images/loading.gif" border="0" width="16" height="11" /></span>
	            <input name="cancleaddi" class="saps mt12 ml8" value="Cancel" id="adcls" type="button" />
	            </li>
	          </ul>
	        </div>
	      <div class="pia f1 mt5">
	        ...
	        <p id="old_img_form_1"></p>
	        <div id="add_large_image" class="db cssie3 mover">
	          <iframe framespacing="0" marginwidth="0" marginheight="0" style="margin-top:2px;" src="images/upload-image-prd-small_002.htm" class="mt5" scrolling="NO" width="123" frameborder="0" height="123"></iframe>
	          </div>
	        <div id="remove_large_image" class="dn mt5"> <a href="javascript:remove_large_image();"><img src="images/remove.gif" align="absmiddle" width="44" height="10" /></a> </div>
	        </div>
	...
	<!--add additional details container:ends-->
		...
		<div class="back-n fw"> <a href="product-list.php">Go Back to Manage Products</a>
		<br><br>
		</div>
		<div class="c3"></div>
		</div>
