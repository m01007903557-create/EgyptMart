<?php
ob_start();//active only by webxtor 23 June 2019
session_start();
include "common.php";
...
if (count($pc_id_arr) == 0) {
   ...
   $c=1; while ($row_dd_cmnuAgainMaster = mysql_fetch_object($res_dd_cmnuAgainMaster)) {
        ...
        //$html .="<img  style=\"height:30px; margin-right: 10px; width:30px;\" src=\"http://egyptmart.online/upload/category/". $row_dd_cmnuAgainMaster->pc_image."\">";
...
<span>
			<?php
        	$sql_dd_cmnuAgainParent2 = "select	 pc_id	,	pc_name 	from product_category_arabyos where pc_parent_id = '" . $cat_row->pc_id . "'  and pc_id in ('$SubCategoryArray') and pc_status=1 order by pc_name asc ";
        	...
        	while ($row_dd_cmnuAgainParent2 = mysql_fetch_object($res_dd_cmnuAgainParent2)) {
        		...
        		<?php
        	if($hide_i == 4){ 
				echo '</span><span style="display:none" id="id'.$cat_row->pc_id.'">';
...
<?php if($rowcount2 > 3): ?>
<span  onclick="myFunction('<?php echo $cat_row->pc_id?>',this)" style="padding:3px; cursor:pointer;font-size:12px"><i class="fa fa-plus"></i>&nbsp;View More..</span>
<?php endif; ?>
</li>
			<?php
			//$html .= ob_get_contents();
...
/*
$sql_mcat="select pc_id,pc_name,pc_image from product_category_arabyos where pc_parent_id = '0' and pc_status = '1' ".$sql_order;
...
$cat_res = mysqli_query($con, "select pc_id,pc_name,pc_sort_name from product_category_arabyos where pc_parent_id = '".$row_mcat->pc_id."' and pc_status = '1' ".$sql_order);
...
<span>
<?php 		
	$scat_res = mysqli_query($con, "select pc_id,pc_sort_name from product_category_arabyos where pc_parent_id = '".$cat_row->pc_id."' and pc_status = '1' order by pc_sort_name asc limit 0,3");
	$scat_i = 1;
...
function myFunction(id,obj) {
  ...
  if (dots.style.display === "none") {
	...
	obj.innerHTML = '<i class="fa fa-minus"></i>&nbsp;View Less..';
  } else {
    dots.style.display = "none";
	obj.innerHTML = '<i class="fa fa-plus"></i>&nbsp;View More..';
  }
}
</script>
