...
{
	$.post("ajax-file/purchased-auctions.php",{page:page},    function(data){ $('#res').html(data); });
}
...
.tabTextColor{color:#fff;}
	...
	<form style="margin:0px;" action="" name="form1">
		...
		Credits</font><span class="mf12">[ 
			...
			<?php } ?>
			]</span><div class="mta mpb8 mpt5 mf11"><a href="transaction_history.php">View Your Transaction History</a></div></div><div class="wd mf18 mc5 mta2 mpb10">Purchased Auctions</div>
		...
		<div class="ap1">
        <?php
		$sql_pur="SELECT count(*) AS cnt from purchased_auction,auction where pauc_auc_id=auc_id and pauc_usr_id='".$_SESSION['uid_indm']."' and pauc_status='1'";
		$res_pur=mysqli_query($con, $sql_pur);
		$row_pur=mysqli_fetch_object($res_pur);
		
		?>
        	<a class="f1 ap2 tabTextColor" onclick="showPurAuctions(1);" style="cursor:pointer;" id="opnReq_tab">Purchased Auctions  (<?php echo $row_pur->cnt; ?>)</a>
