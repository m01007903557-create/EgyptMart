...
{	
	...
	function valid()
	{
		...
		else if($this->telephone_areacode1 == "" && $this->telephone1!="")
		{
			$this->msg= '<font color="#CC0000">Please enter area code.</font>';
			$valid=false;
		}
		else if($this->telephone1 == "" && $this->telephone_areacode1!="")
		{
			$this->msg= '<font color="#CC0000">Please enter Telephone number.</font>'; 
			$valid=false;
		}
		else if($this->telephone_areacode2 == "" && $this->telephone2!="")
		{
			$this->msg= '<font color="#CC0000">Please enter area code.</font>';
			$valid=false;
		}
		else if($this->telephone2 == "" && $this->telephone_areacode2!="")
		{
			$this->msg= '<font color="#CC0000">Please enter Telephone number.</font>';
			$valid=false;
		}
		else if($this->telephone_areacode3 == "" && $this->telephone3!="")
		{
			$this->msg= '<font color="#CC0000">Please enter area code.</font>';
			$valid=false;
		}
		else if($this->telephone3 == "" && $this->telephone_areacode3!="")
		{
			$this->msg= '<font color="#CC0000">Please enter Telephone number.</font>';
			$valid=false;
		}
		else if($this->telephone_areacode4 == "" && $this->telephone4!="")
		{
			$this->msg= '<font color="#CC0000">Please enter area code.</font>';
			$valid=false;
		}
		else if($this->telephone4 == "" && $this->telephone_areacode4!="")
		{
			$this->msg= '<font color="#CC0000">Please enter Telephone number.</font>';
			$valid=false;
		}
		else if($this->fax_areacode1 == "" && $this->fax1!="")
		{
			$this->msg= '<font color="#CC0000">Please enter area code.</font>';
			$valid=false;
		}
		else if($this->fax1 == "" && $this->fax_areacode1!="")
		{
			$this->msg= '<font color="#CC0000">Please enter fax number.</font>';
			$valid=false;
		}
		else if($this->fax_areacode2 == "" && $this->fax2!="")
		{
			$this->msg= '<font color="#CC0000">Please enter area code.</font>';
			$valid=false;
		}
		else if($this->fax2 == "" && $this->fax_areacode2!="")
		{
			$this->msg= '<font color="#CC0000">Please enter fax number.</font>';
			$valid=false;
		}
		else if($this->email_alt1 != '' && !(validate::is_email($this->email_alt1)))
		{
			$this->msg='<font color=#CC0000">Please Enter Valid email</font>';
			$valid=false;
		}
		else if($this->email_alt2 != '' && !(validate::is_email($this->email_alt2)))
		{
			$this->msg='<font color=#CC0000">Please Enter Valid email</font>';
			$valid=false;
		}
		else if($this->email_alt3 != '' && !(validate::is_email($this->email_alt3)))
		{
			$this->msg='<font color=#CC0000">Please Enter Valid email</font>';
			$valid=false;
		}
		else if($this->website_alt != '' && !(validate::is_weblink($this->website_alt)))
		{
	...
	function update()
	{
      ...
      if(empty($errors)==true){
         move_uploaded_file($file_tmp,"server/php/files/thumbnail/".$file_name);
         echo "Success";
		...
		unlink($thumbfilePath);
			  ...
			  $sql="update user set		
				...
				$sql .= " where usr_id=".$this->userd;					
					...
					//$this->msg='<font color="#009900">Profile updated successfully</font>';
...
if(isset($_POST['btnUpdate'])){
 	...
 	$ecms->name_prefix=trim(addslashes($_POST['name_prefix']));
	$ecms->fname=trim(addslashes($_POST['fname'])); 
	$ecms->lname=trim(addslashes($_POST['lname']));
	$ecms->userdesignation=trim(addslashes($_POST['userdesignation']));
	$ecms->ceo_prefix=trim(addslashes($_POST['ceo_prefix']));
	$ecms->ceo_fname=trim(addslashes($_POST['ceo_fname']));
	$ecms->ceo_lname=trim(addslashes($_POST['ceo_lname']));
	$ecms->company_name=trim(addslashes($_POST['company_name']));
	$ecms->address=trim(addslashes($_POST['address']));
	$ecms->address1=trim(addslashes($_POST['address1'])); 
	$ecms->city=trim(addslashes($_POST['city']));
	$ecms->state=trim(addslashes($_POST['state']));
	$ecms->zipcode=trim(addslashes($_POST['zipcode']));
	$ecms->telephone_areacode1=trim(addslashes($_POST['telephone_areacode1']));
	$ecms->telephone1=trim(addslashes($_POST['telephone1']));
	$ecms->telephone_areacode2=trim(addslashes($_POST['telephone_areacode2']));
	$ecms->telephone2=trim(addslashes($_POST['telephone2']));
	$ecms->telephone_areacode3=trim(addslashes($_POST['telephone_areacode3']));
	$ecms->telephone3=trim(addslashes($_POST['telephone3']));
	$ecms->telephone_areacode4=trim(addslashes($_POST['telephone_areacode4']));
	$ecms->telephone4=trim(addslashes($_POST['telephone4']));
	$ecms->mobile1=trim(addslashes($_POST['mobile1']));
	$ecms->mobile2=trim(addslashes($_POST['mobile2']));
	$ecms->mobile3=trim(addslashes($_POST['mobile3']));
	$ecms->mobile4=trim(addslashes($_POST['mobile4']));
	$ecms->fax_areacode1=trim(addslashes($_POST['fax_areacode1']));
	$ecms->fax1=trim(addslashes($_POST['fax1']));
	$ecms->fax_areacode2=trim(addslashes($_POST['fax_areacode2']));
	$ecms->fax2=trim(addslashes($_POST['fax2']));
	$ecms->email_alt1=trim(addslashes($_POST['email_alt1']));
	$ecms->email_alt2=trim(addslashes($_POST['email_alt2']));
	$ecms->email_alt3=trim(addslashes($_POST['email_alt3']));
	$ecms->website_alt=trim(addslashes($_POST['website_alt']));
	$ecms->businessdocument = trim(addslashes($_POST['business_documents']));
	$ecms->profileimage =  trim(addslashes($_POST['profile_photo']));
...
<div style="display: block;" id="msgsave"> <p class="aml"></p><div id="re_link" class="utab" style="height:36px;"><span style="font-size: 12px;*float:left;">Complete &amp; Updated contact information gives your buyers more options to contact you.</span></div> </div>
	<div class="mt5">
...
<div class="ma sbox bnr fw lh" id="div_succ" style="display:none;"></div>
    ...
    <div class=" mp2 bnr">
    <div class="mp3 f1 bnr" style="width:40%;background-image: none;"><h2></h2></div>
    <a href="javascript:headOfficesv();" class="sl f2 edit mt c bnr" id="edi1" >Edit</a>
    <div id="cls1" class="fr" style="width: 150px; display: block;">
	<a style="display: none;" href="javascript:void(0);" onclick="return canc('eform');" class="sl f2 bnr c close mt" id="discard">Discard</a>
	<!--<a href="javascript:headOfficeUp();" class="sl f2 bnr c sav mt" id="savecd" style="display: none;">Save</a>-->
	</div>
	</div>
	<div class="mp10">
...
<tbody><tr>
<td class="label" width="160"><span>*</span>&nbsp;Contact Person</td>
<td>
...
<tr>
<td class="label" width="160">Designation / Job Title</td>
...
<span id="d1a" class="em mt5" style="display:none"></span>
        ...
        <tr>
        <td class="label" width="160">CEO Name</td>
        <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
        <td width="53">        
    ...
    <tr>
    <td class="label" width="160">Company Name</td>
...
<span id="ad" class="em mt5" style="display:none"></span><span id="ad1a" class="em" style="display:none"></span>
    ...
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr class="csp">
        <td width="300"><span>*</span>&nbsp;( City >> State )</td>
        <td style="width:103px;*width:150px;align:left">Postal / ZIP Code</td>
	<td style="*width:0px;" width="16"></td>
        </tr>
        <tr>
         <td colspan="4"><div id="a7" class="tbp1 tbm10 f1" style="display:none"></div>
			<input autocomplete="off" id="city_others" name="city_others" placeholder="Search city..." class="a_f rf"  maxlength="50" style="margin-right:3px;width:220px;*width:70px;*float:left" tabindex="11"  onClick="blankcity()" type="text" value="<?php echo get_city_name(user_info($uid,'bnsprof_city'));?>" >         
			<input class="ui-autocomplete-input" name="city" id="city" autocomplete="off" value="<?php echo user_info($uid,'bnsprof_city');?>" type="hidden">
			<div id="a8" class="tbp1 tbm7 f1" style="display:none"></div>

<input class="a_f ci" name="stateid" id="stateid" autocomplete="off" maxlength="30" style="margin-right:10px;width:80px;*width:70px;*float:left"  tabindex="12"  type="text" value="<?php echo get_state_name(user_info($uid,'bnsprof_state'));?>">
            
			<input class="ui-autocomplete-input" name="state" id="state" autocomplete="off" value="<?php echo user_info($uid,'bnsprof_state');?>" type="hidden">
			<div id="a9" class="tbp1 tbm13 f1" style="display:none"></div>
            <input gtbfieldid="109" name="zipcode" style="margin-right:3px;width:71px;*width:65px;*float:left" id="zipcode" tabindex="13" maxlength="10" onfocus="javascript:ntt('q9','a9');" onblur="nhid('a9');zip_code();" class="a_f ci" type="text" value="<?php echo user_info($uid,'bnsprof_zipcode');?>">
           </td>
        </tr>
        </tbody></table>
...
<input name="country_name" value="<?php echo get_country_name(user_info($uid,'country'));?>" id="country_name1" style="" tabindex="14" class="a_f rf" readonly="readonly" type="text">
    ...
    <tr>
    <td class="label" width="160"><span>*</span>&nbsp;Telephone 1</td>
    ...
    <tr>
    <td class="label" width="160">&nbsp;Telephone 2</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
    ...
    <tr>
    <td class="label" width="160">&nbsp;Telephone 3</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
    ...
    <tr>
    <td class="label" width="160">&nbsp;Telephone 4</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
    ...
    <tr>
    <td class="label" width="160"><span>*</span>&nbsp;Mobile/Cell Phone 1</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
    ...
    <tr>
    <td class="label" width="160">&nbsp;Mobile/Cell Phone 2</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
   ...
   <tr>
    <td class="label" width="160">&nbsp;Mobile/Cell Phone 3</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
    ...
    <tr>
    <td class="label" width="160">&nbsp;Mobile/Cell Phone 4</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
    ...
    <tr>
    <td class="label" width="160">Fax 1</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
    ...
    <tr>
    <td class="label" width="160">Fax 2</td>
    <td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody><tr>
   ...
   <input gtbfieldid="16" name="fax_areacode2" id="fax_areacode2" size="5" maxlength="6" onfocus="javascript:ntt('q24','a24');" onblur="nhid('a24');faxarea_valid('fax_area2','fa2','fax2');" tabindex="39" class="a_f ml8 a_c" value="<?php echo user_info($uid,'bnsprof_faxcode2');?>"></td>
    ...
    <tr>
    <td class="label" width="160"><span>*</span>&nbsp;Primary E-mail</td>
...
communication including enquiries. Your email address will also be your <strong>login id</strong>.</div></div><input readonly="readonly" style="background-color:#eaeaea;" tabindex="41" value="<?php echo user_info($uid,'email');?>" name="email" id="email" maxlength="100" onfocus="javascript:ntt('q26','a26');" onblur="nhid('a26');email_valid('email','eml')" class="a_f rf" type="text"><span id="eml" class="em" style="display:none"></span></td>
    ...
    <tr>
    <td width="160">&nbsp;</td>
    <td><a style="" class="ajax" href="change-email.php" tiptitle="Change your primary Email" id="changepe1">Change Login E-Mail id</a></td>
    </tr>
    <?php }?>
    <tr>
    <td class="label" width="160">Alternate E-mail 1</td>
    ...
    <tr>
    <td class="label" width="160">Alternate E-mail 2</td>
    ...
    <tr>
    <td class="label" width="160">Alternate E-mail 3</td>
    ...
    <tr>
    <td class="label" width="160">Alternate Website</td>
	...
	<tr>
		...
		<td>
			 <div class="upload_div">
							...
							<?php } else{ ?>
							<img src="http://arabyos.com/images/uploadd.png" width="110" id="profilephoto1" height="100">	
							<?php } ?>
    ...
    <input id="btnUpdate" class="saps" value="Save" name="btnUpdate" tabindex="48" type="submit">&nbsp;
...
<div class="mp10" >
	<div style="display: <?php if($msg==''){?>block<?php }else{?>none<?php }?>;" class="mp10 hdedfrm" id="cview">
	<div class="mp8">Contact Person</div>
    <div class="mp7"><strong><label id="lbl_salute"><?php echo user_info($uid,'name_prefix');?></label>
     <label id="lbl_first_name"><?php echo user_info($uid,'fname');?></label>
     <label id="lbl_last_name"><?php echo user_info($uid,'lname');?> </label></strong></div>
	<div class="mp8" style="clear:both">Designation / Job Title</div>
    <div class="mp7"><label id="lbl_designation"><?php echo user_info($uid,'bnsprof_designation'); ?></label></div>
	<div class="mp8">CEO Name</div><div class="mp7">
    <strong><label id="lbl_ceo_salute"><?php echo user_info($uid,'bnsprof_ceoprefix');?></label>
     <label id="lbl_ceo_fname"><?php echo user_info($uid,'bnsprof_ceofname');?></label>
     <label id="lbl_ceo_lname"><?php echo user_info($uid,'bnsprof_ceolname');?></label></strong>
     </div>
	<div class="mp8">Company Name</div><div class="mp7"><label id="lbl_comp_name"><?php echo user_info($uid,'bnsprof_compname');?></label></div>
	<!--<div class="mp8">&nbsp;</div><div class="mp7">&nbsp;</div>-->
	<div class="mp8">Address</div><div class="mp7"><label id="add_1"><?php echo user_info($uid,'bnsprof_address1');?></label></div>
	<div class="mp8"></div><div class="mp7"><label id="add_2"><?php echo user_info($uid,'bnsprof_address2');?></label></div>
	<div class="mp8">City, State, ZIP Code</div>
    <div class="mp7"><label id="city_state_zip">
	<?php echo get_city_name(user_info($uid,'bnsprof_city')).",".get_state_name(user_info($uid,'bnsprof_state')).",".user_info($uid,'bnsprof_zipcode');?></label></div>
	<div class="mp8">Country</div>
    <div class="mp7"><label id="lbl_country_name"><?php echo get_country_name(user_info($uid,'country'));?></label></div>
	<!--<div class="mp8">&nbsp;</div><div class="mp7">&nbsp;</div>-->
	<div class="mp8">Telephone1</div><div class="mp7">
    <label id="lbl_telephone"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_phcode1')."-".user_info($uid,'bnsprof_ph1');?></label>
    </div>
	<div class="mp8">Telephone2</div><div class="mp7">
    <label id="lbl_telephone2"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_phcode2')."-".user_info($uid,'bnsprof_ph2');?></label></div>
	<div class="mp8">Telephone3</div><div class="mp7">
    <label id="lbl_telephone3"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_phcode3')."-".user_info($uid,'bnsprof_ph3');?></label></div>
	<div class="mp8">Telephone4</div><div class="mp7">
    <label id="lbl_telephone4"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_phcode4')."-".user_info($uid,'bnsprof_ph4');?></label></div>
	<div class="mp8">Mobile/Cell Phone1</div><div class="mp7">
    <label id="lbl_mobile"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'mobile1');?></label></div>
	<div class="mp8">Mobile/Cell Phone2</div><div class="mp7">
    <label id="lbl_mobile2"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_mobile2');?></label></div>
	<div class="mp8">Mobile/Cell Phone3</div><div class="mp7">
    <label id="lbl_mobile3"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_mobile3');?></label></div>
	<div class="mp8">Mobile/Cell Phone4</div><div class="mp7">
    <label id="lbl_mobile4"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_mobile4');?></label></div>
	<div class="mp8">Fax 1</div><div class="mp7">
    <label id="lbl_fax"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_faxcode1')."-".user_info($uid,'bnsprof_fax1');?></label></div>
	<div class="mp8">Fax 2</div><div class="mp7">
    <label id="lbl_fax2"><?php echo user_info($uid,'country_ph_code')."-".user_info($uid,'bnsprof_faxcode2')."-".user_info($uid,'bnsprof_fax2');?></label></div>
    ...
    <img src="social_media_images/linkedinLog.png" />&nbsp;&nbsp;&nbsp;
     ...
     <?php if(user_info($uid,'usr_oauth_reg') == '0'){?>
    <a style="" class="ajax" href="change-email.php" tiptitle="Change your primary Email" id="changepe1">Change</a>
     <?php }?>
    </div>
	...
	<div class="mp8">Alternate Email1</div><div class="mp7"><label id="lbl_email2"><?php echo user_info($uid,'bnsprof_emailalt1');?></label></div>
	<div class="mp8">Alternate Email2</div><div class="mp7"><label id="lbl_email3"><?php echo user_info($uid,'bnsprof_emailalt2');?></label></div>
	<div class="mp8">Alternate Email3</div><div class="mp7"><label id="lbl_email4"><?php echo user_info($uid,'bnsprof_emailalt3');?></label></div>
    <div class="mp8">Alternate Website</div><div class="mp7"><label id="lbl_website"><?php echo user_info($uid,'bnsprof_website_alt');?></label></div>
	<div class="mp8 hide">Business Documents</div>
	<div class="mp7 hide"><label id="lbl_website"><img src="<?php echo BASE_URL ?>/server/php/files/thumbnail/<?php echo user_info($uid,'attachment');?>" height="80px"></label></div>
	<div class="mp8">Contact person photo</div>
	<div class="mp7"><label id="lbl_website">
	<?php if(user_info($uid,'image')!=""){ ?>
		<img src="<?php echo 'data:image/jpg;base64,'.base64_encode( getUserInfo($uid,'profileImage'));?>" width="110" id="profilephoto1" height="130">
	<?php } else{ ?>
	<img src="http://arabyos.com/images/uploadd.png" width="110" id="profilephoto1" height="130">	
	<?php } ?>
	</label></div>
    <div class="clb"></div>
	...
	<a href="javascript:addContact();" class="sl f2 bnr c sav mt">Save</a>
...
$.get("ajax-file/editContact.php", {id:id,division:division,
	...
	<tbody><tr>
		...
		<?php
        $divisionres=mysql_query("select * from division where dvtn_status='1' ");	
        while($divisionrow=mysql_fetch_object($divisionres))
		{
        ?>
...
communication including enquiries. Your email address will also be your <b>login id</b>.</div></div><input name="comp_cnt_email" id="comp_cnt_email" class="a_f rf" maxlength="200" tabindex="16" type="text"></td>
	...
	<tr>
        ...
        <input value="Save" name="save" class="saps" onclick="addContact();" id="button1" tabindex="17" type="button">&nbsp; <span id="new_save" style="display: none;"><img src="images/loading.gif" alt="" border="0" width="16" height="11"></span></td>
	</tr>
	</tbody></table>
    </div>
    </form>
...
<!--<a style="display: block;" href="javascript:shownewblk();" class="c am bnr fr" id="add_new"></a> -->  
    ...
    <div>&nbsp;</div>
		...
		<div class="nw7"><table border="0" cellpadding="0" cellspacing="0" width="100%"><tbody><tr><td width="50%">Fact Sheet</td><td width="50%"><div class="nw9 c s3 f1 bnr"><a href="javascript:void(0);">Add Now</a></div></td></tr></tbody></table></div>
			...
			<!--do you know widget:start--><div>&nbsp;</div> <div class="dyk1" id="do_you_know"> <div class="dyk2"><h2>Do you know?</h2></div> <div class="dyk3">You can upload <strong><span>Products</span></strong> and advertise <strong>absolutely</strong> <strong><span>with all advantages</span></strong><div><a href="product-add.php">Add Products, Now</a></div> </div><div class="dyk4">&nbsp;</div></div> <!-- do you know widget:end -->
