...
function purchaseAuction(id)
{
	if(confirm('Are you sure to buy this Auction?'))
	{
		$.post("ajax-file/purchaseAuction.php",{id:id},    function(data){  $("#buy_alert_msg").removeClass("doff"); });	
	}
}
...
{
	alert('Please purchase credits to Buy this Auction.');	
}
</script>
...
$sql="select * from auction,product_category,user,business_profile where auc_pc_id=pc_id and auc_usr_id=usr_id and usr_id=bnsprof_uid and md5(auc_id)='".$auc_id."'";
$res=mysqli_query($con, $sql);
$row=mysqli_fetch_object($res);
if(empty($row)){
	$sql="select * from auction,user,business_profile where auc_usr_id=usr_id and usr_id=bnsprof_uid and md5(auc_id)='".$auc_id."'";
$res=mysqli_query($con, $sql);
$row=mysqli_fetch_object($res);
}
$credit_available=0;
	$sql_pcat="select m.pc_id,m.pc_name,c.pc_id,c.pc_sort_name,s.pc_sort_name from product_category m,product_category c,product_category s where m.pc_id=c.pc_parent_id and c.pc_id=s.pc_parent_id and s.pc_id='".$row->auc_pc_id."'";
	$res_pcat=mysqli_query($con, $sql_pcat);
	$row_pcat=mysqli_fetch_array( $res_pcat);
?>

	<div align="CENTER"><div style="width:100%;">
	 <div class="p3 pl lf mm"><a href="auctions.php" class="c12 td">Auctions</a> &nbsp;&gt; &nbsp; 
    ...
    <h1 class="f6 cl2" style="display:inline" id="lead_headline"><?php echo $row->auc_heading; ?> </h1>
    
    <?php 
		if(isset($_SESSION['uid_indm']) || $_SESSION['uid_indm']!=''){
			$key=trim($row->auc_heading); 
				$sql_key="select * from auction join product_category on product_category.pc_id=auction.auc_pc_id where auc_heading = '".$key."' and pc_status='1'";
				//echo $sql_key;
...
<p style="color: rgb(185, 184, 184);float:right; text-align: right;" class="j1 cb"><font style="color: rgb(152, 151, 151);">Auction Publish Date :</font> <?php echo date("d M, Y",strtotime($row->auc_publish_date)); ?><!-- IST--></p>
<div>
...
<div class="mojozoom_border"></div>
	...
	<span class="c12 bo fs" style="font-size:14px">Auction Details:</span>
	<div class="bdt" id="hdiv1" style="padding-top:5px">
            <div class="g2 fs k7">
              <div>              
              <?php if($row->auc_value!='0' && $row->auc_value != '0.00'){ ?>
              <div><div><span class="c13"><strong>Auction Value </strong>: <?php echo $row->auc_value."&nbsp;".getCurrency($row->auc_currency); ?></span></div></div>
              <?php	}	?>
              <?php if($row->auc_notice_type!=''){ ?>
              <div><div><span class="c13"><strong>Notice Type </strong>: <?php echo $row->auc_notice_type; ?></span></div></div>
              <?php	}	?>
              <?php if($row->auc_qty!='0' && $row->auc_qty != ''){ ?>
              <div><div><span class="c13"><strong>Quantity </strong>: <?php echo $row->auc_qty."&nbsp;".measurement_unit($row->auc_qty_mu_id); ?></span></div></div>
              <?php	}	?>
              <?php if($row->auc_emd!=''){ ?>
              <div><div><span class="c13"><strong>EMD </strong>: <?php echo $row->auc_emd; ?></span></div></div>
              <?php	}	?>
              <?php if($row->auc_document_fees!='0' && $row->auc_document_fees != '0.00'){ ?>
              <div><div><span class="c13"><strong>Document Fees </strong>: <?php echo $row->auc_document_fees."&nbsp;".getCurrency($row->auc_document_fees_currency); ?></span></div></div>
              <?php	}	?>
              <?php if($row->auc_project_period!=''){ ?>
              <div><div><span class="c13"><strong>Project Period </strong>: <?php echo $row->auc_project_period; ?></span></div></div>
              <?php	}	?>
              <?php if($row->auc_products!=''){ ?>
              <div><div><span class="c13"><strong>Products </strong>: <?php echo $row->auc_products; ?></span></div></div>
              <?php	}	?>
      ...
      <br>
  	  ...
  	  <?php if($row->auc_preferred_location!=''){ ?><div><div><span class="c13"><strong>Preferred location </strong>: <?php 
	...
	<div class="bdt" id="hdiv1" style="padding-top:5px">
      ...
      <?php
	  if($row->auc_publish_date!='' && $row->auc_publish_date!='0000-00-00'){ ?><div><div><span class="c13"><strong>Publish Date </strong>: <?php echo date("d-M, Y",strtotime($row->auc_publish_date)); ?></span></div></div> <?php } ?>
       <?php
	  if($row->auc_docSaleStart_date!='' && $row->auc_docSaleStart_date!='0000-00-00'){ ?><div><div><span class="c13"><strong>Document Sale Starts </strong>: <?php echo date("d-M, Y",strtotime($row->auc_docSaleStart_date)); ?></span></div></div> <?php } ?>
       <?php
	  if($row->auc_docSaleEnd_date!='' && $row->auc_docSaleEnd_date!='0000-00-00'){ ?><div><div><span class="c13"><strong>Document Sale Ends </strong>: <?php echo date("d-M, Y",strtotime($row->auc_docSaleEnd_date)); ?></span></div></div> <?php } ?>
       <?php
	  if($row->auc_docSubmitBefore_date!='' && $row->auc_docSubmitBefore_date!='0000-00-00'){ ?><div><div><span class="c13"><strong>Document Submit Before </strong>: <?php echo date("d-M, Y",strtotime($row->auc_docSubmitBefore_date)); ?></span></div></div> <?php } ?>
	  <?php if($row->auc_due_date!='' && $row->auc_due_date!='0000-00-00'){ ?><div><div><span class="c13"><strong>Due Date </strong>: <?php echo date("d-M, Y",strtotime($row->auc_due_date)); ?></span></div></div> <?php } ?>
...
<br>
    ...
    <div class="bdt" id="hdiv1" style="padding-top:5px">
	  ...
	  <div><div><span class="c13"> <?php echo stripslashes($row->auc_prequalification_criteria); ?></span></div><br></div>
	  
	  
	</div>
</div>
	</div>
    <?php	if($row->auc_details!=''){	?>
    <span class="bdd bo" style="font-size:14px"><span class="artb sbg"></span> Detail Description</span>
    <div class="bdt" id="hdiv1" style="padding-top:5px">
		<div class="g2 fs k7">
			<div class="c15 pt4 f1 pl">
				<div><div>
                	<span class="c13"> <?php echo stripslashes($row->auc_details); ?></span>
                </div><br></div>
			</div>
		</div>
	</div>
    ...
    <div class="bdt" id="hdiv1" style="padding-top:5px">
		<div class="g2 fs k7">
			...
			?>
             ...
             <?php
				$sql_aav="select * from auction_additional_value where aav_af_id='".$row_af->tav_af_id."' and aav_auc_id='".$row->auc_id."'";
	...
	<b>Thank You for Purchasing the Auction!</b><br>
	<div style="padding-left:5px;padding-top:3px">&#8226; This Auction is saved in your "<a href="manage-purchased-auctions.php">Purchased Auctions</a>" section
	
	<div style="height:10px;overflow:hidden"></div>
	&#8226; You can submit your response to this Buyer from "<a href="manage-purchased-auctions.php">Purchased Auctions</a>"
	...
	<div class="wd9 lf gv" style="float:right;"> <!--TIME After Buy Lead Service Help: 0.086 -->
        ...
        <?php } ?>
          ...
          <div id="rtmain" class="lbx1" style="background-color:#FAF4FF;">
			<p class="sbg d_bp1 bo f1"><?php echo date("d M, Y",strtotime($row->auc_publish_date)); ?> <span class="x2 date_tp tooltip2 qmart sbg" id="Auction Last Updated on: <?php echo date("d M, Y",strtotime($row->auc_publish_date)); ?> ### Publish Date: <?php echo date("d M, Y",strtotime($row->auc_publish_date)); ?>">&nbsp;</span></p>
		...
		<?php	}
            ...
            <?php if(isset($_SESSION['uid_indm']) && $_SESSION['uid_indm']!=$row->auc_usr_id){	?>
			
                <?php
                	$sql_chk="select * from purchased_auction where pauc_usr_id='".$_SESSION['uid_indm']."' and pauc_auc_id='".$row->auc_id."'";
            	...
            	<div class="mt12 l1 k7 mb">
                    <div class="btn1 point mt12 f4" id="buybtn" style="line-height: 20px;padding: 5px 27px;" onClick="purchaseAuction(<?php echo $row->auc_id; ?>);">
					    <span class="f1">Purchased on: <?php echo date("d M, Y",strtotime($row_chk->pauc_purchase_date)); ?></span>
...
<?php } ?> 
    	...
    	<input type="hidden" id="msg_subject" name="msg_subject" value="<?php echo "Enquiry for Auctions"; ?>" />
        ...
        <p class="c3"></p></div></div>
            	...
            	<div class="mt12 l1 k7 mb">
					<div class="btn1 point mt12 f4" id="buybtn" style="line-height: 20px;padding: 5px 27px;" onClick="purchaseAuction(<?php echo $row->auc_id; ?>);">
                    	View Auction Information<br>
						<span class="f1">Buy this Auction Now</span>
    	                <div class="inAr sbg"></div>  
                    </div>
                    </div>
			</div>
                ...
                <div id="sourcediv1">
            	<div class="mt12 l1 k7 mb">
					<div class="btn1 point mt12 f4" id="buybtn" style="line-height: 20px;padding: 5px 27px;" <?php if(getUserCredit($_SESSION['uid_indm'])>=20){ ?> onClick="purchaseAuction(<?php echo $row->auc_id; ?>);" <?php }else{ ?> onClick="showMessage();" <?php } ?>>
                    	View Auction Information<br>
					    <span class="f1">Buy this Auction Now</span>
    	                <div class="inAr sbg"></div>
        	            <div id="tps" class="doff sbg g1 k7">After Purchasing this Auction, Authority Contact Details and full information will be visible to you</div>
                    </div>
                    </div>
