...
function highlight( $content, $word) {
    $replace = '<span style="color: #f26a22;">' . $word . '</span>'; // create replacement
    $word = str_replace("+"," ",$word);
    $pattern = preg_quote($word);
    $content = preg_replace( "/($pattern)/i", '<span style="color: #f26a22;">$1</span>', $content ); // replace content
...
if (isset($_COOKIE['loc_id'])) { 
	...
	or
	(pd_preferred_buyer_location='abroad' and pd_uid not in(select distinct usr_id from user where country=(select cn_id from country where cn_code='" . $_COOKIE['loc_id'] . "')))
	)";
...
//echo $sql_key;die;
    ...
    if (isset($_GET['exmatch'])) {
        if (isset($_POST['state_id'])) {
            $sqltk = "select " . $Col . " from products,measurement_unit_arabyos,business_profile where bnsprof_uid=pd_uid and ((pd_title LIKE '%" . $keywords . "%') or (bnsprof_compname LIKE '%" . $keywords . "%')) " . $sql_pd_ck . " and bnsprof_state in (" . $stateid . ") and pd_status='1' GROUP BY pd_id order by pd_title asc limit 0,6";
            //echo $sqltk;	 echo '___111';
        } else {
            $sqltk = "select " . $Col . " from products,measurement_unit_arabyos,business_profile where bnsprof_uid=pd_uid and ((pd_title LIKE '%" . $keywords . "%') or (bnsprof_compname LIKE '%" . $keywords . "%'))" . $sql_pd_ck . " and pd_status='1' GROUP BY pd_id order by pd_title asc limit 0,6";
        ...
        if (isset($_POST['state_id'])) {
            $sqltk = "select " . $Col . " from products,measurement_unit_arabyos,business_profile where bnsprof_uid=pd_uid and ((pd_title LIKE " . $keywords_string . ") or (bnsprof_compname LIKE " . $keywords_string_pro_sup . ")) " . $sql_pd_ck . " and pd_status='1' and bnsprof_state in (" . $stateid . ") GROUP BY pd_id order by pd_title asc limit 0,6";
            // echo $sqltk;	 echo '___333';
        } else {
            $sqltk = "select " . $Col . " from measurement_unit_arabyos,products,business_profile where bnsprof_uid=pd_uid and ((pd_title LIKE " . $keywords_string . ") or (bnsprof_compname LIKE " . $keywords_string_pro_sup . ")) " . $sql_pd_ck . " and pd_status='1' GROUP BY pd_id order by pd_title asc limit 0,6";
    ...
    if (isset($_GET['exmatch'])) {
        if (isset($_POST['state_id'])) {
            $sqlk = "select " . $Col . " from products,measurement_unit_arabyos,business_profile where bnsprof_uid=pd_uid and (pd_title LIKE '%" . $keywords . "%' or bnsprof_compname LIKE '%" . $keywords . "%') " . $sql_pd_ck . " and pd_status='1' and bnsprof_state in (" . $stateid . ") GROUP BY pd_id order by pd_title asc limit " . $start_limit . "," . $limits;

            //echo $sqltk;	 echo '___555';
        } else {
            $sqlk = "select " . $Col . " from products,measurement_unit_arabyos,business_profile where bnsprof_uid=pd_uid and (pd_title LIKE '%" . $keywords . "%' or bnsprof_compname LIKE '%" . $keywords . "%') " . $sql_pd_ck . " and pd_status='1' GROUP BY pd_id order by pd_title asc limit " . $start_limit . "," . $limits;
        ...
        $keywords_string = generateProdSearchString($keywords);
        if (isset($_POST['state_id'])) {
            $sqlk = "select " . $Col . " from products,measurement_unit_arabyos,business_profile where bnsprof_uid=pd_uid and (pd_title LIKE " . $keywords_string . " or bnsprof_compname LIKE '%" . $keywords_string . "%') " . $sql_pd_ck . " and pd_status='1' and bnsprof_state in (" . $stateid . ") GROUP BY pd_id order by pd_title asc limit " . $start_limit . "," . $limits;

            //////echo $sqltk;	 echo '___777';	
        } else {
            $sqlk = "select " . $Col . " from products,measurement_unit_arabyos,business_profile where bnsprof_uid=pd_uid and (pd_title LIKE " . $keywords_string . " or bnsprof_compname LIKE '%" . $keywords_string . "%') " . $sql_pd_ck . " and pd_status='1' GROUP BY pd_id order by pd_title asc limit " . $start_limit . "," . $limits;
        ...
        if (isset($_GET['exmatch'])) {
            ...
            if (isset($_POST['country_id'])) {
                ...
                foreach ($countryid as $key => $value) {
                    if ($p == 1) {
                        $cntryval2 .= " and (country.cn_name = '" . $value . "'";
                    } else {
                        $cntryval2 .= " or country.cn_name = '" . $value . "'";
                    }
                    $p++;
                }
...
/* added by Hetal */
	if(isset($_GET['keywords']) && $_GET['rctyp'] == 'buy_lead'){
		...
		//$sql_key= "select * from buy_requirement br JOIN measurement_unit_arabyos mu ON br.br_estimate_qty_unit=mu.mu_id JOIN user u ON u.usr_id = br.br_u_id LEFT JOIN business_profile bf ON bf.bnsprof_uid = br.br_u_id LEFT JOIN country c ON c.cn_id = u.country LEFT JOIN smembership_icon_plan sip ON sip.mp_id = u.usr_mp_id WHERE (br_pd_name like '%".str_replace("+"," ",$_GET['keywords'])."%' OR pc_name like '%".str_replace("+"," ",$_GET['keywords'])."%') and br_display_status = '1' and br_status='1' order by br_pd_name asc";
		//echo $sql_key;die;
		$query_key = mysql_query($sql_key);
		$row_key=mysql_fetch_object($query_key);
		$key_cat_id='';
    ...
    if ($_GET['bbidd'] != '') {
        ...
        $iCategoryMatchNee = " and br_id IN ('$MasterCategoryArrayAA')";
         ...
         $sqlk = "select * , MATCH (br_pd_name) AGAINST ('" . $keywordsssSS . "'  IN BOOLEAN MODE) AS title_relevance from buy_requirement,measurement_unit_arabyos where br_estimate_qty_unit=mu_id   and  (" . $keywords_string . ") and br_approval_status = '1' and br_display_status = '1'  $iCategoryMatchNee  and br_status = '1' " . $sql_br_ck . " " . $sql_extra . " order by title_relevance desc  ";

        //$sqlk = "select * , MATCH (br_pd_name) AGAINST ('".$keywordsssSS."'  IN BOOLEAN MODE) AS title_relevance from buy_requirement,measurement_unit_arabyos where br_estimate_qty_unit=mu_id    ".$keywords_string."  and br_approval_status = '1' and br_display_status = '1' and br_status = '1' ".$sql_br_ck." ".$sql_extra." $iCategoryMatchNee order by title_relevance desc  " ;
        //$sqlk = "select * , MATCH (br_pd_name) AGAINST ('".$keywordsssSS."'  IN BOOLEAN MODE) AS title_relevance from buy_requirement,measurement_unit_arabyos where br_estimate_qty_unit=mu_id   $iCategoryMatchNee  and br_approval_status = '1' and br_display_status = '1' and br_status = '1'  order by title_relevance desc  " ;
    } else {
        ...
        if (isset($_GET['exmatch'])) {
            $sqltk = "select * from buy_requirement,measurement_unit_arabyos where br_estimate_qty_unit=mu_id and (br_pd_name LIKE '%" . $keywords . "%' or br_requirement LIKE '%" . $keywords . "%') and br_approval_status = '1' and br_display_status = '1' " . $sql_br_ck . " " . $sql_extra . " order by br_pd_name asc";
            //echo "9 ".$sqltk;
            // br_id desc
            //echo "2222".$sqltk;
        } else {
        ...
        $start_limit = $limits * ($pageno - 1);
        if (isset($_GET['exmatch'])) {
            $sqlk = "select * , MATCH (br_pd_name) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from buy_requirement,measurement_unit_arabyos where br_estimate_qty_unit=mu_id and (br_pd_name LIKE '" . $keywords . "' or br_requirement LIKE '" . $keywords . "') and br_approval_status = '1'  and br_status = '1' and br_display_status = '1' " . $sql_br_ck . " " . $sql_extra . " order by title_relevance desc limit " . $start_limit . "," . $limits;
            //echo "11 ".$sqltk;
            //echo "4444".$sqlk;
        } else {
            $keywords_string = generateBuyleadSearchString($keywords);
            /*$sqlk = "select * , MATCH (br_pd_name) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance from buy_requirement,measurement_unit_arabyos where br_estimate_qty_unit=mu_id and (" . $keywords_string . ") and br_approval_status = '1' and br_display_status = '1' and br_status = '1' " . $sql_br_ck . " " . $sql_extra . " order by title_relevance desc limit " . $start_limit . "," . $limits;*/
            $sqlk = "select * from buy_requirement br JOIN measurement_unit_arabyos mu ON br.br_estimate_qty_unit=mu.mu_id JOIN user u ON u.usr_id = br.br_u_id LEFT JOIN business_profile bf ON bf.bnsprof_uid = br.br_u_id LEFT JOIN country c ON c.cn_id = u.country LEFT JOIN smembership_icon_plan sip ON sip.mp_id = u.usr_mp_id WHERE br_pd_name LIKE '%".$keywords."%' and br_display_status = '1' and br_status='1' order by br_pd_name asc";
            //echo "12 ".$sqltk;
            //echo "5555".$sqlk;
        }
    }
    ...
    if ($_GET['ttidd'] != '') {
        ...
        $tendsqlk = "select * from tender,product_category_arabyos,user,business_profile where tnd_pc_id=pc_id and tnd_usr_id=usr_id and usr_id=bnsprof_uid and tnd_approval_status='1' and TO_DAYS(tnd_due_date)>=TO_DAYS(now())  and tnd_status='1'    $iCategoryMatchNee order by tnd_id ";
    } elseif ($_GET['ccidd'] != '') {
        $MasterCategoryArrayAA = "";
        ...
        $aucsqlk = "select * from auction,product_category_arabyos,user,business_profile where auc_pc_id=pc_id and auc_usr_id=usr_id and usr_id=bnsprof_uid and auc_approval_status='1' and TO_DAYS(auc_due_date)>=TO_DAYS(now())  and auc_status='1' $iCategoryMatchNee order by auc_id";
    } else {
        ...
        if (isset($_GET['exmatch'])) {
            ...
            $tendsqlk = "SELECT * FROM tender,product_category_arabyos,user,business_profile WHERE   tnd_pc_id=pc_id AND tnd_usr_id=usr_id AND usr_id=bnsprof_uid AND tnd_heading LIKE '" . $keywords . "' OR tnd_details LIKE '" . $keywords . "')  and tnd_due_date>='" . date('Y-m-d') . "' AND tnd_approval_status = '1' AND TO_DAYS(tnd_docSaleEnd_date)>=TO_DAYS(now()) " . $tenderCondition . " ORDER BY tnd_heading ASC";
            ...
            $tendsqlk = "SELECT * FROM tender,product_category_arabyos,user,business_profile WHERE tnd_pc_id=pc_id AND tnd_usr_id=usr_id AND usr_id=bnsprof_uid AND (" . $tender_keywords_string . ")  and tnd_due_date>='" . date('Y-m-d') . "' AND tnd_approval_status = '1'" . $tenderCondition . " ORDER BY tnd_heading ASC"; //AND tnd_approval_status='1' AND TO_DAYS(tnd_docSaleEnd_date)>=TO_DAYS(now()) 
    ...
    $sql_extra = "";
    if (isset($_GET['exmatch'])) {
        $sqltk = "select * from auction,product_category_arabyos,user,business_profile where auc_pc_id=pc_id and auc_usr_id=usr_id and usr_id=bnsprof_uid and auc_approval_status='1' and TO_DAYS(auc_docSaleEnd_date)>=TO_DAYS(now()) " . $sql_auc_ck . " and auc_status='1' and auc_heading='%" . $keywords . "%' order by auc_id desc";
                ...
                if ($data) {
                 ...
                 from smembership_plan,plan_member_id , smembership_icon_plan where smembership_icon_plan.mp_id =plan_member_id.p_id and smembership_plan.mp_id =plan_member_id.p_id  and plan_member_id.b_id = " . $rowk->bnsprof_id;
                    ...
                    if (mysql_num_rows($get_icon)) {
                        ...
                        /*webcast*/
$sql_icon2 = "select icon_id, p_id from plan_member_id where b_id = ".$data['bnsprof_id'];
$get_icon2 = mysql_query($sql_icon2) or die(mysql_error());
$icon2 = mysql_fetch_array($get_icon2);
$sql_icon1 = "select * from smembership_icon_plan where mp_id = ".$icon2['icon_id'];
$get_icon1 = mysql_query($sql_icon1) or die(mysql_error());
$icon1 = mysql_fetch_array($get_icon1);
$sql_icon3 = "select * from smembership_plan where mp_id = ".$icon2['p_id'];
$get_icon3 = mysql_query($sql_icon3) or die(mysql_error());
                }
                $munit = "SELECT * FROM `measurement_unit_arabyos` WHERE mu_id='$rowk->pd_unit'";
                        ...
                        <div class="row">
                            <div class="col-xs-6 col-lg-3 big-img-box box-1">
                                <header><meta http-equiv="Content-Type" content="text/html; charset=windows-1252">
                                    <?php if($_GET['rctyp'] != 'Suppliers'){  //webcast ?>
                                    <?php if (isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != '') { ?>
                                        <a href="javascript:void(0)" class="product_fav_btn" data="<?php echo $rowk->pd_id; ?>"  onclick="return showfavorite(<?php echo $_SESSION['uid_indm']; ?>,<?php echo $rowk->pd_id; ?>)" class="ar-star"><i class="fa fa-star star" style="color:<?php echo (in_array($rowk->pd_id, $myfev)) ? '#E48F23' : '#808080'; ?>"></i> Favorite</a>
                                    <?php } else { ?>
                                        <a href="sign-in.php" class="product_fav_btn" data="<?php echo $rowk->pd_id; ?>" class="ar-star"><i class="fa fa-star star"></i> Favorite</a>
                                    <?php } ?>
                                    
                                    <i class="fa fa-plus star"></i><a href="javascript:void(0)" class="ar-star product_compare" data-prod_img="<?php echo '/upload/myproduct/' . $rowk->pd_image; ?>"  onclick="return showcompare(<?php echo $rowk->pd_id; ?>)" data-prod_id="<?php echo $rowk->pd_id; ?>" title="<?php echo $rowk->pd_title; ?>" > Compare</a><?php } ?> </header>
                                <figure class="box" >
                                    <?php if($_GET['rctyp'] == 'Suppliers') {  //webcast ?>
                                    <?php if (isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != '') { //webcast add ?>
                                        <a href="javascript:void(0)" class="product_fav_btn" data="<?php echo $rowk->pd_id; ?>"  onclick="return showfavorite(<?php echo $_SESSION['uid_indm']; ?>,<?php echo $rowk->pd_id; ?>)" class="ar-star"><i class="fa fa-star star" style="color:<?php echo (in_array($rowk->pd_id, $myfev)) ? '#E48F23' : '#808080'; ?>"></i></a>
                                    <?php } else { ?>
                                        <a href="sign-in.php" class="product_fav_btn" data="<?php echo $rowk->pd_id; ?>" class="ar-star"><i class="fa fa-star star"></i></a>
                                    <?php } } ?>
                                    ...
                                    <div class="zoomthis">
                                        ...
                                        <?php } ?> 
                                        <?php if($pimg1[0]!=''){ echo "<img src='/upload/myproduct/" . $pimg1[0] . "'>"; }else{ echo "<img src='/images/noimage.jpg'>"; } ?></div>
                                    <?php
                                    if (!empty($rowk->pd_imagelogo)) {
                                        ...
                                        <div class="zk" style=" border: 1px solid #267abf;height: auto; width: 100px;position: absolute;bottom: 1px;left: 1px;">
                                            <?php echo "<img style='width: auto; height: 50px;max-width:100%;' src='/upload/myproduct/" . $limg1[0] . "'>"; ?></div>
                                    <?php } ?>   


                                </figure>
                                <center>
                                    <a onclick="zoom_image(this)" data-img="<?php echo $zoom_image_val;?>" style="padding: 10px;" ><i class="fa fa-search-plus"></i> Zoom </a>
                                </center>
                            ...
                            <div class="col-xs-6 col-lg-5 box-2" style="width:100%">
                                <ul>
                                    ...
                                    <li class="text-right" <?php if(!empty($rowk->brand_name)){ echo 'style="display: flow-root;"'; }?>>
                                        <?php if(!empty($rowk->brand_name)){ ?>
                                    	<span style="float: left;"><strong>Brand:</strong> <?php echo $rowk->brand_name; ?></span>
                                    	<?php } ?>
                                        <a <?php if (user_info($rowk->bnsprof_uid, 'bnsprof_compname') != '') { ?> href="company/products.php?c=<?php echo rand(1000, 9999) . md5($rowk->bnsprof_id); ?>&sc=<?php echo rand(10000, 99999) . $rowk->pd_subcat_id; ?>#<?php echo $rowk->pd_id; ?>"<?php } ?>>+  More</a> </li>
                                    ...
                                    <?php if ((int)$rowk->pd_fob_price == 0) {
                                            $style_none = 'hide';
                                        } 
                                        else {$style_none = 'show';} 
                                    ?>
                                    <li class="<?php echo $style_none; ?>">Price  &nbsp; <big class="txt-bold txt-red"><a style="color: #d22027" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/product-add.php"><?php echo $symbol . $rowk->pd_fob_price . '~' . $symbol . $rowk->pd_fob_price2; ?> </a></big> &nbsp;  
                                    <?php if (($_SESSION['uid_indm'] ) == '') { ?>
                                        <a style="float: right;position: relative;right: 20px;top: -8px;" data-enquiry="" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/sign-in.php">
                                            <button type="button" class="btn border-radius-0 btn-enquiry" style="font-weight:bold;">(Get Latest Price)</button></a> 
                                        <?php
                                    } else {
                                        if (($_GET['grid']) == 'active') {
                                            ?>
                                            <a style="float: right;position: relative;right: 20px;" class="ajax" data-price="" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo $rand . md5($rowk->bnsprof_id); ?>&pid=<?php echo $rowk->pd_id; ?>&keywords=<?php echo urlencode($_GET['keywords']); ?>&geo=<?php echo $geo_loc; ?>&conty=<?php echo $countryyyy; ?>&search=1" class="txt-bold txt-black pull-right inquiry_but" id="btn_ajax_send<?php echo $rowk->pd_id; ?>" rel="product-send-inquiry">
                                                <button type="button" class="btn border-radius-0 btn-enquiry" style="font-weight:bold;">(Get Latest Price)</button>
                                            </a>
                                        <?php } else { ?>
                                            <a  style="font-weight:bold; color: black;float: right;position: relative;right: 20px;top: -8px;" class="ajax" data-price="" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo $rand . md5($rowk->bnsprof_id); ?>&pid=<?php echo $rowk->pd_id; ?>&keywords=<?php echo urlencode($_GET['keywords']); ?>&geo=<?php echo $geo_loc; ?>&conty=<?php echo $countryyyy; ?>&search=1" class="txt-bold txt-black pull-right inquiry_but" id="btn_ajax_send<?php echo $rowk->pd_id; ?>" rel="product-send-inquiry">
                                                <button type="button" class="btn border-radius-0 btn-enquiry" style="font-weight:bold; color: black;">(Get Latest Price)</button>
                                            </a>
                                            <?php
                                        }
                                    }
                                    ...
                                    <!-- webcast works supplier -->
                                    <?php if ($rowk->pd_pn_capct != '') {
                                    ?>
                                    <li><a class="" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/product-details.php?token=<?php echo rand(1000, 9999) . md5($rowk->pd_id) ?>&c=<?php echo rand(1000, 9999) . md5($rowk->bnsprof_id); ?>" target="_blank">Logistics</a></li>
                                    <?php } ?>
                                    <li class="margin-top-5">
                                        <table class="table">
                                            <tr>
                                                ...
                                                <?php if ($rowk->pd_pdf_attach) { ?>
                                                <td><a href="//arabyos.com/upload/productdoc/<?php echo $rowk->pd_pdf_attach;?>" target="_blank"><img src="/images/pdf_icon.png" style="width: 28px;height: 28px;"> PDF</a></td>
                                                <?php } ?>
                            ...
                            <div class="col-lg-4 box-3">
                                <div class="ar-box-1 ar-box padding-5 margin-bottom-5 bg-gray" style="overflow-x: hidden;">
                                    <header class="sub-box"> <?php if ($fevrow_icon) { ?> <a href="https://www.arabyos.com/membership_plans.php"><img src="https://www.arabyos.com/admin/images/<?php echo $fevrow_icon['producticon']; ?>" title="<?php echo $fevrow_icon['pplan']; ?>" width="25px" height="25px"/></a><?php }
                                        elseif(mysql_num_rows($get_icon1)){ ?><a href="https://www.arabyos.com/membership_plans.php"><img src="./admin/images/<?php echo $icon1['mst_icon']; ?>" width="25px" height="25px" title="<?php echo $icon1['mst_name']; ?>"/></a><?php } ?>
                                    ...
                                    <img src="https://www.arabyos.com/images/country_flag/<?php echo $rowk->cn_flag; ?>" alt="<?php echo $rowk->cn_flag; ?>" style=" width:21.6px; height:21.6px;"/>
                                    ...
                                    <table class="table margin-top-5">
                                        ...
                                        <td class="padding-0 txt-bold" title="<?php echo $bus_type1; ?>"> <?php echo $bus_type; ?> </td>
                                            ...
                                            <tr>
                                                <td class="txt-light-gray padding-0"> <a style="font-size:12px;font-weight:100;color:#8a8a8a" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos.com/product-add.php">Trade Location:</a> </td>
                                                <td class="padding-0 txt-bold"> <a style="font-size:12px;color:#242424" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos.com/product-add.php"><?php
                ...
                <?php
									...
									//print("<BR> 3 ha:".$suppRowCount."---".$suppTotalRow);
                ...
                if ($suppRowCount == $suppTotalRow) {
                    ...
                    if ($totitems > $totalpage) {
                        echo '<div class="col-lg-12 text-center" style="padding:30px;"><a href="https://arabyos.com/search.php?rctyp=' . $_GET['rctyp'] . '&keywords=' . $_GET['keywords'] . '&page=' . $pages . '"><button type="button" class="btn btn-md btn-warning border-radius-0 btn-enquiry" style="font-size:16px; font-weight:bolder;">Display More Products / Services </button></a></div>';
                    }
                }
                $suppRowCount ++;
            }
    ...
    if ($_GET['grid'] != 'active') {
        ...
        <div id="search_result" class="list-grid-active" style="postion:relative;">
            ...
            <?php
            /* $sql_prd="select * from products,measurement_unit_arabyos,country where mu_id=pd_unit ".$keywords_string." and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' and pd_image!='' order by pd_id desc limit 0,5";
             */
            /*  $sql_prd="select * from products,measurement_unit_arabyos,country where mu_id=pd_unit and (pd_title LIKE '%".$_GET['keywords']."%') and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' and pd_image!='' order by pd_id desc limit 0,5";
             */
            if ($_GET['idd'] != "") {
                //$sql_prd="SELECT * FROM `products` WHERE `pd_subcat_id` = ".$_GET['idd']." ORDER BY pd_id DESC";
                // 	  
                // $sql_prd="select measurement_unit_arabyos.*,country.*,".$prod_col." from products,measurement_unit_arabyos,country where mu_id=pd_unit and pd_subcat_id = ".$_GET['idd']." and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' and pd_image!='' GROUP BY pd_id order by pd_title asc limit 0,15";
                $newkw = generateProdSearchString($keywords);
                
                //$sql_prd="select measurement_unit_arabyos.*,country.*,".$prod_col.", MATCH (pd_title) AGAINST ('".$keywords."'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid = pd_uid and b_id = bnsprof_id and mu_id=pd_unit and pd_subcat_id = ".$_GET['idd']."  and (pd_title LIKE ".$newkw.") and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' and pd_image!='' AND plan_member_id.expiry_date > ". time() ."   GROUP BY pd_id ORDER BY title_relevance DESC, FIELD(plan_member_id.p_id,'5','4','3','15'), pd_title asc LIMIt 0, 15"; 

                $sql_prd = "select * from products,measurement_unit_arabyos,country, business_profile, plan_member_id, smembership_plan
 where mu_id=pd_unit and pd_currency=cn_id " . $sql_pd_ck . " and  business_profile.bnsprof_uid = products.pd_uid and plan_member_id.b_id = business_profile.bnsprof_id and pd_status='1'  ANd smembership_plan.mp_id = plan_member_id.p_id and pd_image!='' and plan_member_id.expiry_date > " . time() . " and pd_subcat_id = '" . $_GET['idd'] . "' ORDER BY FIELD(p_id,'5','4','3','15','0') ";
                ...
                if (isset($_POST['srchbustype'])) {
                    ...
                    if (isset($_POST['mst_type'])) {
                        ...
                        $sql_prd = "select measurement_unit_arabyos.*,country.*," . $bus_col . "," . $prod_col . " , MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,business_profile,country, plan_member_id where bnsprof_uid=pd_uid and mu_id=pd_unit   and ((pd_title LIKE " . $newkw . " ) OR ( bnsprof_compname LIKE " . $bnewkw . ")) and pd_currency=cn_id " . $sql_pd_country . "  and pd_status='1'  and pd_image!=''" . $checkCountry . " and  bnsprof_id =b_id and " . $keywordmem . ") AND plan_member_id.expiry_date > " . time() . $keyword . " GROUP BY pd_id ORDER BY title_relevance DESC, FIELD(plan_member_id.p_id,'5','4','3','15','0'), pd_title asc limit 0,20";
					...
					( pd_uid in(select distinct usr_id from user where country='" . $_COOKIE['loc_id'] . "')))";
                        ...
                        $newkw = generateProdSearchString($keywords);
                        $bnewkw = generateProdSearchString_pro_sup($keywords);
                        $sql_prd = "select measurement_unit_arabyos.*,country.*," . $bus_col . "," . $prod_col . ", MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid=pd_uid and b_id = bnsprof_id  and mu_id=pd_unit   and ((pd_title LIKE " . $newkw . " ) OR ( bnsprof_compname LIKE " . $bnewkw . ")) and pd_currency=cn_id " . $sql_pd_country . " AND pd_status='1'" . $checkCountry . " AND pd_image!=''  " . $keyword . " AND plan_member_id.expiry_date > " . time() . " GROUP BY pd_id ORDER BY title_relevance DESC, FIELD(plan_member_id.p_id,'5','4','3','15','0'), pd_title asc limit 0,20";
                    ...
                    if ($_GET['keywords'] != "") {
                        ...
                        if (isset($_POST['country_id'])) {
                            ...
                            $newkw = generateProdSearchString($keywords);
                            $bnewkw = generateProdSearchString_pro_sup($keywords);
                            $sql_prd = "select measurement_unit_arabyos.*,country.*," . $bus_col . "," . $prod_col . "," . $usr_col . ", MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country,business_profile,user,plan_member_id where user.usr_id = bnsprof_uid and  bnsprof_uid=pd_uid and b_id = bnsprof_id  and mu_id=pd_unit and ((pd_title LIKE " . $newkw . " ) OR ( bnsprof_compname LIKE " . $bnewkw . ")) " . $cntryval . ") and pd_currency=cn_id and pd_status='1' and pd_image!=''" . $min_qty . " AND plan_member_id.expiry_date > " . time() . " GROUP BY pd_id ORDER BY title_relevance DESC, FIELD(plan_member_id.p_id,'5','4','3','15','0'), pd_title asc limit 0,20";

                            //echo $sql_prd.'________33';
                        } elseif (isset($_POST['state_id'])) {
                            $newkw = generateProdSearchString($keywords);
                            $bnewkw = generateProdSearchString_pro_sup($keywords);
                            $sql_prd = "select measurement_unit_arabyos.*,country.*," . $bus_col . "," . $prod_col . ", MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid=pd_uid and mu_id=pd_unit and b_id = bnsprof_id  and ((pd_title LIKE " . $newkw . " ) OR ( bnsprof_compname LIKE " . $bnewkw . ")) and pd_currency=cn_id " . $sql_pd_ck . " and pd_status='1' and pd_image!=''" . $min_qty . " and bnsprof_state IN (" . $stateid . ") AND plan_member_id.expiry_date > " . time() . " GROUP BY pd_id ORDER BY title_relevance DESC, FIELD(plan_member_id.p_id,'5','4','3','15','0'), pd_title asc limit 0,20";
                            ...
                            // $sql_prd = "select measurement_unit_arabyos.*,country.*," . $bus_col . "," . $prod_col . ", MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid = pd_uid and b_id = bnsprof_id and mu_id=pd_unit and ((pd_title LIKE " . $newkw . " ) OR ( bnsprof_compname LIKE " . $bnewkw . ")) and pd_currency=cn_id " . $sql_pd_ck . " and pd_status='1' and pd_image!=''" . $min_qty . " AND plan_member_id.expiry_date > " . time() . "   GROUP BY pd_id ORDER BY  FIELD(plan_member_id.p_id,'5','4','3','15'),title_relevance DESC, pd_title asc" . $setLimit; //OLD ---  ORDER BY FIELD(pd_title,'5','4','3','15')
                            $sql_prd = "select measurement_unit_arabyos.*,country.*," . $bus_col . "," . $prod_col . ", MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid = pd_uid and b_id = bnsprof_id and mu_id=pd_unit and ((pd_title LIKE " . $newkw . " ) OR ( bnsprof_compname LIKE " . $bnewkw . ")) and pd_currency=cn_id " . $sql_pd_ck . " and pd_status='1' and pd_image!=''" . $min_qty . " AND plan_member_id.expiry_date > " . time() . "   GROUP BY pd_id ORDER BY title_relevance DESC, FIELD(plan_member_id.p_id,'5','4','3','15','0'), pd_title asc" . $setLimit; //OLD ---  ORDER BY FIELD(pd_title,'5','4','3','15')
                        ...
                        if (isset($_POST['country_id'])) {
                            ...
                            $sql_prd = "select measurement_unit_arabyos.*,country.*," . $bus_col . "," . $prod_col . ", MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid=pd_uid and mu_id=pd_unit and b_id = bnsprof_id and pd_currency=cn_id " . $sql_pd_ck . " and pd_status='1' and pd_image!=''" . $min_qty . " and bnsprof_state IN (" . $stateid . ") AND plan_member_id.expiry_date > " . time() . " GROUP BY pd_idORDER BY title_relevance DESC, FIELD(plan_member_id.p_id,'5','4','3','15','0'), pd_id dessc limit 0,20";

                            //	 echo $sql_prd.'________77';
                        } else {
                            $sql_prd = "select measurement_unit_arabyos.*,country.*," . $prod_col . ", MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products,measurement_unit_arabyos,country where mu_id=pd_unit and pd_currency=cn_id " . $sql_pd_ck . " and pd_status='1' and pd_image!=''" . $min_qty . " GROUP BY pd_id order by title_relevance DESC, pd_title asc limit 0,20";
...
///---santosh number for produycts
	 ...
	 $keywords = $_GET['keywords'];
			...
			//total product//
			    ...
			    $sql_prd_total = "select *,  MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products as prod,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid = pd_uid and b_id = bnsprof_id and mu_id=pd_unit and ((pd_title LIKE " . $newkw . " ) OR (bnsprof_compname LIKE " . $bnewkw . ")) and pd_currency=cn_id " . $sql_pd_ck . " and pd_status='1' and pd_image!=''  AND plan_member_id.expiry_date > " . time() . " ORDER BY  title_relevance DESC, FIELD(p_id,'5','4','3','15','0'), pd_title asc ";
            /* $sql_prd="select * from products,measurement_unit_arabyos,country where mu_id=pd_unit and pd_title LIKE '%".urldecode($keywords)."%'  and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' and pd_image!='' order by pd_id desc".$gridsetLimit; */
            
/* 
 *   Currency-country fix by webxtor (here not tested)
 */
$sql_prd_total= preg_replace('#\bcountry,#msi', 'country c,', $sql_prd_total);
$sql_prd_total= preg_replace('#^(.+?\b)where(.+)$#msi', '$1, city, country c2 where c2.cn_id=pd_currency AND bnsprof_city=ct_id AND $2', $sql_prd_total);
$sql_prd_total= preg_replace('#pd_currency\s*=\s*cn_id#msi', 'cn_id=ct_cn_id', $sql_prd_total);
$sql_prd_total = preg_replace('#([^.a-z_-])cn_(id|code)#msi', '$1c.cn_$2', $sql_prd_total);
$sql_prd_total= preg_replace('#\bcountry\.#msi', 'c.', $sql_prd_total);	/**/
$sql_prd_total = str_replace("select c.cn_id from country where c.cn_code","select cn_id from country where cn_code",$sql_prd_total);
 $run_query_tot = mysql_query($sql_prd_total) or die(mysql_error());
$gettot_product = mysql_num_rows($run_query_tot);
	//total product//
	// echo '1234';exit;
	

if(strpos(" limit ",$sql_prd,0)) $sql_prd."00000";
if(!strpos(" limit ",$sql_prd)) $sql_prd."limit 0, 00000";

            $rss1_temp= mysql_query($sql_prd) or die(mysql_error());
			$number_of_products=mysql_num_rows($rss1_temp);

/* 
 *   Currency-country fix by webxtor
 */
                            ...
                            <div class="row">
                                <div class="col-xs-6 col-lg-3 big-img-box box-1" id="div-<?php echo $row['pd_id']; ?>">
                                    <header>
                                        <?php if($_GET['rctyp'] != 'Suppliers'){ //webcast ?>
                                        <?php if (isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != '') { ?>
                                            <a href="javascript:void(0)" class="product_fav_btn" data="<?php echo $row['pd_id']; ?>"  onclick="return showfavorite(<?php echo $_SESSION['uid_indm']; ?>,<?php echo $row['pd_id']; ?>)" class="ar-star"><i class="fa fa-star star" style="color:<?php echo (in_array($row['pd_id'], $myfev)) ? '#E48F23' : '#808080'; ?>"></i> Favorite</a>
                                        <?php } else {
                                            ?>
                                            <a href="sign-in.php" class="product_fav_btn" data="<?php echo $row['pd_id']; ?>" class="ar-star"><i class="fa fa-star star"></i> Favorite</a>
                                        <?php } ?>
                                        
                                        <a href="javascript:void(0)" class="ar-star product_compare" data-prod_img="<?php echo 'https://www.arabyos.com/upload/myproduct/' . $row['pd_image']; ?>" onClick="return addcompare(<?php echo $row['pd_id']; ?>)" data-prod_id="<?php echo $row['pd_id']; ?>" title="<?php echo $row['pd_title']; ?>" ><i class="fa fa-plus star" ></i> Add To Compare</a><?php } ?> </header>
                                    <figure class="box" >
                                        
                                        <?php if ($fevrow_icon) { 
                                            
                                            ?>
                                            <div class="ribbon">  <img src="./admin/images/<?php echo $fevrow_icon['sponsericon']; ?>"/> 
                                            </div>
                                        <?php } 
                                        elseif(mysql_num_rows($get_icon3)){
                                        $fevrow_icon3 = mysql_fetch_array($get_icon3, MYSQL_ASSOC);
                            ...
                            <!--<img src="images/3.png"  class="zoomthis" alt="Rice"/>-->
                                        ...
                                        <?php if (!empty($row['pd_imagelogo'])) { ?>
                                            <div class="zk" style=" border: 1px solid #267abf;height: auto; width: 100px;position: absolute;top: 172px; left: 5px;">
                                                <?php echo "<img style='width: auto; height: 100px;max-width:100%;' src='/upload/myproduct/" . $limg[0] . "'>"; ?></div>
                                        <?php } ?>             
                                    </figure>
                                    <center>
                                    <a onclick="zoom_image(this)" data-img="<?php echo $zoom_image_val;?>" style="padding: 10px;" ><i class="fa fa-search-plus"Title="  شاهد - الصورة - بحجم كبير "></i>  </a>
                                    </center>
                                </div>
                                <div class="col-xs-6 col-lg-5 box-2">
                                    <ul>
                                        <li class="margin-bottom-0">
                                            <h4 class="txt-blue"><a class="txt-blue" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/product-details.php?token=<?php echo rand(1000, 9999) . md5($row['pd_id']) ?>&c=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>" target="_blank"><?php //echo $row['pd_title'];
                                            echo highlight($row['pd_title'], urlencode($_GET['keywords'])); ?></a></h4>
                                        </li>
                                        <li>
                                    	...
                                    	<span style="float: left;"><strong>Brand:</strong> <?php echo $row['brand_name']; ?></span>
                                    	<?php } ?>
                                        <a href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/product-details.php?token=<?php echo rand(1000, 9999) . md5($row['pd_id']) ?>&c=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>" target="_blank">+  More</a> </li>
                                        ...
                                        $symbol;
                                        ?>
                                        <?php if ((int)$row['pd_fob_price'] == 0) {
                                    		$style_none = 'hide';
                                    	} 
                                        else {$style_none = 'show';}
                                        ?>   
                                        <li class="<?php echo $style_none; ?>">Price  &nbsp; <big class="txt-bold txt-red"><a style="color: black;" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/product-add.php"><?php echo $symbol . $row['pd_fob_price'] . '<span style="color:black"> ~ </span>' . $symbol . $row['pd_fob_price2']; ?> </a></big>
                                        <?php if (($_SESSION['uid_indm'] ) == '') { ?>
                                            <a style="float: right;position: relative;right: 20px;" class="hidden-xs" data-price="" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos.com/sign-in.php">(Get Letest Price)</a> 
                                            <?php
                                        } else {
                                            ...
                                            if ($_GET['keywords'] != "") {
                                                ?>
                                                <a style="float: right;position: relative;right: 20px;" class="ajax" data-enquiry="" id="btn_ajax_send<?php echo $row['pd_id']; ?>" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&keywords=<?php echo urlencode($_GET['keywords']); ?>&geo=<?php echo $geo_loc; ?>&conty=<?php echo $countryyyy; ?>&search=1">(Get Letest Price)</a>
                                                <?php
                                            }//keyword 
                                            else {
                                                ?>
                                        ...
                                        <?php if ($row['pd_pn_capct'] != '') {
                                        ?>
                                        <li><a class="" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/company/product-details.php?token=<?php echo rand(1000, 9999) . md5($row['pd_id']) ?>&c=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>" target="_blank">Logistics</a></li>
                                        <?php } ?>
                                        <li class="margin-top-5">
                                            <table class="table">
                                                <tr>
                                                    ...
                                                    $compnamers;
                                                    ?>
                                                    <td style="padding-left:0px;"><a href="/company/index.php?c=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>" target="_blank" class="txt-blue txt-bold hidden-xs"><img src="images/users.png" width="25px"/> About Us</a></td>
                                                    <td class="hidden-xs"><a href="/company/products.php?c=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&flaag=whsuccess" target="_blank" class="txt-blue  txt-bold hidden-xs"><img src="images/icon.png" width="20px"/> View Products</a></td>
                                                    <?php if ($row['pd_pdf_attach']) { ?>
                                                    <td><a href="//arabyos.com/upload/productdoc/<?php echo $row['pd_pdf_attach'];?>" target="_blank"><img src="/images/pdf_icon.png" style="width: 28px;height: 28px;"> PDF</a></td>
                                                    <?php } ?>
                                                    <td>
                                                        <a onclick="open_chat()" class="hidden-xs">
                                                          <i class="fa fa-comments"></i> Chat  
                                                        </a>
                                        ...
                                        <li>
                                            <table class="table enquiry-tb margin-bottom-0">
                                                <tr class="bg-gray">
                                                    ...
                                                    <?php $Countryphone = mysql_fetch_array(mysql_query("SELECT * FROM `country` where cn_id = " . $data['country'])); ?>
                                                    <td class="padding-0 col-sm-6" style="vertical-align: middle;"><big class=""><img src="images/mobile.png" width="25px"/> &nbsp;<a href="tel:<?php echo '+'.user_info($data['bnsprof_uid'],'country_ph_code');  ?><?php echo user_info($data['bnsprof_uid'], 'mobile1'); //echo $data['bnsprof_mobile2']; //echo user_info($data['bnsprof_uid'],'mobile1');  ?>" class="txt-black txt-lg" style="vertical-align: middle;"><b><?php echo $Countryphone['cn_ph']; //echo user_info($data['bnsprof_uid'],'country_ph_code');  ?>-<?php echo user_info($data['bnsprof_uid'], 'mobile1'); //echo $data['bnsprof_mobile2']; //echo user_info($data['bnsprof_uid'],'mobile1');  ?> </b></a></big></td>
                                                <td class="text-right padding-0 col-sm-6">
                                                    <?php if (($_SESSION['uid_indm'] ) == '') { ?>
                                                        <a data-enquiry="" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/sign-in.php"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry" style="font-weight:bold;">Send Enquiry</button></a> 
                                                        <?php
                                ...
                                <div class="col-lg-4 box-3">
                                    <div class="ar-box-1 ar-box padding-5 margin-bottom-5 bg-gray" style="overflow-x: hidden;">
                                        <header class="sub-box"><?php if ($fevrow_icon) { ?><a href="https://www.arabyos.com/membership_plans.php"><img src="./admin/images/<?php echo $fevrow_icon['producticon']; ?>" width="25px" height="25px" title="<?php echo $fevrow_icon['pplan']; ?>"/></a> <?php }
                                        elseif(mysql_num_rows($get_icon1)){ ?><a href="https://www.arabyos.com/membership_plans.php"><img src="./admin/images/<?php echo $icon1['mst_icon']; ?>" width="25px" height="25px" title="<?php echo $icon1['mst_name']; ?>"/></a><?php } ?><b class="txt-dark-gray"> 
                                        ...
                                        <table class="table margin-top-5">
                                            ...
                                            <tr>
                                                <td class="txt-light-gray padding-0"> <a style="font-size:12px;font-weight:100;color:#8a8a8a" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos.com/product-add.php" target="_blank">Trade Location :</a> </td>
                                                <td class="padding-0 txt-bold"> <a style="font-size:12px;color:#242424" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos.com/product-add.php"><?php
                                            ...
                                            <tr>
                                                <td class="txt-light-gray " colspan="2" style="padding-left: 0;"><a href="<?php echo $data['bnsprof_website_alt']; ?>" target="_blank"><?php echo $data['bnsprof_website_alt']; ?></a></td>
                                                <td class="padding-0"></td>
                                            </tr>
                                        </table>
                                    </div>
...
<a class="thumb-images" href="search.php?keyword_type=' . $_GET['keyword_type'] . '&keywords=' . $rrow['pd_title'] . '&rctyp=Products"><img data-toggle="tooltip" data-placement="top" title="' . $rrow['pd_title'] . '" class="photo" src="./upload/myproduct/' . $imgarr[0] . '"  data-large_photo="upload/myproduct/' . $imgarr[0] . '"/></a>';
             ...
             $releted_pro .= '<a class="inner-search-right-img" href="search.php?keyword_type=' . $_GET['keyword_type'] . '&keywords=' . $rrow['pd_title'] . '&rctyp=Products" style="width: auto;height: auto;"><img data-toggle="tooltip" data-placement="top" title="' . $rrow['pd_title'] . '" style="width: 35px;max-width: 100%;" class="photo" src="./upload/myproduct/' . $logoarr[0] . '" data-large_photo="upload/myproduct/' . $logoarr[0] . '"/></a>';
                                                        }
    ...
    <a class="thumb-images" href="search.php?keyword_type=' . $_GET['keyword_type'] . '&keywords=' . $rrow['pd_title'] . '&rctyp=Products"><img data-toggle="tooltip" data-placement="top" title="' . $rrow['pd_title'] . '" class="photo" src="./upload/myproduct/' . $imgarr[0] . '"  data-large_photo="upload/myproduct/' . $imgarr[0] . '"/></a>';
           ...
           $releted_pro .= '<a class="inner-search-right-img" href="search.php?keyword_type=' . $_GET['keyword_type'] . '&keywords=' . $rrow['pd_title'] . '&rctyp=Products" style="width: auto;height: auto;"><img data-toggle="tooltip" data-placement="top" title="' . $rrow['pd_title'] . '" style="width: 35px;max-width: 100%;" class="photo" src="./upload/myproduct/' . $logoarr[0] . '" data-large_photo="upload/myproduct/' . $logoarr[0] . '"/></a>';
                    ...
                    <?php
														...
														//print("<BR> 1 ha:".$number_of_products."---".$getSearchCount);
                    ...
                    if ($getSearchCount > 4) {
                        ...
                        if ($countRec == $getSearchCount) {
                            ...
                            if ($_GET['page'] > 1 || $_GET['page'] == 1) {
                                $pages = $_GET['page'] + 1;
								$gettot_product1=($gettot_product-( $_GET['page']*30));
                            } else {
								$gettot_product1=$gettot_product;
                                $pages = 2;
                            }
							
						//	echo "bbb".$gettot_product;
						//	echo "aaaa".$gettot_product1;
  ...
  $gettot_product = mysql_num_rows($run_query_tot);
	//total product//
            ...
            $bnewkw = generateProdSearchString_pro_sup($keywords);
             ...
             $sql_prd = "select *,  MATCH (pd_title) AGAINST ('" . $keywords . "'  IN BOOLEAN MODE) AS title_relevance  from products as prod,measurement_unit_arabyos,country,business_profile,plan_member_id where bnsprof_uid = pd_uid and b_id = bnsprof_id and mu_id=pd_unit and ((pd_title LIKE " . $newkw . ") OR (bnsprof_compname LIKE " . $bnewkw . ")) and pd_currency=cn_id " . $sql_pd_ck . " and pd_status='1' and pd_image!=''  AND plan_member_id.expiry_date > " . time() . " ORDER BY  title_relevance DESC, FIELD(p_id,'5','4','3','15','0'), pd_title asc ";
            /* $sql_prd="select * from products,measurement_unit_arabyos,country where mu_id=pd_unit and pd_title LIKE '%".urldecode($keywords)."%'  and pd_currency=cn_id ".$sql_pd_ck." and pd_status='1' and pd_image!='' order by pd_id desc".$gridsetLimit; */
...
$get_icon3 = mysql_query($sql_icon3) or die(mysql_error());
                    ...
                    <div class="col-md-4 compared-box compared-box1 style_prevu_kit">
                        ...
                        <header style="padding: 15px 0;width: 100% !important;" class="titleLim box-2">
                            ...
                            <span class="txt-blue">
                                <a href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/<?php echo 'company'; ?>/product-details.php?token=<?php echo rand(1000, 9999) . md5($row['pd_id']) ?>&c=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>" target="_blank" class="h4" style="font-weight:bold;"><?php //echo $row['pd_title'];
                                echo highlight($row['pd_title'], urlencode($_GET['keywords'])); ?></a>
                            </span>
                            ...
                            <div class="ara-links">
                                <?php if (isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != '') { ?>            
                                    <a href="javascript:void(0)" class="product_fav_btn" data="<?php echo $row['pd_id']; ?>"  onclick=" showfavorite(<?php echo $_SESSION['uid_indm']; ?>,<?php echo $row['pd_id']; ?>);
                                            addfavorite(<?php echo $row['pd_id']; ?>)" data-prod_id="<?php echo $row['pd_id']; ?>" title="<?php echo $row['pd_title']; ?>"><i class="fa fa-star" aria-hidden="true"></i> Favorite</a>
                                   <?php } else { ?>
                                    <a  href="sign-in.php"  class="product_fav_btn" title="<?php echo $row['pd_title']; ?>"><i class="fa fa-star" aria-hidden="true"></i> Favorite</a>
...
<?php echo "<img src='/upload/myproduct/" . $pimg2[0] . "' class='zoomthis' style='' alt='/upload/myproduct/" . $pimg2[0] . "'>"; ?> <!--<img src="images/3.png" class="zoomthis" alt="Rice"/>--> </a></div>
                                <?php
                                ...
                                <div class="zk" style=" border: 1px solid #267abf;height: auto; width:100px;position: absolute;top: 121px;left: 1px;">
                                    <?php echo "<img style='width: auto; height: 50px;max-width:100%;' src='/upload/myproduct/" . $limg2[0] . "'>"; ?></div>
                            <?php } ?>   
                        </figure>
                        <section>
                            ...
                            <table style="text-align: left;">
                                <?php if ($data['bnsprof_compname'] != "") { ?>
                                    <tr>
                                        <td><?php if ($fevrow_icon) { ?> <a href="https://www.arabyos.com/membership_plans.php"><img src="https://www.arabyos.com/admin/images/<?php echo $fevrow_icon['producticon']; ?>" title="<?php echo $fevrow_icon['pplan']; ?>" width="25px" height="25px"/></a><?php }
                                        elseif(mysql_num_rows($get_icon1)){ ?><a href="https://www.arabyos.com/membership_plans.php"><img src="./admin/images/<?php echo $icon1['mst_icon']; ?>" width="25px" height="25px" title="<?php echo $icon1['mst_name']; ?>"/></a><?php } ?></td>
                                ...
                                <tr>
                                    <td><a href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos.com/product-add.php"><!--Trade Location:--></a> </td>
                                    <td colspan="2"></td>
                                ...
                                <tr>
                                    <td class="txt-light-gray " colspan="2" style="display: none;"><a href="<?php echo $data['bnsprof_website_alt']; ?>"><?php echo $data['bnsprof_website_alt']; ?></a></td>
                                    <td class="padding-0"></td>
                                </tr>
                            </tbody>
                        </table>
                        <table class="webcast-new-table">
                            <tbody>
                                ...
                                <?php echo measurement_unit_arabyos($row['pd_unit']); ?> (Min Order)</td>
                                </tr>
                                <tr>
                                    <td><!--Fob Price:-->  </td>
                                    <td>
                                        ...
                                        $symbol;
                                        ?>
                                <big class="txt-bold txt-red"><a style="color: #d22027" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos.com/product-add.php"> <?php echo $symbol . $row['pd_fob_price'] . '~ ' . $symbol . $row['pd_fob_price2']; ?></a></big>
                                ...
                                <tr>
                                    ...
                                    <?php $Countryphone = mysql_fetch_array(mysql_query("SELECT * FROM `country` where cn_id = " . $data['country'])); ?>
                                    <td colspan="2" class="grid_mobile"><img src="images/mobile.png" width="25px"/> &nbsp;<a href="#" class="txt-black h4"><b>
                                    <?php 
                                    /*if (strpos(user_info($data['bnsprof_uid'], 'country_ph_code'), '+') !== false) {
									    echo user_info($data['bnsprof_uid'], 'country_ph_code');
									}
									else {
                                    echo '+'.user_info($data['bnsprof_uid'], 'country_ph_code'); }?>-<?php echo user_info($data['bnsprof_uid'], 'mobile1');*/ 
                                    //$data['bnsprof_mobile2'];
                                    echo $Countryphone['cn_ph'];   ?>-<?php echo user_info($data['bnsprof_uid'], 'mobile1'); ?> </b></a></td>
                                </tr>
                                <tr>
                                    <td></td>
					...
					//print("<BR> 2 ha:".$gridRecCount."---".$totalgridpage);
                    if ($gridRecCount == $totalgridpage) {
                        ...
                        echo '<div class="col-lg-12 text-center" style="padding:30px;"><a href="https://arabyos.com/search.php?keywords=' . $_GET['keywords'] . '&grid=' . $_GET['grid'] . '&page=' . $pages . '"><button type="button" class="btn btn-md btn-warning border-radius-0 btn-enquiry" style="font-size:16px; font-weight:bolder;">Display More Products / Services </button></a></div>';
                    }
                    $gridRecCount++;
                }
                ?>
...
if ($chrome) {
    if ($_SESSION['uid_indm'] != '') {
        $style = 'style="left:-144px;z-index: 99999;"';
    } else {
