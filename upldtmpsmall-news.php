<?php
/**
 * File: upldtmpsmall-news.php

 * Description: رفع صورة مصغرة مؤقتة للأخبار مع تغيير الحجم
...
// إعدادات رفع الملفات
$targetFolder = __DIR__ . '/upload/mynews/small/';
...
try {
    ...
    $check_sql = "SELECT COUNT(*) as count FROM temp_newsimage WHERE tmpns_uid = ? AND tmpns_status = '1'";
    ...
    if ($image_count >= 1) {
        throw new RuntimeException("You can only upload one small image");
    }
    ...
    $insert_sql = "INSERT INTO temp_newsimage (tmpns_uid, tmpns_image, tmpns_status) VALUES (?, ?, '1')";
    ...
    echo "1|Small image uploaded successfully|" . $fileName;

} catch (RuntimeException $e) {
    error_log("Upload Temp News Small Image Error: " . $e->getMessage() . " | User ID: $uid");
    http_response_code(400);
    echo "0|" . $e->getMessage();
} catch (Exception $e) {
    error_log("Unexpected Upload Temp News Small Image Error: " . $e->getMessage() . " | User ID: $uid");
