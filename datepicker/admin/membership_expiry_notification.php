...
$data = mysql_query("select pm.*, u.*, sp.* from plan_member_id pm JOIN business_profile bf ON pm.b_id = bf.bnsprof_id JOIN user u ON u.usr_id = bf.bnsprof_uid JOIN smembership_plan sp ON sp.mp_id = pm.p_id Where notification_email_status =0 AND expiry_date = '".strtotime(date("Y-m-d"))."'") or die(mysql_error());
 while($row = mysql_fetch_object($data)) {
   ...
   $billing_detail = mysql_fetch_object($billing_query);
	...
	$subject = "Your ".$plan_name." plan is ended today on ".get_page_settings(4);	
...
$from_name = get_page_settings(4);
    ...
    mysql_query("UPDATE plan_member_id SET notification_email_status = 1 WHERE b_id =".$row->b_id);
 $sql = "UPDATE user SET usr_mp_id = '0', user_type = '1' WHERE usr_id =".$row->usr_id;
   mysql_query($sql) or die(mysql_error()); 

$message = 'Your '.$plan_name. 'plan is ended on '.$expiry_date.'. Please renew your membership.';
