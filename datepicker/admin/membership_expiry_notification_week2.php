...
$data = mysql_query("select pm.*, u.*, sp.* from plan_member_id pm JOIN business_profile bf ON pm.b_id = bf.bnsprof_id JOIN user u ON u.usr_id = bf.bnsprof_uid JOIN smembership_plan sp ON sp.mp_id = pm.p_id Where notification_email_week2_status =0") or die(mysql_error());
 while($row = mysql_fetch_object($data)) {
	  if(date('Y-m-d', strtotime("+22 days"))==date('Y-m-d',$row->expiry_date)){
	...
	$from_name = get_page_settings(4);
		...
		mysql_query("UPDATE plan_member_id SET notification_email_week2_status = 1 WHERE b_id =".$row->b_id);
