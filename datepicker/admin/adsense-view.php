...
class listMessage{
	...
	function deleterecord($adid){
		...
		mysqli_query($con, "delete from google_adsense where ga_id='".$adid."'");
	}
...
/********************delete record*********************/
	if($_GET['action']=="del"){
		...
		//header("location:adsense-view.php?".rtrim($_SERVER['QUERY_STRING'],"&action=del&id=".$_GET['id']));
		}
/***********************************************/
...
{ 
	...
	{		
		mysqli_query($con, "delete from google_adsense where ga_id='".$cb."'");		
	}
	header("Location:adsense-view.php");
}
?>
...
<button type="button" class="btn btn-xs btn-success" onClick="window.location='adsense-add.php' "><i class="icon-plus-sign"></i> Add Google Adsense</button>
</div>
...
$count=mysqli_num_rows($recObj);
	if($count >0)
	{
        ...
        <tr <?php if($j % 2 == 1) { ?> class="row-clr" <?php } ?> >
	        <td class="center"><label><input name="cb[]"  class="ace" type="checkbox" value="<?php echo $row->ga_id; ?>" /><span class="lbl"></span></label></td>
            <td><?php echo $row->ga_content; ?></td>
            <td style="text-align:center">
            <a href="adsense-edit.php?fid=<?php echo $row->ga_id ?>" title="Edit"><img alt="edit" src="images/edit.jpg" border="0"></a>
            <a href="<?php echo $al->deletelink($row->ga_id)?>" title="Delete" onclick="return confirm('Are you sure to delete the Adsense?')"><img alt="Delete" src="images/delete.jpg" border="0"></a>
            </td>
        </tr>
        <?php $j++; } }?>
</tbody>
