...
// initilize all variable
	...
	while( $row = mysqli_fetch_object($queryRecords) ) {			
		...
		if($row->br_pic !=''){
			...
			$res[1] = $img;
			$res[2] = $row->br_pd_name;
			$res[3] = ucwords(stripslashes($row->br_requirement));
			$res[4] = ucwords(stripslashes($row->bnsprof_compname));
			$res[5] = $row->cn_name;
			$res[6] = $row->mst_name;
			$res[7] = date_format(date_create($row->br_posting_date),"jS F Y");
			$res[8] ='';
			$res[9] = '';
