...
if(isset($_SESSION['mp_credits'])){	$mp_credits=$_SESSION['mp_credits'];	unset($_SESSION['mp_credits']);	}else{	$mp_credits="";	}
if(isset($_SESSION['mp_amount'])){	$mp_amount=$_SESSION['mp_amount'];	unset($_SESSION['mp_amount']);	}else{	$mp_amount="";	}
...
class addPlan{
	...
	function __construct($mp_name, $mp_credits, $mp_amount)
	{
	...
	function valid()
	{
		...
		else if($this->mp_credits == "" || $this->mp_credits == " " || $this->mp_credits == "0")
		{
			$this->msg= '<div class="alert alert-danger"><i class="icon-remove"></i> Please enter number of Credits</div>';
			$valid=false;
		}
		else if($this->mp_credits%20 != 0)
		{
			$this->msg= '<div class="alert alert-danger"><i class="icon-remove"></i> Credits must be in multiple of 20</div>';
			$valid=false;
		}
		else if($this->mp_amount == "" || $this->mp_amount == " ")
		{
	   ...
	   global $con;
		$sql="insert into membership_plan 
			set					
				...
				mp_credits ='".$this->mp_credits."',
				mp_amount ='".$this->mp_amount."',
				mp_updated_date=now()";							
...
if(isset($_POST['btnAdd']))
{
	...
	$adn=new addPlan( addslashes(trim($_POST['mp_name'])), addslashes(trim($_POST['mp_credits'])), addslashes(trim($_POST['mp_amount'])));
...
{	
	...
	if(mp_name.value=='' || mp_name.value == null)
	{
		message='Please enter Membership Plan Name';
		mp_name.focus();
		valid=false;
	}
	else if(mp_credits.value=='' || mp_credits.value == null || mp_credits.value == ' ')
	{
	...
	else if(mp_credits.value%20 != 0)
	{
		message='Credits must be in multiple of 20';
		mp_credits.value='';
		mp_credits.focus();
		valid=false;
	}
	else if(mp_amount.value=='' || mp_amount.value == null || mp_amount.value == ' ')
	{
...
<div class="form-group">
		<label class="col-sm-3 control-label no-padding-right" for="form-field-2">Credits <span style="color:#CC0000">*</span></label>
		<div class="col-sm-9">
			<input name="mp_credits" id="mp_credits" class="col-xs-10 col-sm-5" type="text" value="<?php echo $mp_credits; ?>" />
		</div>
	</div>
    
    <div class="form-group">
		<label class="col-sm-3 control-label no-padding-right" for="form-field-2">Plan Rate <span style="color:#CC0000">*</span></label>
		<div class="col-sm-9">
			<input name="mp_amount" id="mp_amount" class="col-xs-10 col-sm-5" type="text" value="<?php echo $mp_amount; ?>" />
		</div>
	</div>
