...
class sliderviewlist{
	...
	function deleterecord($adid){
		global $con;
	    $chquesql="select * from video_slider where adv_id ='".$adid."' ";
		$chqueres=mysqli_query($con, $chquesql);
		$chquerow=mysqli_fetch_array( $chqueres);
		$path="../upload/video_slider/".$chquerow['adv_img'] ;
		unlink($path);
		$sql_del="delete from video_slider where adv_id='".$adid."'";
	...
	function deletelink($id){
		if($_SERVER['QUERY_STRING']==""){
			...
			$dellink="videoslider-view.php?".$_SERVER['QUERY_STRING']."&action=del&fid=".$id;
		}
		return $dellink;
	}
}
...
/********************delete record*********************/
	if(isset($_GET['action']) && $_GET['action']=="del"){
		...
		//header("location:videoslider-view.php?".rtrim($_SERVER['QUERY_STRING'],"&action=del&id=".$_GET['id']));
		}
/***********************************************/
...
$adjacents=1;
$targetpage = "videoslider-view.php";
	...
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb">
			<li>
				...
				<li>
					<a href="videoslider-view.php">Manage Slider</a>
				</li>
...
<button class="btn btn-xs btn-danger" name="btnDelete" type="submit" onClick="return confirm('Are you sure to delete the record?')" ><i class="icon-trash bigger-120"></i>Delete</button>
 <button type="button" class="btn btn-xs btn-success" onClick="window.location='videoslider-add.php' "><i class="icon-pencil align-top bigger-120"></i>Add Slider</button>
 </div>
...
$count=mysqli_num_rows($recObj);
	if($count >0)
	{
        ...
        <tr>
			...
			<td style="text-align:center;"><?php echo "<strong>Video Url :</strong>".$row->adv_link."<br/>"."<strong>Redirect Url :</strong>".$row->adv_redirect;?></td>
			<td style="text-align:center"><?php echo $row->adv_title;?></td>
          ...
          <script>
            ...
            <td align="center">
     <a href="videoslider-edit.php?aid=<?php echo $row->adv_id; ?>" title="edit"><img alt="edit" src="images/edit.jpg" border="0"></a>
