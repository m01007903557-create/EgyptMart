<?php
/**
 * File: sendLeadEnquiry-form.php
 * Description: نموذج إرسال استفسار عن طلب شراء (Buy Lead)
...
// التحقق من وجود معرف المستخدم والعنوان
if (!isset($_GET['id']) || empty($_GET['id']) || !isset($_GET['headline'])) {
    header('Location: index.php');
    exit;
}

$usr_hash = substr($_GET['id'], 4); // إزالة أول 4 أحرف
$lead_headline = trim($_GET['headline']);
$current_user = (int)$_SESSION['uid_indm'];

global $con;

// جلب بيانات المستخدم المستهدف (صاحب طلب الشراء)
...
$subject_default = "الإستفسار من : " . $sender_name;

$lead_headline_escaped = htmlspecialchars($lead_headline, ENT_QUOTES, 'UTF-8');
?>
...
function sendEnquiry() {
    ...
    if (msg_message.value == '' || msg_message.value == null) {
        ...
        alert("Enquiry must be at least 50 characters length.");
        msg_message.focus();
        valid = false;
    }
...
<div class="ov-base">
    <div class="neff2-nw">
        <p style="width:635px; text-align:center;">: إرسل إستفسار الى <span class="co-name"></span></p>
    </div>
    
    <div class="bo k9 err-msg" id="errmsg" style="display:none;"></div>
    
    <form name="dataform" style="margin:0px; padding:0px;" method="post">
        <div id="lead_headline" style="display:none;"><?php echo $lead_headline_escaped; ?></div>
        ...
        <!-- عرض بيانات المستلم (صاحب طلب الشراء) -->
        <div>
            <div class="w12" style="font-size:14px; padding:5px; border-bottom:1px solid #6500CA; margin:5px; width:658px; color:rgb(15,84,135); background-color:rgb(241,241,241); float:center; font-weight:700" align="center">
                <b>بيانات الراسل</b>
            </div>
        ...
        <div>
            <div class="w12" style="font-size:14px; padding:5px; border-bottom:1px solid #6500CA; margin:5px; width:658px; color:rgb(15,84,135); background-color:rgb(241,241,241); float:left; font-weight:700" align="LEFT">
                <b>بيانات الإتصال :</b>
            </div>
            <div class="text" style="padding-top:5px; border:4px double #6500CA; padding-bottom:10px; padding-left:10px; background-color:rgb(241,241,241);" align="LEFT">
                <div style="clear:both"></div>
                <div id="yourcontactinfo">
                    <div class="text" style="padding-top:5px;" align="LEFT">
                        ...
                        <?php if ($sender_country > 0): ?>
                            <?php echo htmlspecialchars(get_country_name($sender_country), ENT_QUOTES, 'UTF-8'); ?><br>
                        <?php endif; ?>
