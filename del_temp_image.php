<?php
/**
 * File: del_temp_image.php

 * Version: PHP 8.3
 * Description: حذف صورة مؤقتة من صفحة "من نحن"
 * 
 * هذا الملف يقوم بحذف صورة مؤقتة من مجلد upload/myprofile
 * ومن قاعدة البيانات temp_about_us
 */
...
$select_sql = "SELECT * FROM temp_about_us WHERE tmabs_id = {$image_id} LIMIT 1";
...
if (!empty($rowk->tmabs_images)) {
    $image_path = "upload/myprofile/" . $rowk->tmabs_images;
    
    if (file_exists($image_path)) {
        if (!unlink($image_path)) {
            // فشل في حذف الملف - لكن نستمر في حذف سجل قاعدة البيانات
            error_log("فشل في حذف الملف: " . $image_path);
        }
    } else {
        error_log("الملف غير موجود: " . $image_path);
    }
}

// حذف السجل من قاعدة البيانات
$delete_sql = "DELETE FROM temp_about_us WHERE tmabs_id = {$image_id}";
...
if ($delete_result) {
    ...
    if (mysqli_affected_rows($con) > 0) {
        echo json_encode([
            'success' => true,
            'message' => 'تم حذف الصورة المؤقتة بنجاح',
            'action' => 'deleted',
