...
// تم تعديله ليعمل مع rfq_id مع الحفاظ على الهيكل الأصلي
// ============================================================
...
// ============================================================
// الحصول على rfq_id أو chat_code من الرابط
// ============================================================
...
if (empty($chat_code) && $rfq_id > 0) {
    ...
    if ($chat_check && mysqli_num_rows($chat_check) > 0) {
        ...
        // إنشاء غرفة محادثة جديدة إذا لم تكن موجودة
        $offer_check = mysqli_query($con, "SELECT supplier_id, buyer_id FROM offers WHERE rfq_id = $rfq_id AND price > 0 LIMIT 1");
        if ($offer_check && mysqli_num_rows($offer_check) > 0) {
            ...
            die('لا يوجد عرض سعر لهذا الطلب');
        }
    }
}
