...
// ============================================
// جلب rfq_id و chat_code من الرابط
// ============================================
...
if (!$chat_check || mysqli_num_rows($chat_check) == 0) {
    ...
    if ($offer_check && mysqli_num_rows($offer_check) > 0) {
        ...
        $new_chat_code = 'CHAT_' . time() . '_' . $rfq_id;
        $insert_chat = "INSERT INTO chat_rooms (rfq_id, supplier_id, buyer_id, chat_code, created_at, status) 
                        VALUES ($rfq_id, $supplier_id, $buyer_id, '$new_chat_code', NOW(), 'active')";
        mysqli_query($con, $insert_chat);
        
        // تحديث $chat_code للمتابعة
        $chat_code = $new_chat_code;
    } else {
        die("لا يمكن إنشاء غرفة محادثة: لا يوجد عرض سعر لهذا الطلب");
    }
} else {
...
// التحقق من وجود chat_code أو rfq_id
if (empty($chat_code) && $rfq_id == 0) {
    die("خطأ: لم يتم توفير كود الشات أو رقم الطلب في الرابط");
}

// إذا كان هناك rfq_id وليس chat_code، جلب chat_code من قاعدة البيانات
if (empty($chat_code) && $rfq_id > 0) {
    $chat_check = mysqli_query($con, "SELECT chat_code FROM chat_rooms WHERE rfq_id = $rfq_id LIMIT 1");
    if ($chat_check && mysqli_num_rows($chat_check) > 0) {
        $chat_data = mysqli_fetch_assoc($chat_check);
...
// إذا لم يتم العثور على chat_code، أظهر خطأ
if (empty($chat_code)) {
...
// ============================================================
include_once __DIR__ . '/../includes/header_new.php';
include_once __DIR__ . '/../includes/header_menu.php';
?>
