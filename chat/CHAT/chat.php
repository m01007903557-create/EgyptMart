<?php
/**
 * chat/chat.php - صفحة محادثة Two-Sided Chat (نهائي)
 */
    ...
    <div class="chat-messages" id="messages">
        ...
        <?php else: ?>
            <?php foreach ($messages as $msg): 
                // ✅ تحديد نوع الرسالة بناءً على sender_id الحقيقي
                ...
                if ($isOutgoing) {
                    ...
                    if ((int)$msg['sender_id'] === $supplier_id) {
                        $senderName = $chat['supplier_fname'] . ' ' . $chat['supplier_lname'];
                    } elseif ((int)$msg['sender_id'] === $buyer_id) {
                        $senderName = $chat['buyer_fname'] . ' ' . $chat['buyer_lname'];
                    } else {
                        $senderName = $msg['sender_type'] === 'supplier' ? $chat['supplier_fname'] : $chat['buyer_fname'];
                    }
                }
            ?>
...
// ============================================================
// عرض الرسائل مع التنسيق الصحيح (Two-Sided)
// ============================================================
function displayMessages(messages) {
    ...
    for (var i = 0; i < messages.length; i++) {
        ...
        // ✅ تحديد الاتجاه بناءً على sender_id (وليس sender_type)
        ...
        if (isOutgoing) {
            ...
            if (parseInt(msg.sender_id) === supplierId) {
                senderName = '<?php echo addslashes($chat['supplier_fname'] . ' ' . $chat['supplier_lname']); ?>';
            } else if (parseInt(msg.sender_id) === buyerId) {
                senderName = '<?php echo addslashes($chat['buyer_fname'] . ' ' . $chat['buyer_lname']); ?>';
            } else {
                senderName = msg.sender_type === 'supplier' ? '<?php echo addslashes($chat['supplier_fname']); ?>' : '<?php echo addslashes($chat['buyer_fname']); ?>';
            }
        }
...
// ============================================================
// إضافة رسالة جديدة فوراً (بدون Refresh)
// ============================================================
function addNewMessage(msg, senderId, senderType, time) {
    var box = document.getElementById('messages');
    ...
    if (isOutgoing) {
        ...
        if (parseInt(senderId) === supplierId) {
            senderName = '<?php echo addslashes($chat['supplier_fname'] . ' ' . $chat['supplier_lname']); ?>';
        } else if (parseInt(senderId) === buyerId) {
            senderName = '<?php echo addslashes($chat['buyer_fname'] . ' ' . $chat['buyer_lname']); ?>';
        } else {
            senderName = senderType === 'supplier' ? '<?php echo addslashes($chat['supplier_fname']); ?>' : '<?php echo addslashes($chat['buyer_fname']); ?>';
        }
    }
...
function sendMessage() {
    ...
    .then(data => {
        if (data.success) {
            ...
            // ✅ عرض الرسالة فوراً مع التنسيق الصحيح
            var now = new Date();
            var time = now.toLocaleTimeString('ar-EG', {hour: '2-digit', minute: '2-digit'});
            addNewMessage(msg, currentUserId, senderType, time);
...
// ============================================================
// تحميل الرسائل من الخادم (مرة واحدة عند فتح الصفحة)
// ============================================================
...
// ============================================================
// تهيئة الصفحة - تحميل الرسائل مرة واحدة فقط
// ============================================================
document.addEventListener('DOMContentLoaded', function() {
    loadMessages();
    // ❌ تم إزالة setInterval لضمان عدم إعادة تحميل الرسائل
});
</script>
