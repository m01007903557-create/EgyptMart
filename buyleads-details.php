...
<?php
$br_pic_raw = trim((string)($row->br_pic ?? ''));
$br_pic_clean = ltrim($br_pic_raw, '/');
$br_pic_base = basename($br_pic_clean);
$br_thumb_src = 'upload/buy_requirement/no-image.png';
$br_zoom_src = 'upload/buy_requirement/no-image.png';
if ($br_pic_clean !== '') {
    $buy_path = 'upload/buy_requirement/' . $br_pic_base;
    $buy_thumb_path = 'upload/buy_requirement/thumb/' . $br_pic_base;
    $gallery_path = strpos($br_pic_clean, '/') !== false ? $br_pic_clean : 'upload/image_gallery/' . $br_pic_base;
    if (is_file(__DIR__ . '/' . $buy_thumb_path)) {
        $br_thumb_src = $buy_thumb_path;
    } elseif (is_file(__DIR__ . '/' . $buy_path)) {
        $br_thumb_src = $buy_path;
    } elseif (is_file(__DIR__ . '/' . $gallery_path)) {
        $br_thumb_src = $gallery_path;
    }
    if (is_file(__DIR__ . '/' . $buy_path)) {
        $br_zoom_src = $buy_path;
    } elseif (is_file(__DIR__ . '/' . $gallery_path)) {
        $br_zoom_src = $gallery_path;
    } else {
        $br_zoom_src = $br_thumb_src;
    }
}
?>
...
<div>
<img src="<?php echo htmlspecialchars($br_thumb_src, ENT_QUOTES, 'UTF-8'); ?>" id="zoomImage<?php echo $row->br_id; ?>" border="0" height="100" width="125" data-zoomsrc="<?php echo htmlspecialchars($br_zoom_src, ENT_QUOTES, 'UTF-8'); ?>"/>
</div>
<span class="c12 bo fs">تفاصيل طلب الشراء:</span>
<div class="bdt" id="hdiv1">
    <div class="g2 fs k7">
        <div><?php echo nl2br(htmlspecialchars(strip_tags(preg_replace('/<style\b[^>]*>.*?<\/style>/is', '', stripslashes($row->br_requirement))))); ?><br>
        ...
        <?php if($row->br_description != ''){ ?>التطبيق/الاستخدام: <?php echo nl2br(htmlspecialchars(strip_tags(preg_replace('/<style\b[^>]*>.*?<\/style>/is', '', stripslashes($row->br_description))))); ?><br><?php } ?>
