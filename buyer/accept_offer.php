...
if ($action == 'accept') {
    // قبول العرض
    mysqli_query($con, "UPDATE offers SET status = 'accepted', updated_at = NOW() WHERE id = $offer_id");
    mysqli_query($con, "UPDATE offers SET status = 'rejected' WHERE rfq_id = $rfq_id AND id != $offer_id");
    mysqli_query($con, "UPDATE chat_rooms SET status = 'active', expiry_date = DATE_ADD(NOW(), INTERVAL 7 DAY), accepted_at = NOW(), accepted_offer_id = $offer_id WHERE rfq_id = $rfq_id");
    
    // إشعار للمورد
    $msg = "تم قبول عرض سعرك من قبل المشتري. يمكنك الآن التواصل معه مباشرة.";
    ...
    // رفض العرض
    mysqli_query($con, "UPDATE offers SET status = 'rejected', updated_at = NOW() WHERE id = $offer_id");
