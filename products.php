...
<div id="waModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:9999;">
    <div style="background:#fff; width:400px; max-width:90%; margin:100px auto; padding:25px; border-radius:10px; direction:rtl;">
        <span onclick="document.getElementById('waModal').style.display='none'" style="float:left; cursor:pointer; font-size:20px;">&times;</span>
...
function openWaRfq(pid, pname) {
    alert("تم استدعاء الدالة - ID: " + pid);
    if (!pid || pid == 0) {
    ...
    if (modal) {
        ...
        alert('خطأ: لم يتم العثور على نافذة البوب اب');
    }
}
...
// إنشاء الـ Popup إذا لم يكن موجوداً
if (!document.getElementById('waModal')) {
    var modalHtml = '<div id="waModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:9999;">';
    modalHtml += '<div style="background:#fff; width:400px; max-width:90%; margin:100px auto; padding:25px; border-radius:10px; direction:rtl;">';
    modalHtml += '<span onclick="closeWaModal()" style="float:left; cursor:pointer; font-size:20px;">&times;</span>';
    modalHtml += '<h3 style="color:#25D366; margin-top:0;">طلب سعر عبر واتساب</h3>';
    modalHtml += '<form id="waForm">';
    modalHtml += '<input type="hidden" id="wa_pid"><input type="hidden" id="wa_pname">';
    modalHtml += '<div style="margin-bottom:15px;"><label>الكمية التقريبية (من)</label><input type="number" id="wa_qty_from" required style="width:100%; padding:8px;"></div>';
    modalHtml += '<div style="margin-bottom:15px;"><label>إلى</label><input type="number" id="wa_qty_to" required style="width:100%; padding:8px;"></div>';
    modalHtml += '<div style="margin-bottom:15px;"><label>التفاصيل</label><textarea id="wa_details" rows="4" required style="width:100%; padding:8px;"></textarea></div>';
    modalHtml += '<button type="submit" style="background:#25D366; color:#fff; border:none; padding:12px; width:100%; border-radius:5px;">إرسال الطلب</button>';
    ...
    document.getElementById('waForm').addEventListener('submit', async function(e) {
        ...
        formData.append('product_id', document.getElementById('wa_pid').value);
