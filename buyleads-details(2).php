...
if (!function_exists('isPremiumMember')) {
    function isPremiumMember($plan) {
        if (empty($plan)) return false;
        $plan_lower = strtolower($plan);
        return (strpos($plan_lower, 'sponsor') !== false || 
                strpos($plan_lower, 'sponser') !== false || 
                strpos($plan_lower, 'senior') !== false || 
                strpos($plan_lower, 'senier') !== false);
    }
}


// ✅ تعريف $membership_plan في بداية الملف
$membership_plan = '';
...
// ✅ إذا كانت $membership_plan فارغة، استخدم قيمة افتراضية
if (empty($membership_plan)) {
    $membership_plan = 'junior';
}


// ✅ تصحيح: عرض قيمة br_id
$br_id = isset($_GET['id']) ? $_GET['id'] : '';
error_log("🔍 br_id from URL: " . $br_id);
...
// ✅ تعريف دوال مساعدة للتحقق من العضوية
$is_sponsor = stripos($membership_plan, 'sponsor') !== false || stripos($membership_plan, 'sponser') !== false;
$is_senior = stripos($membership_plan, 'senior') !== false || stripos($membership_plan, 'senier') !== false;
$is_premium = $is_sponsor || $is_senior;

// ✅ تعريف دالة مساعدة للاستخدام في جميع أنحاء الملف
function isPremiumMember($plan) {
    if (empty($plan)) return false;
    $plan_lower = strtolower($plan);
    return (strpos($plan_lower, 'sponsor') !== false || 
...
// ✅ إذا كان br_id فارغاً، أعد التوجيه
if (empty($br_id)) {
    header("Location: buyleads.php");
    exit;
}

// ✅ الاستعلام الرئيسي (محاولة بـ MD5 أولاً)
$sql = "SELECT br.*, pc.pc_id, pc.pc_name, 
               u.usr_id, u.fname, u.lname, u.email, u.mobile1, u.country,
               bp.bnsprof_id, bp.bnsprof_compname, bp.bnsprof_city
        FROM buy_requirement br
        LEFT JOIN product_category pc ON br.br_pc_id = pc.pc_id
        LEFT JOIN user u ON br.br_u_id = u.usr_id
        LEFT JOIN business_profile bp ON u.usr_id = bp.bnsprof_uid
        WHERE MD5(br.br_id) = '" . mysqli_real_escape_string($con, $br_id) . "'
        LIMIT 1";
$res = mysqli_query($con, $sql);
$row = mysqli_fetch_object($res);

// ✅ إذا لم يتم العثور، حاول باستخدام br_id مباشرة
if (!$row && is_numeric($br_id)) {
    $sql = "SELECT br.*, pc.pc_id, pc.pc_name, 
                   u.usr_id, u.fname, u.lname, u.email, u.mobile1, u.country,
                   bp.bnsprof_id, bp.bnsprof_compname, bp.bnsprof_city
            FROM buy_requirement br
...
if (!$row) {
    // ✅ سجل الخطأ للتصحيح
    error_log("❌ No row found for ID: " . $br_id);
    die("⚠️ طلب الشراء غير موجود أو تم حذفه.");
}
?>
...
$sql_pcat = "SELECT m.pc_id as main_id, m.pc_name as main_name,
                    ...
                    s.pc_id as sub_id, s.pc_name as sub_name
             FROM product_category m
             LEFT JOIN product_category c ON m.pc_id = c.pc_parent_id
             LEFT JOIN product_category s ON c.pc_id = s.pc_parent_id
             WHERE s.pc_id = " . (int)$row->br_pc_id . "
             LIMIT 1";
$res_pcat = mysqli_query($con, $sql_pcat);
$row_pcat = mysqli_fetch_assoc($res_pcat);

if (!$row_pcat) {
    $row_pcat = [
        'main_id' => 0, 'main_name' => 'غير محدد',
        'cat_id' => 0, 'cat_name' => 'غير محدد',
        'sub_id' => 0, 'sub_name' => 'غير محدد'
    ];
}
...
// ✅ إذا كانت $membership_plan فارغة، استخدم قيمة افتراضية
if (empty($membership_plan)) {
    $membership_plan = 'junior'; // أو أي قيمة افتراضية
}
?>
...
<div class="p3 pl lf mm">
    <a href="buyleads.php" class="c12 td">طلبات شراء</a>  >  
    <?php if(isset($row_pcat) && is_array($row_pcat)): ?>
    <a href="category.php?token=<?php echo rand(1000,9999) . md5($row_pcat['main_id']); ?>" class="c12"><?php echo htmlspecialchars($row_pcat['main_name'] ?? 'غير محدد'); ?></a>
     > <a href="catcompany.php?token=<?php echo rand(1000,9999) . md5($row_pcat['cat_id']); ?>" class="c12"><?php echo htmlspecialchars($row_pcat['cat_name'] ?? 'غير محدد'); ?></a>
     > <?php echo htmlspecialchars($row_pcat['sub_name'] ?? 'غير محدد'); ?>
    <?php else: ?>
    <span class="c12">غير محدد</span>
    <?php endif; ?>
    <br>
</div>
</div>
...
<h1 class="f6 cl2" id="lead_headline"><?php echo htmlspecialchars($row->br_pd_name ?? ''); ?></h1>
<?php 
$cid = !empty($row->bnsprof_id) ? rand(1000,9999) . md5($row->bnsprof_id) : '';
if(!empty($row->br_preferred_supplier_location)){
?>
- <span class="f5">
<?php
    if($row->br_preferred_supplier_location == 'any') echo "من كل مكان";    
        ...
        echo htmlspecialchars(get_country_name($row->country)); ?>
        <img src="images/country_flag/<?php echo get_country_flag($row->country); ?>" alt="" height="16" width="24">
    <?php } else if($row->br_preferred_supplier_location == 'my_city' && !empty($row->bnsprof_city) && $row->bnsprof_city != '0') {
        echo htmlspecialchars(get_city_name($row->bnsprof_city));
    } ?>
</span> 
<?php } ?>
<?php
...
<p style="color:rgb(185,184,184);float:right;text-align:right;" class="j1 cb"><font style="color:rgb(152,151,151);">: تاريخ النشر</font> <?php echo date("d M, Y", strtotime($row->br_updated_date ?? 'now')); ?> | <font style="color:rgb(152,151,151);">: آخر تحديث</font> <?php echo date("d M, Y", strtotime($row->br_posting_date ?? 'now')); ?></p>
<div>



<?php 
// ✅ مسار الصورة الديناميكي (نسخة بسيطة)
$image_name = !empty($row->br_pic) ? $row->br_pic : 'no-image.png';

// ✅ التحقق من وجود الصورة في image_gallery
$image_path = 'upload/image_gallery/';
$full_path = $_SERVER['DOCUMENT_ROOT'] . '/' . $image_path . $image_name;

if (!file_exists($full_path)) {
    // ✅ إذا لم تكن موجودة، استخدم الصورة الافتراضية
    $image_name = 'no-image.png';
    $image_path = 'upload/buy_requirement/thumb/';
}
?>
<img src="<?php echo $image_path . $image_name; ?>" 
     id="zoomImage<?php echo $row->br_id; ?>" 
     border="0" height="100" width="125" 
     data-zoomsrc="<?php echo $image_path . $image_name; ?>"
...
<div class="bdt" id="hdiv1">
    <div class="g2 fs k7">
        ...
        <?php if(!empty($row->br_estimate_qty) && $row->br_estimate_qty != '0'){ ?>
        ...
        <br>
        <?php
        if(($row->br_apprx_order_value ?? '') != '' && ($row->br_apprx_order_currency ?? '') != '' || ($row->br_description ?? '') != '' || ($row->br_website ?? '') != '' && $row->br_website != "http://" || ($row->br_need_quote_for ?? '') != '' || ($row->br_purchase_time ?? '') != '' || ($row->br_preferred_supplier_location ?? '') != '' || ($row->br_need_for ?? '') != '' || ($row->br_requirement_frequency ?? '') != ''){
        ?>
        ...
        <?php if($row->br_website != '' && $row->br_website != "http://"){ ?>الموقع الإلكتروني: <?php echo stripslashes($row->br_website); ?><br><?php } ?>
...
<?php 
$is_sponsor = !empty($membership_plan) && (stripos($membership_plan, 'sponsor') !== false || stripos($membership_plan, 'sponser') !== false);
$is_senior = !empty($membership_plan) && (stripos($membership_plan, 'senior') !== false || stripos($membership_plan, 'senier') !== false);
if (!$is_sponsor && !$is_senior) { 
?>
...
<?php if(!isPremiumMember($membership_plan)) { ?>
...
<p class="sbg d_bp1 bo f1"><?php echo date("d M, Y", strtotime($row->br_updated_date ?? 'now')); ?></p>
<div class="ef3">
...
<span class="e4 f1 wb e5"><b><?php echo htmlspecialchars(get_country_name($row->country)); ?></b></span>
<?php } else { ?>
