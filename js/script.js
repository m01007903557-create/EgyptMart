...
function handleText(node, targetRe) {
  ...
  if (match) {
    ...
    // Not appendChild: bcz, menu not working properly
...
$(window).bind('scroll', function() {
          ...
          $('.webcast-alert-fix').css('position', 'fixed');
					$('.webcast-alert-fix').css('top', '170px');
          $('.webcast-alert-fix1').css('position', 'fixed');
          $('.webcast-alert-fix1').css('top', '320px');
                } else {
                    
					$('#right-image').css('position', 'static');
          $('.webcast-alert-fix').css('position', 'static');
          $('.webcast-alert-fix1').css('position', 'static');
                }
            });
      ...
      var div_top = '';
      if ($('.webcast-alert-fix').length>0) {
          div_top = $('.webcast-alert-fix').offset().top;
      }
      $(window).bind('scroll', function() {
        var window_h = $(document).height();
        var footer_h = $('.footer').height();
        var pick_point = window_h-(footer_h+div_top);
        //console.log(pick_point);
        if ($(window).scrollTop() > pick_point) {
          $('.webcast-alert-fix').css('position', 'static');
          $('.webcast-alert-fix1').css('position', 'static');
        }
      });
