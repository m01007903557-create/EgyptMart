...
//echo $view_product; die;
          ...
          $userArray = mysqli_query($view_product);
...
<?php
	...
	(pd_preferred_buyer_location='domestic' and pd_uid in(select distinct usr_id from user where country='".mysqli_real_escape_string($_COOKIE['loc_id'])."'))

	or

	(pd_preferred_buyer_location='any' and pd_uid in(select distinct usr_id from user where country='".mysqli_real_escape_string($_COOKIE['loc_id'])."'))

	or

	(pd_preferred_buyer_location='my_city' and pd_uid in(select distinct bnsprof_uid from business_profile where bnsprof_city in (select ct_id from city where ct_cn_id='".mysqli_real_escape_string($_COOKIE['loc_id'])."'))))";
	...
	(pd_preferred_buyer_location='abroad' and pd_uid not in(select distinct usr_id from user where country=(select cn_id from country where cn_code='".mysqli_real_escape_string($location_geo_country[0])."')))
...
?>
       <?php /*?>   <header style="font-size: 16px;color: #00297c;font-weight: 700;"><span><?php if($_GET['rctyp'] == 'buy_lead'){ echo " Buy Leads"; } ?> Countries / States</span> </header><?php */?>
		<?php if($_GET['rctyp']){ $rctypval = $_GET['rctyp'];}
		 ...
		 $run_sql= mysqli_query($view_country, $link);
		 $counter=0;
		 $country_id = array();
	 ...
	 if(count($country_buy)>0){
		...
		//echo "<br>"."--------------".$country_buy;
             //  echo "SELECT * FROM `country` where cn_name ='$country_buy'"; exit;
				$Couflag =mysqli_query("SELECT * FROM `country` where cn_name ='$cb'");
				 while($rowk=mysqli_fetch_array($Couflag))
				  { 
				//echo $Couflag; die;
  ...
  ?>
              <div class="checkbox" style="text-align:left;">
				...
				/*$rr = "SELECT * FROM `country` where cn_id in($country_string)"; 
					echo "-----------------------------".$rr;  die;	*/
							$CountryName =mysqli_query("SELECT * FROM `country` where cn_id in($country_string)");
							$count_country = mysqli_num_rows($CountryName);
							   while($getCountryName=mysqli_fetch_array($CountryName, mysqli_ASSOC))
                            {
								 $getCountryName;
								$counter++;
                /*if($counter== 5)
...
<?php // $sql = "select * from states where state_cn_id = ".$_COOKIE['loc_id']." and state_cn_id in( select distinct country from user where country='".$_COOKIE['loc_id']."' and usr_id in(".$view_country2.") )";
    ...
    $sql = "select * from states where state_id in (".$view_country2.") ";
		 ...
		 */
  //echo $sql;
       $run_sql1= mysqli_query($sql,$link) or die(mysqli_error());
                         $state_id = array();
...
//if(!empty($state_id))
//if($state_id!='') {
	$count_states = mysqli_num_rows($run_sql1);
	if($count_states!='') {
	?>
  <header><span class="h4">Supplier States</span></header>
   <?php }
                        $counter=0;
						//$count_states = mysqli_num_rows($run_sql1);
                         while($rowz=mysqli_fetch_array($run_sql1, mysqli_ASSOC))
                            {
								$counter++;
						...
						$result=mysqli_query($sql_country_flag_image);
						$row_flag=mysqli_fetch_array($result);
				if($counter== 5)
                {
