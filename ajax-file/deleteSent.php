<?php
/**
 * File: ajax/deleteSent.php

 * Description: حذف رسائل متعددة من الصادر (تعطيلها) للمستخدم الحالي
...
try {
    ...
    foreach ($msg_ids as $mid) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            $failed_ids[] = $mid;
            error_log("Failed to delete sent message ID $mid: " . mysqli_error($con));
        }
    ...
    if (!empty($failed_ids)) {
        error_log("Failed to delete sent message IDs: " . implode(', ', $failed_ids));
    }

} catch (Exception $e) {
    // تراجع عن المعاملة في حالة الخطأ
    mysqli_rollback($con);
    error_log("Delete Multiple Sent Messages Error: " . $e->getMessage());
