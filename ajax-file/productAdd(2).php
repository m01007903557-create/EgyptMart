 ...
 * Description: حفظ منتج جديد مع التحقق من الكلمات الممنوعة ودعم الحقول الإضافية
 * Version: 2.1.0 (PHP 8.3 Compatible)
 */
...
// ===== الحقول الإضافية =====
$additional_fields = isset($_POST['additional_fields']) ? $_POST['additional_fields'] : [];
...
if ($valid) {
    ...
    if (mysqli_stmt_execute($stmt_insert)) {
        ...
        // ===== حفظ الحقول الإضافية =====
        if (!empty($additional_fields) && $product_id > 0) {
            // جلب تعريفات الحقول الإضافية لهذا التصنيف
            $fields_sql = "SELECT af_id, af_name, af_type FROM additional_field WHERE af_pc_id = ?";
            $fields_stmt = mysqli_prepare($con, $fields_sql);
            mysqli_stmt_bind_param($fields_stmt, 'i', $pd_subcat_id);
            mysqli_stmt_execute($fields_stmt);
            $fields_result = mysqli_stmt_get_result($fields_stmt);
            
            $insert_field_sql = "INSERT INTO product_additional_field 
                                 (paf_product_id, paf_field_id, paf_value, paf_created_date) 
                                 VALUES (?, ?, ?, NOW())";
            $field_stmt = mysqli_prepare($con, $insert_field_sql);
            
            while ($field = mysqli_fetch_assoc($fields_result)) {
                $field_name = 'af_' . $field['af_name'];
                $field_value = isset($additional_fields[$field_name]) ? $additional_fields[$field_name] : '';
                
                if (!empty($field_value)) {
                    mysqli_stmt_bind_param($field_stmt, 'iis', $product_id, $field['af_id'], $field_value);
                    mysqli_stmt_execute($field_stmt);
                }
            }
            mysqli_stmt_close($field_stmt);
            mysqli_stmt_close($fields_stmt);
        }
