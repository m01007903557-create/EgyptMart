...
if (isset($_COOKIE['loc_id']) && !empty($_COOKIE['loc_id'])) {
    ...
    // تعطيل $sql_pd_ck مؤقتاً للتجربة
$sql_pd_ck = " AND (
    ...
    $sql_pd_ck = " AND (
        ...
        (pd_preferred_buyer_location = 'abroad' AND pd_uid NOT IN (
            ...
            WHERE country = (SELECT cn_id FROM country WHERE cn_code = '$location_geo_country')
        ))
    )";
}
// =============================================
...
// =============================================
// بناء الاستعلام مع علامات الاستفهام
$ids = '5,4,3,15'; // القيم مفصولة بفواصل بدون علامات اقتباس حول المجموعة

// تأكد من أن $sql_pd_ck لا يحتوي على علامات ?
$sql_pd_ck = str_replace('?', '', $sql_pd_ck);
                        ...
                        <li class="als-item">
                            ...
                            <p class="cnt-phone">
                                <span class="cnt-phone-inner">
                                    ...
                                    <a class="a_tel" href="tel:+<?php echo $phone_code; ?>-<?php echo $mobile; ?>">
                                        <?php echo $phone_code; ?>-<?php echo $mobile; ?>
                                    </a>
                                    <?php else: ?>
                                    <a class="a_tel" href="/sign-in.php#loginform">إظهر الرقم</a>
                                    <?php endif; ?>
                                </span>
                            </p>
