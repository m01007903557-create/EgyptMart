<?php
/**
 * File: ajax/editProdImg.php

 * Description: تحديث صور المنتج مع إنشاء صورة مصغرة وحذف الصورة القديمة
...
try {
    ...
    $sql_pd = "SELECT pd_image FROM products WHERE pd_id = ? LIMIT 1";
    $stmt_pd = mysqli_prepare($con, $sql_pd);
    ...
    $old_image = $row_pd->pd_image ?? '';
    if (!empty($old_image)) {
        $old_images = explode(',', $old_image);
        foreach ($old_images as $old_img) {
            if (!empty($old_img)) {
                $old_path_lrg = $targetFolder . $old_img;
                if (file_exists($old_path_lrg) && is_file($old_path_lrg)) {
                    unlink($old_path_lrg);
                }
                
                $old_path_thumb = $thumbFolder . $old_img;
    ...
    // تحديث قاعدة البيانات - إضافة الصورة الجديدة إلى بداية السلسلة
    $allimg = $row_pd->pd_image ?? '';
    $newimg = $newFileName . (!empty($allimg) ? ',' . $allimg : '');
    
    $update_sql = "UPDATE products SET pd_image = ? WHERE pd_id = ?";
    $stmt_update = mysqli_prepare($con, $update_sql);
    mysqli_stmt_bind_param($stmt_update, 'si', $newimg, $pd_id);
    ...
    echo "1|Product images updated successfully";

} catch (RuntimeException $e) {
    error_log("Edit Product Images Error: " . $e->getMessage() . " | Product ID: $pd_id");
    http_response_code(400);
    echo "0|" . $e->getMessage();
} catch (Exception $e) {
    error_log("Unexpected Edit Product Images Error: " . $e->getMessage() . " | Product ID: $pd_id");
