 ...
 * Version: 3.0.0 (نهائي - مع أزرار قبول/رفض للمشتري)
 */
...
// ============================================
// تحديد دور المستخدم الحالي (المهم للأزرار)
// ============================================
$is_buyer = false;
$is_supplier = false;
$user_role = 'unknown';

// إذا كان المستخدم الحالي هو مرسل الطلب الأصلي فهو المشتري
if ($current_user == $row->msg_from) {
    $is_buyer = true;
    $user_role = 'buyer';
}
// إذا كان المستخدم الحالي هو مستقبل الطلب فهو المورد
elseif ($current_user == $row->msg_to) {
    $is_supplier = true;
    $user_role = 'supplier';
}
...
function openChat(chatCode) {
    if (chatCode) {
        ...
        alert('لا توجد محادثة مفتوحة لهذا الطلب');
    }
}
</script>
...
<div class="fl_m2 my_m2" id="det" align="left">
    ...
    <div id="mhmdd">
        <div class="f4_m2 b5_m2 b11_m2 b10_m2 b7_m2 bg2_m2 ac_m2 p6_m2 hh_m2 b18_m2" id="mainheader">
                        ...
                        ?>
                            <div class="offer-card" style="border:1px solid #ddd; padding:15px; margin:10px 0; border-radius:8px; background:#fff;">
                                ...
                                <?php elseif ($is_supplier): ?>
                                    <!-- أزرار المورد (تعديل / تحديث) -->
                                    <div class="offer-actions" style="margin-top:15px;">
                                        <?php if ($update_count == 0): ?>
                                            <button class="btn btn-success" onclick="openOfferPopup(<?php echo $rfq_id; ?>, 'send', <?php echo $offer_id; ?>)">
                                                <i class="fa fa-money"></i> إرسال عرض سعر
                                            </button>
                                        <?php elseif ($update_count == 1): ?>
                                            <button class="btn btn-warning" onclick="openOfferPopup(<?php echo $rfq_id; ?>, 'update', <?php echo $offer_id; ?>)">
                                ...
                                <?php elseif ($is_buyer): ?>
                                    <!-- أزرار المشتري (قبول / رفض / فتح المحادثة) -->
                                    <div class="offer-actions" style="margin-top:15px;">
                                        <button class="btn btn-success" onclick="acceptOffer(<?php echo $offer_id; ?>, <?php echo $rfq_id; ?>)">
                                            <i class="fa fa-check"></i> قبول العرض & كشف بيانات المورد
                                        </button>
                                        ...
                                        <?php if ($chat_exists && $chat_code): ?>
                                            <button class="btn btn-info" onclick="openChat('<?php echo $chat_code; ?>')">
            ...
            <!-- زر فتح المحادثة العام (إذا كان موجوداً) -->
            <?php if ($chat_exists && $chat_code): ?>
                        ...
                        <td class="sh_m2">
                            <table cellpadding="0" cellspacing="0">
                                <tbody>
                                    ...
                                    <tr>
                                        <td><b> From:</b></td>
                                        <td><?php echo $from_name; ?> <<?php echo $from_email; ?>></td>
                                    </tr>
                                    <tr>
                                        <td><b> Date:</b></td>
                    ...
                    <tr class="f5_m2">
                        <td class="sh_m2">
                            <span style="width:750px; word-wrap:break-word;" id="wbr">
                                <?php 
                                $allowed_tags = '<div><span> <br> <p> <b> <strong> <i> <u> <h1> <h2> <h3> <h4> <table> <tr> <tbody> <img> <hr> <ul> <ol> <li>';
