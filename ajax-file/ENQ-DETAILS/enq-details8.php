...
<?php elseif ($is_supplier && $rfq_id > 0): ?>
            <!-- لا توجد عروض - زر إنشاء عرض جديد -->
            <div class="offers-section" style="margin:15px 0; padding:12px; background:#f9f9f9; border-radius:8px; text-align:center;">
                <p style="margin:0 0 8px 0; font-size:13px;">لم تقم بتقديم عرض سعر لهذا الطلب بعد.</p>
                <button class="btn btn-success" onclick="createNewOffer(<?php echo $rfq_id; ?>)">
                    <i class="fa fa-money"></i> إرسال عرض سعر (مرة واحدة)
                </button>
            </div>
            <?php endif; ?>
...
// ============================================
// دالة إرسال عرض السعر من المورد
// ============================================
function sendSupplierOffer(rfqId) {
    let price = prompt('💰 أدخل السعر (USD):');
    if (!price) return;
    ...
    let confirmMsg = `تأكيد عرض السعر:\n💰 السعر: ${price} USD\n🚚 مدة التوصيل: ${delivery} يوم\n📝 ${notes || 'لا توجد'}`;
    if (!confirm(confirmMsg)) return;
