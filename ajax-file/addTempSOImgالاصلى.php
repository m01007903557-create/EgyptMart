<?php
/**
 * File: ajax/addTempSOImg.php

 * Description: رفع صورة مؤقتة لعرض البيع مع إنشاء صورة مصغرة وحذف الصورة القديمة
...
try {
    ...
    $select_sql = "SELECT tsi_image FROM temp_selloffer_image WHERE tsi_usr_id = ? LIMIT 1";
    $stmt_select = mysqli_prepare($con, $select_sql);
    mysqli_stmt_bind_param($stmt_select, 'i', $usr);
    mysqli_stmt_execute($stmt_select);
    $result_select = mysqli_stmt_get_result($stmt_select);
    $row_tsi = mysqli_fetch_object($result_select);
    mysqli_stmt_close($stmt_select);

    // حذف الصورة القديمة إذا كانت موجودة
    if ($row_tsi && !empty($row_tsi->tsi_image)) {
        $old_image = $row_tsi->tsi_image;
    ...
    error_log("Upload Temp Sell Offer Image Error: " . $e->getMessage() . " | User ID: $usr");
    http_response_code(400);
    echo "0|" . $e->getMessage();
} catch (Exception $e) {
    error_log("Unexpected Upload Temp Sell Offer Image Error: " . $e->getMessage() . " | User ID: $usr");
