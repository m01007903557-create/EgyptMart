...
function sendOffer(rfqId) {
    console.log('1- بدء sendOffer, rfqId:', rfqId);
    ...
    .then(response => {
        ...
        return response.text();  // ✅ نأخذ النص أولاً للتشخيب
    })
    .then(text => {
        console.log('7- الاستجابة الخام:', text);
        try {
            let data = JSON.parse(text);
            console.log('8- البيانات المحللة:', data);
            alert(data.message);
            
            if (data.is_first === false && data.whatsapp_url) {
                window.open(data.whatsapp_url, '_blank');
            }
            
            location.reload();
        } catch(e) {
            console.error('9- خطأ في تحليل JSON:', e);
            alert('❌ خطأ في استجابة الخادم');
        }
    })
    .catch(error => {
        console.error('10- خطأ:', error);
