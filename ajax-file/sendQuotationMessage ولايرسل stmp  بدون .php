...
if (mysqli_stmt_execute($stmt)) {
    ...
    // إرسال البريد الإلكتروني
        // ------------------------------------------------------------------
    // إرسال البريد الإلكتروني باستخدام PHPMailer - متوافق مع Hostinger
    // ------------------------------------------------------------------
    ...
    // إعدادات SMTP
    $from_mail = 'info@egyptmart.shop';
    $from_password = 'ANAehab@64';
    $from_name = get_page_settings(4) ?: 'EgyptMART';
    $to_email = user_info($msg_to, 'email');
    $subject = $row_usr->bnsprof_compname . ' إستفسار شراء من';
    // إنشاء محتوى البريد من القالب
        // =================================================================
    // الكود الصحيح والنهائي بدءًا من إنشاء محتوى البريد
    // =================================================================
    ...
    if (!empty($to_email) && !empty($message_content) && !empty($from_password)) {
        // استخدام الاسم الكامل للكلاس بدون "use"
        $mail = new \PHPMailer\PHPMailer\PHPMailer(true);

        try {
// إعدادات SMTP البديلة - TLS على المنفذ 587
...
$mail->SMTPDebug = 2;
            $mail_sent = $mail->send();
            error_log("✅ SMTP: تم إرسال البريد بنجاح إلى: $to_email");
            
        } catch (Exception $e) {
            error_log("❌ SMTP: فشل إرسال البريد: " . $mail->ErrorInfo);
            $mail_sent = false;
        }
    } else {
        if (empty($from_password)) {
            error_log("❌ SMTP: كلمة مرور البريد الإلكتروني غير مدخلة في الكود");
        } else {
            error_log("❌ SMTP: البريد الإلكتروني للمستلم أو محتوى الرسالة فارغ");
        }
    }
    
    if (!$mail_sent) {
        error_log("⚠️ تنبيه: لم يتم إرسال البريد الإلكتروني إلى $to_email، ولكن تم حفظ البيانات في قاعدة البيانات");
    }
    
    // إرجاع النجاح (تم حفظ الرسالة في قاعدة البيانات)
    echo "1";
