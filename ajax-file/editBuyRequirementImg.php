<?php
/**
 * File: ajax/editBuyRequirementImg.php


 * Description: تحديث صورة طلب الشراء مع إنشاء صورة مصغرة وحذف الصورة القديمة
...
try {
    ...
    $sql_br = "SELECT br_pic FROM buy_requirement WHERE br_id = ? LIMIT 1";
    ...
    if (!$row_br) {
        throw new RuntimeException("Buy requirement not found");
    }
    ...
    error_log("Edit Buy Requirement Image Error: " . $e->getMessage() . " | Requirement ID: $br_id");
    http_response_code(400);
    echo "0|" . $e->getMessage();
} catch (Exception $e) {
    error_log("Unexpected Edit Buy Requirement Image Error: " . $e->getMessage() . " | Requirement ID: $br_id");
