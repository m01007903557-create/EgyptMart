<?php
/**
 * File: ajax/addSellofferAlertCat.php

 * Description: نقل التصنيفات المؤقتة لتنبيهات البيع إلى الجدول الدائم
...
try {
    ...
    $select_sql = "SELECT tsac_pc_id FROM temp_selloffer_alert_cat WHERE tsac_usr_id = ?";
    ...
    while ($row = mysqli_fetch_object($result)) {
        $pc_id = (int)$row->tsac_pc_id;
        
        // التحقق من عدم وجود التصنيف مسبقاً في الجدول الدائم
        $check_sql = "SELECT sac_id FROM selloffer_alert_category 
                      WHERE sac_usr_id = ? AND sac_pc_id = ? 
                      LIMIT 1";
    ...
    $delete_sql = "DELETE FROM temp_selloffer_alert_cat WHERE tsac_usr_id = ?";
    ...
    mysqli_rollback($con);
    error_log("Save Sell Alert Categories Error: " . $e->getMessage() . " | User ID: $user_id");
