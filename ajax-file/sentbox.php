<?php
/**
 * File: ajax/sentbox.php

 * Description: تحميل وعرض الرسائل المرسلة مع التصفح (Pagination)
...
// استعلام جلب الرسائل المرسلة
$sql_sent = "SELECT m.*, u.* 
             FROM message m
             INNER JOIN user u ON m.msg_to = u.usr_id
             WHERE m.msg_from = ? 
             AND m.msg_from_status = '1' 
             ORDER BY m.msg_date DESC 
             LIMIT ?, ?";

$stmt = mysqli_prepare($con, $sql_sent);
...
<span id="fol_m2"><h1>الرسائل المرسلة</h1></span><!-- My Mailbox start -->
            ...
            <span class="pagenavigation">
                <div class="f1_m2 rf_m2 p9_m2">
                    ...
                    if ($first_btn && $cur_page > 1) {
                        echo '<a href="javascript:showSent(\'1\')"><img id="firstmail" src="images/firsten.gif"></a>';
                    } elseif ($first_btn) {
                    ...
                    if ($previous_btn && $cur_page > 1) {
                        $pre = $cur_page - 1;
                        echo '<a href="javascript:showSent(\'' . $pre . '\')"><img id="prevmail" src="images/prven.gif"></a>';
                    } elseif ($previous_btn) {
                    ...
                    if ($next_btn && $cur_page < $no_of_paginations) {
                        $nex = $cur_page + 1;
                        echo '<a href="javascript:showSent(\'' . $nex . '\')"><img id="nextmail" src="images/nxten.gif"></a>';
                    } elseif ($next_btn) {
                    ...
                    if ($last_btn && $cur_page < $no_of_paginations) {
                        echo '<a href="javascript:showSent(\'' . $no_of_paginations . '\')"><img id="lastmail" src="images/lastenv.gif"></a>';
                    } elseif ($last_btn) {
                            ...
                            <ul id="delete_m2" style="margin-top: 0px; padding-top: 0pt;">
                                <li style="z-index: 0; margin: 0px; padding: 0pt;"> 
                                    <span id="deleteall_m2">
                                        <a title="Delete" iscontextmenu="true" onclick="deleteSentbox();"></a>
                                    </span>
                                </li>
                            </ul>
                        </div>
                                ...
                                <tbody>
                                    <tr class="c1_m2 bo_m2 bg_m2 ff_m2 f3_m2">
                                        <td class="p2_m2 sb_m2 b_m2" width="3%">
                                            <input class="select-all" name="check_all" value="yes" id="check_all" type="checkbox" onClick="return checkedAll_sent();">
                                        </td>
                                        <td class="b_m2 sb_m2 p1_m2" width="43%">المستلم&nbsp;</td>
                                        <td class="sb_m2 p_m2" width="40%">الموضوع&nbsp;</td>
                                        <td class="sb_m2 p_m2" width="11%">التاريخ&nbsp;</td>
                                    </tr>
                                    ...
                                    <?php while ($row = mysqli_fetch_object($result)): 
                                        ...
                                        $msg_subject = htmlspecialchars(stripslashes($row->msg_subject ?? ''), ENT_QUOTES, 'UTF-8');
                                        $msg_date = !empty($row->msg_date) ? date("d M Y", strtotime($row->msg_date)) : 'N/A';
                                    ...
                                    <tr class="f1_m2" id="mail<?php echo $msg_id; ?>" 
                                        ...
                                        <td class="p2_m2">
                                            <div class="td_m2">
                                                <input id="cbS" name="cbS" type="checkbox" value="<?php echo $msg_id; ?>">
                                            </div>
                                        </td>
                                        
                                        <td class="p1_m2" onclick="openmail('<?php echo $msg_id; ?>','sent')" 
                                            onmouseover="showusercompany('comname1')" 
                                            onmouseout="showusercompany('comname1')">
                                            <div class="td_m2" style="width:200px">
                                                <?php echo trim($name_prefix . ' ' . $fname . ' ' . $lname); ?>
                                            </div>
                                            <div class="p14_m2"></div>
                                        </td>
                                        
                                        <td class="p_m2" onclick="openmail('<?php echo $msg_id; ?>','sent')">
                                            <div class="td_m2" style="width:320px" title="إضغط هنا لفتح هذا الاستفسار">
                                                <?php echo !empty($msg_subject) ? $msg_subject : 'No Subject'; ?>
                                            </div>
                                        </td>
                    ...
                    <tr class="f1_m2">
                        <td>
                            <div class="bo_m2 f3_m2 b9_m2 b12_m2 lh1_m2" id="nor_m2" align="center">
                                There are no messages in your Sent Box.
                            </div>
                            <table cellpadding="0" cellspacing="0" width="100%">
                                <tbody>
