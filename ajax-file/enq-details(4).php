...
function sendOffer(rfqId) {
    // ✅ النافذة الأولى: السعر
    let price = prompt('السعر (USD):');
    if (price === null) return; // إذا ألغى المستخدم
    ...
    // ✅ النافذة الثانية: مدة التوصيل
    let delivery = prompt('مدة التوصيل (أيام):');
    if (delivery === null) return;
    delivery = parseInt(delivery);
    if (isNaN(delivery) || delivery <= 0) {
        alert('يرجى إدخال مدة توصيل صحيحة');
        return;
    }

    // ✅ النافذة الثالثة: ملاحظات (اختيارية)
    let notes = prompt('ملاحظات:', '');

    // ✅ إرسال البيانات إلى المعالج
    ...
    .then(d => {
        ...
        // ✅ إذا كان هناك رابط واتساب، افتحه
        if (d.whatsapp_url && d.whatsapp_url != '') {
            window.open(d.whatsapp_url, '_blank');
        }
        
        // ✅ إذا كان هناك chat_code، افتح المحادثة
