...
// الآن قم بعرض الرسائل كما كان يفعل الكود الأصلي
// (هذا الجزء يعتمد على كيفية تصميم عرض الرسائل في ملف inbox.php نفسه)
// إذا كان هناك كود HTML لعرض الجدول، يرجى إرساله لأكمله.

// لإظهار النتيجة، سنقوم بطباعة عدد الرسائل كاختبار
echo "<h3>صندوق الوارد (" . $total_messages . " رسالة)</h3>";
foreach ($messages as $msg) {
    $sender = $msg->bnsprof_compname ?? $msg->fname ?? $msg->username ?? 'النظام';
    $type_label = ($msg->source == 'whatsapp_rfq') ? '[WhatsApp]' : '[إيميل]';
    echo "<div><strong>$sender $type_label</strong>: " . htmlspecialchars($msg->msg_subject) . " - " . $msg->msg_date . "</div>";
}
?>