...
// ============================================
// تحديد الأدوار (نسخة تصحيح نهائي)
// ============================================
...
<?php
// ============================================
// 1. جلب المتغيرات الأساسية
// ============================================
$rfq_id = $row->msg_entity_id ?? 0; 
$user_id = $_SESSION['uid_indm'] ?? 0; 

// ============================================
// 2. منطق إظهار زر "بدء المحادثة"
// ============================================
// جلب رمز المحادثة
$chat_code = '';
...
echo "<!-- Debug: chat_code from DB = " . ($chat_code ? $chat_code : 'غير موجود') . " -->";

// 2.2 لو يوجد عرض، هات chat_code
if ($show_chat_btn) {
    $chat_check = mysqli_query($con, "SELECT chat_code FROM chat_rooms WHERE rfq_id = $rfq_id LIMIT 1");
    if ($chat_check && mysqli_num_rows($chat_check) > 0) {
        $chat_data = mysqli_fetch_assoc($chat_check);
        ...
        $show_chat_btn = false; // لو مفيش كود يبقى متظهرش الزر
    }
}
?>
...
<!-- زر المحادثة -->
<!-- ============================================ -->
<!-- زر بدء المحادثة (يظهر فقط عند وجود عرض حقيقي) -->
<!-- ============================================ -->
<?php
$has_real_offer = false;
$offer_check = mysqli_query($con, "SELECT 1 FROM offers WHERE rfq_id = $rfq_id AND price > 0 AND status IN ('pending', 'negotiation') LIMIT 1");
if ($offer_check && mysqli_num_rows($offer_check) > 0) {
    $has_real_offer = true;
}
?>

<?php if ($rfq_id > 0 && $has_real_offer): ?>
    <div style="margin-top:15px; padding:10px; background:#f0f8ff; border-radius:8px; border-right:4px solid #17a2b8;">
        <a href="/chat/chat.php?rfq_id=<?php echo $rfq_id; ?>" class="btn btn-info" target="_blank">
