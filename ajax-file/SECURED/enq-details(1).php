...
<!-- زر المحادثة -->
<?php if ($chat_code): ?>
    <a href="/chat/chat.php?chat_code=<?php echo $chat_code; ?>" class="btn btn-info" target="_blank">بدء المحادثة</a>
<?php endif; ?>
...
function sendOffer(rfqId) {
    ...
    .then(d => {
        ...
        // ✅ فتح واتساب فقط في حالة التعديل (وليس العرض الأول)
        if (d.whatsapp_url && d.whatsapp_url != '' && d.is_first === false) {
            window.open(d.whatsapp_url, '_blank');
        }
        
        // ✅ فتح المحادثة مباشرة (بدون سؤال)
        if (d.chat_code) {
