...
$user_type = 'buyer'; // أو 'supplier' حسب الصفحة
...
// =============================================
// استعلام موحد لرسائل الإيميل و WhatsApp من جدول message
// =============================================

// 1️⃣ رسائل الإيميل العادية
$sql_email = "SELECT m.*, 
              ...
              AND (m.msg_entity IS NULL OR m.msg_entity != 'whatsapp_rfq')";

// 2️⃣ رسائل WhatsApp RFQ (مخزنة في نفس جدول message)
$sql_whatsapp = "SELECT m.*, 
                        NULL as name_prefix, NULL as fname, NULL as lname, NULL as email,
                        NULL as bnsprof_compname,
                        NULL as username,
                        'whatsapp_rfq' as source
                 FROM message m
                 WHERE m.msg_to = ? 
                 AND m.msg_to_status = '1' 
                 AND m.msg_entity = 'whatsapp_rfq'";

// للتصحيح: سجل عدد رسائل WhatsApp للمورد
$check_whatsapp = mysqli_query($con, "SELECT COUNT(*) as count FROM message WHERE msg_to = $current_user AND msg_entity = 'whatsapp_rfq'");
$whatsapp_count = mysqli_fetch_assoc($check_whatsapp);
error_log("WhatsApp RFQ messages for supplier $current_user: " . $whatsapp_count['count']);
...
$sql_inbox = "SELECT m.*, 
              ...
              WHERE m.msg_to = ? 
              AND m.msg_to_status = '1' 
              -- لا تحذف السطر التالي، فقط تأكد من عدم وجود شرط يستثني whatsapp_rfq
                                    ...
                                    <tr class="c1_m2 bo_m2 bg_m2 ff_m2 f3_m2">
                                        <td class="p2_m2 sb_m2 b_m2" width="3%">
                                            <input class="select-all" name="check_all" value="yes" id="check_all" type="checkbox" onClick="return checkedAll_inbox();">
                                                                                <tr>
                                        <td class="b_m2 sb_m2 p1_m2 dp_m2" width="35%">Sender&nbsp;</td>
                                        <td class="b_m2 sb_m2 p1_m2 dp_m2" width="10%">Type&nbsp;</td>
                                        <td class="sb_m2 p_m2" width="40%">Subject&nbsp;</td>
                                        <td class="sb_m2 p_m2" width="15%">Date&nbsp;</td>
                                    </tr>
                                        ...
                                        if (!empty($row->bnsprof_compname)) {
                                            ...
                                            $sender_name = htmlspecialchars(
                                                ...
                                                'UTF-8'
                                                                                                              ...
                                                                                                              // أضف هذا الكود بعد سطر تحديد اسم المرسل مباشرة
$message_type = '';
if (isset($row->communication_type) && $row->communication_type == 'whatsapp') {
    $message_type = '<span style="color:#25D366;"><i class="fa fa-whatsapp"></i> WhatsApp RFQ</span>';
} elseif (isset($row->source) && $row->source == 'whatsapp_rfq') {
    $message_type = '<span style="color:#25D366;"><i class="fa fa-whatsapp"></i> WhatsApp RFQ</span>';
} else {
    $message_type = '<span style="color:#2196F3;"><i class="fa fa-envelope"></i> Email</span>';
}
                                    ...
                                    <tr class="f1_m2" id="mail<?php echo $msg_id; ?>" 
                                        ...
                                        <td>
    <?php 
    if (isset($row->source) && $row->source == 'whatsapp_rfq') {
        echo '<span class="label label-success"><i class="fa fa-whatsapp"></i> WhatsApp RFQ</span>';
    } else {
    ...
    ?>
                                       ...
                                       <td class="p1_m2" onclick="openmail('<?php echo $msg_id; ?>','inbox')">
                                            <div class="td_m2" style="width:200px"><?php echo $message_type; ?></div>
                                        </td>
