...
// 5. التحقق من وجود عرض سابق
$check = mysqli_query($con, "SELECT id FROM offers WHERE rfq_id = $rfq_id AND supplier_id = $supplier_id");
...
if (!$existing) {
    ...
    // تحديث عرض
    $update = "UPDATE offers SET price=$price, delivery_days=$delivery, notes='$notes', update_count=update_count+1 WHERE id={$existing['id']}";
