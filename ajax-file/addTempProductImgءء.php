...
/**
 * File: ajax/addTempProductImg.php

 * Description: رفع صورة منتج مؤقتة مع إنشاء صورة مصغرة وحذف الصورة القديمة
...
try {
    ...
    $select_sql = "SELECT tpi_image FROM temp_product_image WHERE tpi_usr_id = ? LIMIT 1";
    ...
    if ($row_tpi && !empty($row_tpi->tpi_image)) {
        $old_image = $row_tpi->tpi_image;
    ...
    echo "1|Product image uploaded successfully";

} catch (RuntimeException $e) {
    error_log("Upload Temp Product Image Error: " . $e->getMessage() . " | User ID: $usr");
    http_response_code(400);
    echo "0|" . $e->getMessage();
} catch (Exception $e) {
    error_log("Unexpected Upload Temp Product Image Error: " . $e->getMessage() . " | User ID: $usr");
