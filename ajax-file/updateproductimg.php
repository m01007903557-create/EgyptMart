<?php
/**
 * File: ajax/deleteProductMainImage.php
 * Description: حذف الصورة الرئيسية من منتج
...
// جلب بيانات المنتج
$sql_proImg = "SELECT pd_image FROM products WHERE pd_id = ?";
...
$sql = "UPDATE products SET pd_image = ? WHERE pd_id = ?";
$stmt = mysqli_prepare($con, $sql);
mysqli_stmt_bind_param($stmt, 'si', $new_img, $product_id);

if (mysqli_stmt_execute($stmt)) {
    echo "1|Main image deleted successfully";
} else {
    error_log("Delete Product Main Image Error: " . mysqli_error($con) . " | Product ID: $product_id");
    echo "0|Failed to delete main image";
}
