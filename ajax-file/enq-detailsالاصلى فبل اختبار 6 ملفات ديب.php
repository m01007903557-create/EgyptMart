...
echo "<!-- Debug: msg_from=" . $row->msg_from . ", msg_to=" . $row->msg_to . ", current_user=" . $current_user . " -->";

// ============================================
// تحديد الأدوار (تصحيح مؤقت)
// ============================================
$rfq_id = $row->msg_entity_id ?? 0;
$is_buyer = false;
$is_supplier = false;

// ✅ إذا كان المستخدم هو msg_to، فهو مورد
if ($current_user == $row->msg_to && $row->msg_to != 0) {
    $is_supplier = true;
}

// ✅ إذا كان المستخدم هو msg_from، فهو مشتري
if ($current_user == $row->msg_from && $row->msg_from != 0) {
    $is_buyer = true;
}

// ✅ إذا كانت الرسالة من الأدمن (msg_from = 0)
if ($row->msg_from == 0 && $rfq_id > 0) {
    // جلب المشتري والمورد من offers
