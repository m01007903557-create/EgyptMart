...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $product_image_src = htmlspecialchars(em_product_image_src($row['pd_image'] ?? ''), ENT_QUOTES, 'UTF-8');
        ...
        $html .= '<img src="' . $product_image_src . '" alt="' . $product_title . '" class="img-responsive" onerror="this.src=\'upload/myproduct/noimage.jpg\'">';
        $html .= '</a>';
        ...
        <ul class="als-wrapper">
            <?php if (mysqli_num_rows($result) > 0): ?>
                <?php while ($row_prd = mysqli_fetch_object($result)): 
                    ...
                    $product_image_src = htmlspecialchars(em_product_image_src($row_prd->pd_image ?? '', true), ENT_QUOTES, 'UTF-8');
