...
echo "<!-- Debug: start of enq-details.php -->";
...
// ============================================
// جلب رمز المحادثة (مع إنشاء تلقائي إذا لم يكن موجوداً)
// ============================================
$chat_code = '';
...
if ($chat_check && mysqli_num_rows($chat_check) > 0) {
    ...
    // ✅ إذا لم توجد غرفة محادثة، أنشئ واحدة
    if ($rfq_id > 0 && ($is_supplier || $is_buyer)) {
        ...
        $insert_chat = "INSERT INTO chat_rooms (rfq_id, supplier_id, buyer_id, chat_code, created_at, status, expiry_date) 
                        VALUES ($rfq_id, " . ($is_supplier ? $current_user : 0) . ", " . ($is_buyer ? $current_user : 0) . ", '$chat_code', NOW(), 'active', DATE_ADD(NOW(), INTERVAL 7 DAY))";
...
function sendOffer(rfqId) {
    ...
    .then(d => {
        ...
        // ✅ إذا كان هناك chat_code، اسأل المستخدم إذا كان يريد فتح المحادثة
        if (d.chat_code) {
