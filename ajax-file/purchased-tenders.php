<?php
/**
 * اسم الملف: purchased-tenders.php
 * الوصف: عرض المناقصات المشتراة للمستخدم مع التقسيم (Pagination)
 * الإصدار: 2.0.0
...
/**
 * دالة الحصول على المناقصات المشتراة
 */
function getPurchasedTenders($con, $user_id, $start, $per_page) {
    $sql = "SELECT t.*, pt.* 
            FROM tender t
            JOIN purchased_tender pt ON pt.ptnd_tnd_id = t.tnd_id
            WHERE t.tnd_status = '1' 
            AND pt.ptnd_usr_id = ?
            ORDER BY pt.ptnd_purchase_date DESC
            LIMIT ?, ?";
    ...
    return executeSecureQuery(
        ...
        "iii", 
        "خطأ في جلب المناقصات المشتراة"
    );
}

/**
 * دالة حساب العدد الإجمالي للمناقصات المشتراة
 */
function getTotalPurchasedCount($con, $user_id) {
    ...
    $result = executeSecureQuery(
        ...
        [$user_id], 
        "i", 
        "خطأ في حساب عدد المناقصات المشتراة"
    );
...
function displayPaginationButtons($cur_page, $no_of_paginations) {
    ...
    <div class="f1_m2 rf_m2 p9_m2">
        ...
        // زر الصفحة الأولى
        if ($cur_page > 1) {
            echo '<a href="javascript:purchasedTenders(\'1\')">';
        ...
        if ($cur_page > 1) {
            $pre = $cur_page - 1;
            echo '<a href="javascript:purchasedTenders(\'' . $pre . '\')">';
        ...
        if ($cur_page < $no_of_paginations) {
            $nex = $cur_page + 1;
            echo '<a href="javascript:purchasedTenders(\'' . $nex . '\')">';
        ...
        // زر الصفحة الأخيرة
        if ($cur_page < $no_of_paginations) {
            echo '<a href="javascript:purchasedTenders(\'' . $no_of_paginations . '\')">';
    ...
    <tr>
        <td valign="TOP">
            <div class="mp5 blhd" style="border-right: 1px solid #E7EAEE;">
                <p>
                    <a onclick="detailPurTenders(<?php echo intval($row->ptnd_id); ?>)" style="cursor:pointer;">
                    ...
                    <?php if(isset($row->br_approval_status) && $row->br_approval_status == '0'): ?>
                        <img src="images/waiting_ico1.png" 
                             title="&lt;b&gt;طلب الشراء قيد المراجعة من قبل النظام&lt;/b&gt;" 
                ...
                <div style="padding:0 0 5px 0;font-size:11px;color:#727272;">
                    <b>تاريخ الشراء:</b> 
                    <?php echo date("d M, Y", strtotime($row->ptnd_purchase_date)); ?>
                </div>
                <div class="blvd">
                    <a onclick="detailPurTenders(<?php echo intval($row->ptnd_id); ?>)" style="cursor:pointer;">
        ...
        <td class="mp5" valign="TOP" width="208">
            <div class="bulletImage fleft">
                <a id="apprvDel1" class="apprvDel" style="cursor:pointer;" 
                   onClick="delTender(<?php echo intval($row->tnd_id); ?>,'op');"></a>
            </div>
            <div class="bulletImage fleft">
                <a class="ajax" rel="nofollow" 
                   href="sendTenderEnquiry-form.php?id=<?php echo rand(1000,9999) . md5($row->tnd_usr_id); ?>">
                    إرسل إستفسار
                </a>
            </div>
        </td>
...
// تنفيذ الاستعلامات
$recObj = getPurchasedTenders($con, $user_id, $start, $per_page);
...
<div style="clear:both;">
    <div class="pbl-sr" style="background-position:0px -35px; min-height:90px;color:#F00;" align="center">
        <br>
        <b style="line-height:50px;">لا توجد مناقصات مشتراة حتى الآن</b>
        <br><br>
    </div>
