...
function readCookie(name) {
	...
	function showfavorite(product_id)
	{
			var existingids=readCookie("fpids");
		if(existingids==''){
			createCookie("fpids",product_id);
		} else {
			createCookie("fpids", existingids+","+product_id);
		}
/*		$.ajax({                              
		type: 'POST',
		url: "fevrate.php?product_id="+product_id,
...
//echo "2 ".$sqltk;
	...
	else if($rctyp=="tender")
	{
		$sql_extra="";
		if(isset($_COOKIE['loc_id'])){
			$auctionCondition = " AND ((auc_preferred_location='domestic' AND auc_usr_id in(SELECT DISTINCT usr_id FROM user WHERE country='".$_COOKIE['loc_id']."')) OR (auc_preferred_location='any' AND auc_usr_id in(SELECT DISTINCT usr_id FROM user WHERE country='".$_COOKIE['loc_id']."')) OR (auc_preferred_location='my_city' AND tnd_usr_id in(SELECT DISTINCT bnsprof_uid FROM business_profile WHERE bnsprof_city in (SELECT ct_id FROM city WHERE ct_cn_id='".$_COOKIE['loc_id']."'))))";
                      ...
                      <div class="row">
                          <div class="col-lg-3 big-img-box box-1">
                              ...
                              <?php if(isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != ''){ ?>
                                <a href="javascript:void(0)" class="product_fav_btn" data="<?php echo $rowk->pd_id; ?>"  onclick="return showfavorite(<?php echo $rowk->pd_id;?>)" class="ar-star"><i class="fa fa-star star" style="color:<?php echo (in_array($rowk->pd_id, $myfev))?'#E48F23':'#808080'; ?>"></i> Favourite</a>
                              <?php  }
                              else{ ?>
                              ...
                              <?php $pimg1= explode(',',$rowk->pd_image);  ?>
                                  <div class="zoomthis"> <?php  echo "<img src='/upload/myproduct/".$pimg1[0]."'>"; ?></div>
...
?>         
                                  ...
                                  <li> Fob Price  &nbsp; <big class="txt-bold txt-red"><a style="color: #d22027" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/product-add.php"><?php echo $symbol.$rowk->pd_fob_price.'~'.$symbol.$rowk->pd_fob_price2; ?> </a></big> &nbsp;  
                                <?php if(($_SESSION['uid_indm'] )==''){ ?>
                                  <a data-enquiry="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/sign-in.php"><button type="button" class="btn btn-link border-radius-0 btn-enquiry" style="font-weight:bold;">(Get Latest Price)</button></a> 
                                    <?php }else{ 
									if(($_GET['grid'])=='active'){				
									?>
                                   <a  class="ajax" data-price="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo $rand. md5($rowk->bnsprof_id); ?>&pid=<?php echo $rowk->pd_id; ?>&keywords=<?php echo urlencode($_GET['keywords']);?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1" class="txt-bold txt-black pull-right inquiry_but" id="btn_ajax_send<?php echo $rowk->pd_id; ?>" rel="product-send-inquiry">
                                    <button type="button" class="btn btn-link border-radius-0 btn-enquiry" style="font-weight:bold;">(Get Latest Price)</button>
                                    </a>
                                    <?php }else{?>
                                    <a  class="ajax" data-price="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo $rand. md5($rowk->bnsprof_id); ?>&pid=<?php echo $rowk->pd_id; ?>&keywords=<?php echo urlencode($_GET['keywords']);?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1" class="txt-bold txt-black pull-right inquiry_but" id="btn_ajax_send<?php echo $rowk->pd_id; ?>" rel="product-send-inquiry">
...
<?php
 $Countryphone =mysql_fetch_array(mysql_query("SELECT * FROM `country` where cn_id = ".$data['country'])); ?>
                                              <td class="padding-0"><big class=""><img src="images/mobile.png" width="25px"/> &nbsp;<a href="#" class="txt-black txt-lg"><b><?php echo $Countryphone['cn_ph']; //echo user_info($data['bnsprof_uid'],'country_ph_code');?>-<?php echo user_info($data['bnsprof_uid'],'mobile1');//echo $data['bnsprof_mobile2']; //echo user_info($data['bnsprof_uid'],'mobile1');?> </b></a></big></td>
                                              <td class="text-right padding-0">
                                               <?php if(($_SESSION['uid_indm'] )==''){ ?>
                                  <a data-enquiry="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/sign-in.php"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry" style="font-weight:bold;">Send Enquiry</button></a> 
                                    <?php }else{ 
									if(($_GET['grid'])=='active'){				
									?>
                                    <a  class="ajax" data-price="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo $rand. md5($rowk->bnsprof_id); ?>&pid=<?php echo $rowk->pd_id; ?>&keywords=<?php echo urlencode($_GET['keywords']);?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1" class="txt-bold txt-black pull-right inquiry_but" id="btn_ajax_send<?php echo $rowk->pd_id; ?>" rel="product-send-inquiry">
                          ...
                          <div class="col-lg-4 box-3">
                              <div class="ar-box-1 ar-box padding-5 margin-bottom-5 bg-gray" style="overflow-x: hidden;">
                                  <header class="sub-box"> <?php if($fevrow_icon){ ?> <a href="http://www.arabyos.com/membership_plans.php"><img src="http://www.arabyos.com/admin/images/<?php echo $fevrow_icon['producticon']; ?>" title="<?php echo $fevrow_icon['pplan']; ?>" width="25px" height="25px"/></a> <?php } ?><b class="txt-dark-gray"> 
                                  ...
                                  <img src="http://www.arabyos.com/images/country_flag/<?php echo $rowk->cn_flag; ?>" alt="<?php echo $rowk->cn_flag; ?>" style=" width:21.6px; height:21.6px;"/>
                                  ...
                                  <table class="table margin-top-5">
                                      <tr >
										...
										$i++;
                                          ...
                                          //$userArrayRow_Type ?> </td>
                                      </tr>
                                      <tr>
                                          <td class="txt-light-gray padding-0"> <a style="font-size:12px;font-weight:100;color:#8a8a8a" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/product-add.php">Trade Location:</a> </td>
                                          <td class="padding-0 txt-bold"> <a style="font-size:12px;color:#242424" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/product-add.php"><?php if(($rowk->pd_preferred_buyer_location)=='abroad'){echo "Abroad Only";}else if(($rowk->pd_preferred_buyer_location)=='any'){echo "Abroad + Domestic";}else if(($row->pd_preferred_buyer_location)=='domestic'){echo "Domestic Only";}else if(($row->pd_preferred_buyer_location)=='my_city'){echo "My City Only";} ?></a> </td>
                                      </tr>
                                      <tr>
              ...
              while($rrow = mysql_fetch_array($rresult, MYSQL_ASSOC)){
                ...
                $releted_pro.= '<td class="padding-0 small-box-td1"><div class="thumb" style="padding-right:3px; width:106px;"></div>
                     <a href="search.php?keyword_type='.$_GET['keyword_type'].'&keywords='.$rrow['pd_title'].'&rctyp=Products"><img  data-toggle="tooltip" data-placement="top" title="'.$rrow['pd_title'].'" class="photo" src="./upload/myproduct/'.$rrow['pd_image'].'" data-large_photo="upload/myproduct/'.$rrow['pd_image'].'"/></a></td>';
                }
                else{
		...
		if($suppRowCount == $suppTotalRow){
			...
			if($totitems > 10){
				echo '<div class="col-lg-12 text-center" style="padding:30px;"><a href="http://arabyos.com/search.php?rctyp='.$_GET['rctyp'].'&keywords='.$_GET['keywords'].'&page='.$pages.'"><button type="button" class="btn btn-md btn-warning border-radius-0 btn-enquiry" style="font-size:16px; font-weight:bolder;">Display More Products / Services </button></a></div>';
			}
		}
    $suppRowCount ++;
	} ?>
	...
	{	
		...
		*/
		...
		if(isset($_POST['mst_type']))
		{
			...
			{ 	if($k == 1){ $keywordmem .= "  ( p_id = '".$value."'";  }
				else{$keywordmem .= " or p_id = '".$value."'";}
				$k++;
			}
		if(isset($_COOKIE['loc_id']))
		...
		if(isset($minQty) && $minQty > 0){
			...
			if($_GET['keywords']!=""){
				...
				{ 	if($p == 1){ $cntryval .= " and (user.country = '".$value."'";  }
					else{$cntryval .= " or user.country = '".$value."'";}
					$p++;
				}
				$newkw=generateProdSearchString($keywords);
	 ...
	 $getSearchCount =  mysql_num_rows($run_query);   
        ...
        //  print_r($fevrow_icon);
           ...
           ?>
              ...
              <div class="row ar-mid-box">
                  <div class="col-lg-11 col-sm-11 col-md-11 ar-box-1  margin-top-10 ">
                      <div class="row">
                          <div class="col-lg-3 big-img-box box-1" id="div-<?php echo $row['pd_id'];?>">
                              <header>
                              <?php if(isset($_SESSION['uid_indm']) && $_SESSION['uid_indm'] != ''){ ?>
                                <a href="javascript:void(0)" class="product_fav_btn" data="<?php echo $row['pd_id']; ?>"  onclick="return showfavorite(<?php echo $row['pd_id'];?>)" class="ar-star"><i class="fa fa-star star" style="color:<?php echo (in_array($row['pd_id'], $myfev))?'#E48F23':'#808080'; ?>"></i> Favourite</a>
                              <?php  }
                              else{ ?>
                                <a href="sign-in.php" class="product_fav_btn" data="<?php echo $row['pd_id']; ?>" class="ar-star"><i class="fa fa-star star"></i> Favourite</a>
                              <?php }  ?>
                              <a href="javascript:void(0)" class="ar-star product_compare" data-prod_img="<?php echo 'http://www.arabyos.com/upload/myproduct/'.$row['pd_image']; ?>" onClick="return addcompare(<?php echo $row['pd_id'];?>)" data-prod_id="<?php echo $row['pd_id']; ?>" title="<?php echo $row['pd_title']; ?>" ><i class="fa fa-plus star" ></i> Add Compare</a> </header>
                              <figure class="box" >
                              <?php if($fevrow_icon){ ?>
                                  <div class="ribbon">  <img src="./admin/images/<?php echo $fevrow_icon['sponsericon']; ?>"/> </div>
                                  <?php } ?>
                                  <!--<img src="images/3.png"  class="zoomthis" alt="Rice"/>-->
<?php $pimg= explode(',',$row['pd_image']); $limg=explode(',',$row['pd_imagelogo']) ?>
                                  <div class="zoomthis"> <?php  echo "<img src='/upload/myproduct/".$pimg[0]."'>"; ?></div>
       ...
       <?php  echo "<img style='width: 77px; height: 77px;' src='/upload/myproduct/".$limg[0]."'>"; ?></div>
                 <?php  } ?>             </figure>
                          ...
                          <div class="col-lg-5 box-2">
                              <ul>
                                  <li class="margin-bottom-10">
                                      <h4 class="txt-blue"><a class="txt-blue" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/product-details.php?token=<?php echo rand(1000,9999).md5($row['pd_id']) ?>&c=<?php echo rand(1000,9999).md5($data['bnsprof_id']); ?>" target="_blank"><?php echo $row['pd_title']; ?></a></h4>
                                  </li>
                                  <li>
                                      <?php echo substr($row['pd_desc'],0,132); ?>
                                  </li>
                                  <li class="text-right"> <a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/product-details.php?token=<?php echo rand(1000,9999).md5($row['pd_id']) ?>&c=<?php echo rand(1000,9999).md5($data['bnsprof_id']); ?>" target="_blank">+  More</a> </li>
...
$symbol;
?>                                                        
                                  <li> Fob Price  &nbsp; <big class="txt-bold txt-red"><a style="color: #d22027" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/product-add.php"><?php echo $symbol.$row['pd_fob_price'].'~ '.$symbol.$row['pd_fob_price2']; ?> </a></big> &nbsp;  
                                <?php if(($_SESSION['uid_indm'] )==''){ ?>
                                  <a data-price="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/sign-in.php">(Get Letest Price)</a> 
                                    <?php }else{ 
									$geo_loc= $location_geo_country[0];
									$countryyyy = $_COOKIE['loc_id'];
								 if($_GET['keywords']!=""){
									?>
                                    <a  class="ajax" data-enquiry="" id="btn_ajax_send<?php echo $row['pd_id']; ?>" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&keywords=<?php echo urlencode($_GET['keywords']);?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1">(Get Letest Price)</a>
                                    <?php }//keyword 
									else {?>
                                      <a data-enquiry=""  class="ajax" id="btn_ajax_send<?php echo $row['pd_id']; ?>" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&keywords=<?php echo urlencode($_GET['keywords']);?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1">(Get Letest Price)</a> 
                                   <?php }//no keyword
								   		} ?> 
                                    </li>
                                  <li class="margin-top-5">
								 ...
								 <!--  <td style="padding-left:0px;"><a href="<?php echo $data['bnsprof_compname']?>/products.php?c=<?php echo rand(1000,9999).md5($data['bnsprof_id']); ?>&sc=<?php echo rand(10000,99999).$data['pd_subcat_id']; ?>#<?php echo $row['pd_id']; ?>" class="txt-blue txt-bold"><img src="images/users.png" width="25px"/> About Us</a></td>
                                  ...
                                  <a data-enquiry="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/sign-in.php"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry" style="font-weight:bold;">Send Enquiry</button></a> 
									...
									if(($_GET['grid'])=='active'){				
									?>
                                    <a data-enquiry="" id="btn_ajax_send<?php echo $row['pd_id']; ?>" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/?&keywords=<?php echo $_GET['keywords'];?>&grid=active"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry" style="font-weight:bold;">Send Enquiry</button></a>
                                    <?php }else{
											$geo_loc= $location_geo_country[0];
									$countryyyy = $_COOKIE['loc_id'];
                                     if($_GET['keywords']!=""){
									?>
                                    <a data-enquiry=""  class="ajax" id="btn_ajax_send<?php echo $row['pd_id']; ?>" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&keywords=<?php echo urlencode($_GET['keywords']);?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry" style="font-weight:bold;">Send Enquiry</button></a>
                                    <?php }else{ ?>
                                    <a data-enquiry=""  class="ajax" id="btn_ajax_send<?php echo $row['pd_id']; ?>" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry" style="font-weight:bold;">Send Enquiry</button></a>                                    
                                    <?php } ?>
                                   <?php } } ?>          
                                              </td>
                                          </tr>
                          ...
                          <div class="col-lg-4 box-3">
                              <div class="ar-box-1 ar-box padding-5 margin-bottom-5 bg-gray" style="overflow-x: hidden;">
                                  <header class="sub-box"><?php if($fevrow_icon){ ?><a href="http://www.arabyos.com/membership_plans.php"><img src="./admin/images/<?php echo $fevrow_icon['producticon']; ?>" width="25px" height="25px" title="<?php echo $fevrow_icon['pplan']; ?>"/></a> <?php } ?><b class="txt-dark-gray"> 
								...
								//echo"<pre>";	print_r($countryId); //die;
									...
									<? } else { $getCountryflag = mysql_fetch_array(mysql_query("SELECT * FROM `country` where cn_id='".$_COOKIE['loc_id']."'")); ?>
                                  ...
                                  <table class="table margin-top-5">
                                      ...
                                      <tr>
                                          <td class="txt-light-gray padding-0"> <a style="font-size:12px;font-weight:100;color:#8a8a8a" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/product-add.php" target="_blank">Trade Location :</a> </td>
                                          <td class="padding-0 txt-bold"> <a style="font-size:12px;color:#242424" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/product-add.php"><?php if($row['pd_preferred_buyer_location']=='abroad'){echo "Abroad Only";}else if($row['pd_preferred_buyer_location']=='any'){echo "Abroad + Domestic";}else if($row['pd_preferred_buyer_location']=='domestic'){echo "Domestic Only";}else if($row['pd_preferred_buyer_location']=='my_city'){echo "My City Only";} ?></a> </td>
                                      </tr>
                                      <tr>
                                          <td class="txt-light-gray padding-0" width="95"> Member Since : </td>
...
//$row['pd_uid']
			...
			$rqury = 'select pd_id,pd_title,pd_imagelogo, pd_image,pd_subcat_id from products where pd_image!="" and pd_uid = '.$row['pd_uid'].' and pd_status = "1" and pd_id !='.$row['pd_id'].' ORDER by pd_id DESC limit 2';
              ...
              while($rrow = mysql_fetch_array($rresult, MYSQL_ASSOC)){
                ...
                $releted_pro.= '<td class="padding-0 small-box-td1"><div class="thumb" style="padding-right:3px;"></div>
  <a href="search.php?keyword_type='.$_GET['keyword_type'].'&keywords='.$rrow['pd_title'].'&rctyp=Products"><img data-toggle="tooltip" data-placement="top" title="'.$rrow['pd_title'].'" class="photo" src="./upload/myproduct/'.$imgarr[0].'"  data-large_photo="upload/myproduct/'.$imgarr[0].'"/></a>';

                     if($rrow['pd_imagelogo']){

                     $logoarr = explode(',',$rrow['pd_imagelogo']);
                  
                   $releted_pro.=  '<a href="search.php?keyword_type='.$_GET['keyword_type'].'&keywords='.$rrow['pd_title'].'&rctyp=Products"><img data-toggle="tooltip" data-placement="top" title="'.$rrow['pd_title'].'" style="position: absolute; bottom:5px; width: 42px;height: 41px;" class="photo" src="./upload/myproduct/'.$logoarr[0].'" data-large_photo="upload/myproduct/'.$logoarr[0].'"/></a>';
                }
                   ...
                   $releted_pro.=   '</td>';
                	...
                	$imgarr = explode(',',$rrow['pd_image']);
                 $releted_pro.= '<td class="padding-0 text-right small-box-td2"><div class="thumb" style="padding-left:3px;"></div>
                      <a href="search.php?keyword_type='.$_GET['keyword_type'].'&keywords='.$rrow['pd_title'].'&rctyp=Products"><img data-toggle="tooltip" data-placement="top" title="'.$rrow['pd_title'].'" class="photo" src="./upload/myproduct/'.$imgarr[0].'"  data-large_photo="upload/myproduct/'.$imgarr[0].'"/></a>';

                     if($rrow['pd_imagelogo']){

                     $logoarr = explode(',',$rrow['pd_imagelogo']);
                   $releted_pro.=  '<a href="search.php?keyword_type='.$_GET['keyword_type'].'&keywords='.$rrow['pd_title'].'&rctyp=Products"><img data-toggle="tooltip" data-placement="top" title="'.$rrow['pd_title'].'" style="position: absolute; bottom:5px; width: 42px;height: 41px;    left: 134px;" class="photo" src="./upload/myproduct/'.$logoarr[0].'" data-large_photo="upload/myproduct/'.$logoarr[0].'"/></a>';
             }
           ...
           ?>
            <div class="col-md-4 compared-box compared-box1 style_prevu_kit">
            	...
            	<header style="padding:5px;" class="titleLim">
                   ...
                   <a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/<?php echo 'company';?>/product-details.php?token=<?php echo rand(1000,9999).md5($row['pd_id']) ?>&c=<?php echo rand(1000,9999).md5($data['bnsprof_id']); ?>" target="_blank" class="h4" style="font-weight:bold;"><?php echo $row['pd_title']; ?></a>
                </header>
                ...
                <figure class="img-box" >
                    ...
                    <div class="ara-links">
                    	<a href="javascript:void(0)"  onclick="return showfavorite(<?php echo $row['pd_id'];?>)"><i class="fa fa-star"></i> Favourite</a>
                        <a href="javascript:void(0)" onClick="return showcompare(<?php echo $row['pd_id'];?>)"><i class="fa fa-plus"></i> Compare</a>
                    </div>
                    <?php $pimg2= explode(',',$row['pd_image']);  ?>
                	<div class="zoomthis">  <?php  echo "<img src='/upload/myproduct/".$pimg2[0]."' class='zoomthis' alt='/upload/myproduct/".$pimg2[0]."'>"; ?> <!--<img src="images/3.png" class="zoomthis" alt="Rice"/>--> </div>
<?php if(!empty($row['pd_imagelogo'])){ 
       ...
       <?php  echo "<img style='width: 77px; height: 77px;' src='/upload/myproduct/".$limg2[0]."'>"; ?></div>
                ...
                <section>
					...
					<?php } ?>
                        ...
                        <tr>
                        	<td><a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/product-add.php"><!--Trade Location:--></a> </td>
                        	<td colspan="2"></td>
                            <!-- <td colspan="2"><a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/product-add.php" style="color:#777"><?php if($row['pd_preferred_buyer_location']=='abroad'){echo "Abroad Only";}else if($row['pd_preferred_buyer_location']=='any'){echo "Abroad + Domestic";}else if($row['pd_preferred_buyer_location']=='domestic'){echo "Domestic Only";}else if($row['pd_preferred_buyer_location']=='my_city'){echo "My City Only";} ?></a></td> -->
                        </tr>
                        <?php /*?><tr>
...
$symbol;
?>
                                          <big class="txt-bold txt-red"><a style="color: #d22027" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/product-add.php"> <?php echo $symbol.$row['pd_fob_price'].'~ '.$symbol.$row['pd_fob_price2']; ?></a></big>
                                          <!--<a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>" class="txt-bold txt-black pull-right" id="btn_ajax<?php echo $row['pd_id']; ?>">(Get Letest Price)</a>-->
                          <?php /*?>                
                                 <?php if(($_SESSION['uid_indm'] )==''){ ?>
                                  <a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/arabyos/sign-in.php">(Get Letest Price)</a> 
                                    <?php }else{ 
										$geo_loc= $location_geo_country[0];
									$countryyyy = $_COOKIE['loc_id'];
									?>
                                    <a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>" class="txt-bold txt-black pull-right" id="btn_ajax<?php echo $row['pd_id']; ?>">(Get Letest Price)</a> 
                                   <?php } ?>   <?php */?>       
                                          </td>
                        </tr>
						<tr>
...
<?php */?>						</tr>
                        <tr>
                           ...
                           <?php /*?> <td colspan="2"><img src="images/mobile.png" width="25px"/>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="txt-black h4"><?php echo user_info($data['bnsprof_uid'],'bnsprof_phcode1');?>-<?php echo user_info($data['bnsprof_uid'],'mobile1');?></a> </td><?php */?>
                            ...
                            <?php
 $Countryphone =mysql_fetch_array(mysql_query("SELECT * FROM `country` where cn_id = ".$data['country'])); ?>
                                              <td colspan="2"><img src="images/mobile.png" width="25px"/> &nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="txt-black h4"><b><?php echo $Countryphone['cn_ph']; ?>-<?php echo user_info($data['bnsprof_uid'],'mobile1');//$data['bnsprof_mobile2']; ?> </b></a></td>
                        </tr>
                        <tr>
                        	<td></td>
                            <td>
                             <script>
                                   ...
                                   $("#btn_ajax_send<?php echo $row['pd_id']; ?>").colorbox({iframe: true, width: "62%", height: "89%"});
                                               ...
                                               <?php if(($_SESSION['uid_indm'] )==''){ ?>
                                  <a data-enquiry="" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/sign-in.php"><button class="btn btn-sm btn-default btn-enquiry1 border-radius-0"><span style="font-weight:bold;">Send Enquiry</span></button></a> 
                                    <?php }else{ 
									$geo_loc= $location_geo_country[0];
									$countryyyy = $_COOKIE['loc_id'];
									if(($_GET['grid'])=='active'){				
									?>
                                    <a data-enquiry=""  class="ajax" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1" id="btn_ajax_send<?php echo $row['pd_id']; ?>"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry" style="font-weight:bold;">Send Enquiry</button></a>
                                    <?php }else{?>
      <a data-enquiry=""  class="ajax" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/company/quotationRequest_supplier.php?id=<?php echo rand(1000, 9999) . md5($data['bnsprof_id']); ?>&pid=<?php echo $row['pd_id']; ?>&geo=<?php echo $geo_loc;?>&conty=<?php echo $countryyyy;?>&search=1"><button type="button" class="btn btn-sm btn-warning border-radius-0 btn-enquiry"  id="btn_ajax_send<?php echo $row['pd_id']; ?>" style="font-weight:bold;">Send Enquiry</button></a>
                                   <?php } } ?>  
                            </td>
                            <td><a href="company/products.php?c=<?php echo rand(1000,9999).md5($data['bnsprof_id']); ?>&sc=<?php echo rand(10000,99999).$data['pd_subcat_id']; ?>#<?php echo $row['pd_id']; ?>">Chat<img src="images/chat.png" style="width:20px; height:20px; margin-left:5px;"/></a></td>
                        </tr>
                    </table>
                </section>
            </div>
             <?php 
		  		if($gridRecCount == $totalgridpage){
					...
					echo '<div class="col-lg-12 text-center" style="padding:30px;"><a href="http://arabyos.com/search.php?keywords='.$_GET['keywords'].'&grid='.$_GET['grid'].'&page='.$pages.'"><button type="button" class="btn btn-md btn-warning border-radius-0 btn-enquiry" style="font-size:16px; font-weight:bolder;">Display More Products / Services </button></a></div>';
				}
				$gridRecCount++;
			 }  ?>
            <?php }else{
...
if($chrome){
	if($_SESSION['uid_indm'] != ''){
		$style = 'style="left:-149px;"';
	}else{
	$style = 'style="left:-115px;"';
	}
}else{
	if($_SESSION['uid_indm'] != ''){
		$style = 'style="left:-149px;"';
	}else{
		$style = 'style="left:-116px;"';
	}
}
?>
			...
			<div class="col-lg-12 popup-sub-box">
			  ...
			  <section class="col-lg-12">
				...
				<div class="col-lg-12 margin-top-10 margin-bottom-10" style="padding: 0px; text-align: left;white-space: nowrap;z-index:9999;">
				  ...
				  <b style="padding:3px 5px; font-size:15px; font-weight:normal;">If these supplier don't contact me, recommend matching suppliers.</b>
				</div>
