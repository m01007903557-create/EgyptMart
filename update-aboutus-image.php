<?php
/**
 * File: update-aboutus-image.php

 * Description: نافذة رفع وتحرير صور "من نحن" (About Us)
...
if (!isset($_GET['abtid']) || !is_numeric($_GET['abtid'])) {
    die("Invalid about us ID");
}

$abtid = (int)$_GET['abtid'];
?>
<!DOCTYPE html>
...
<head>
    ...
    <script>
    function list_photo() {
        $.get("list_photo_edit.php", {'abtid': <?php echo $abtid; ?>}, function(data) {
            $('#list_photo').html(data);
        });
    }

    function DelabtImage(imid) {
        if (confirm("Do you really want to remove this image?")) {
            $.get("del_image_edit.php", {imid: imid}, function(data) {
                list_photo();
            });
        }
    }
    ...
    $(function() {
        $('#file_upload').uploadifive({
            ...
            'uploadScript': 'upload-image-edit.php',
        ...
        <div class="file_button" style="margin:0px;" id="addbut">
            ...
            <div id="buttons" onMouseOver="showImgButt();" onMouseOut="hideImgButt();">
                <div id="file-wrapper" class="title" title="upload images from your computer" style="">
                    <div style="display:block;" class="newAddimage cpr" id="old_img_f" align="center">
