...
// تحديد category_p_id من token أو pc_id
$category_p_id = 0;

if (isset($_GET['token'])) {
    $token = $_GET['token'];
    $md5_part = substr($token, -32);
    
    $sql_token = "SELECT pc_id FROM product_category WHERE MD5(pc_id) = '$md5_part'";
    $res_token = mysqli_query($con, $sql_token);
    
    if ($res_token) {
        $row_token = mysqli_fetch_assoc($res_token);
        if ($row_token) {
            $category_p_id = $row_token['pc_id'];
        }
    }
}

if ($category_p_id == 0 && isset($_POST['pc_id'])) {
    $category_p_id = intval($_POST['pc_id']);
}

if ($category_p_id == 0 && isset($_GET['pc_id'])) {
    $category_p_id = intval($_GET['pc_id']);
}

if ($category_p_id == 0) {
    die("Error: Category ID not found. Token: " . htmlspecialchars($_GET['token'] ?? 'none'));
}
...
if(isset($_COOKIE['loc_id']))
{
    ...
    or
    pd_preferred_buyer_location='my_state' and pd_uid in(select distinct bnsprof_uid from business_profile bf JOIN states s ON bf.bnsprof_state = s.state_id where state_cn_id='$loc_id')))";
}
else
{
...
$token = isset($_GET['token']) ? substr($_GET['token'], 4) : '';

$sql_scat = "select * from product_category where md5(pc_id)='".mysqli_real_escape_string($con, $token)."' and pc_status ='1'";
$res_scat = mysqli_query($con, $sql_scat);
$row_scat = mysqli_fetch_object($res_scat);
$category_p_id = $category_p_id ?? 0;
?>
...
<head>
    <title><?php echo getSiteTitle(); ?> :: Suppliers - <?php echo htmlspecialchars($row_scat->pc_name ?? ''); ?></title>
        ...
        <div class="a1 wd cl" align="left">
            ...
            <div class="maincontainertop ps-15">
                <div class="brdcrm">
                    <ul>
                        <!-- Groups start-->
                        <li> <a href="dir.php" target="_top" itemprop="url"><img class="h_imgin bg nr" alt=""
                                    src="images/zero.gif" style="padding-right: 5px;" height="20px" width="24px"><span
                                    itemprop="title">دليل الموردين</span></a><span><img src="images/zero.gif"
                                    alt="" class="brdcrm-arwin_pgen bg nr" height="16px" width="16px"></span> </li>
                        <!-- Groups end-->
                        <li style="border:none;font-size:13px;color:#444; display:inline;">
                            <?php echo htmlspecialchars($row_scat->pc_name ?? ''); ?></li>
                    </ul>
...
$_GET['pc_id'] = $category_p_id;  // أضف هذا السطر
if (file_exists('index_leftsidebar_catcompany.php')) {
...
<div class="col-md-8 col-sm-8 col-xs-12 middle-part">
    <button type="button"
        class="btn btn-default border-radius-0 txt-bold bold-xs btn-white text-capitalize <?php echo $pc_id ?? ''; ?>"
        style="border-top:2px solid #ff7519 !important;border:0px;font-weight:700;">موردون</button>
    <button type="button"
        class="btn btn-default border-radius-0 txt-bold bold-xs btn-white text-capitalize"
        style="background-color: #F5F7FA"><a
            href="products.php?c=<?php echo md5((string)$category_p_id); ?>"
            style="color:#000;font-weight: 700;" target="_new">منتجات</a></button>
    <?php

   $country_filter = "";
if (isset($_POST['country_id']) && is_array($_POST['country_id']) && count($_POST['country_id']) > 0) {
    $country_ids = array_map('intval', $_POST['country_id']);
    $country_filter = " AND u.country IN (" . implode(',', $country_ids) . ") ";
} 
 
    $state_filter = "";
if (isset($_POST['state_id']) && is_array($_POST['state_id']) && count($_POST['state_id']) > 0) {
    $state_ids = array_map('intval', $_POST['state_id']);
    $state_filter = " AND bf.bnsprof_state IN (" . implode(',', $state_ids) . ") ";
}
     $sql_comp = "SELECT * FROM business_profile bf 
             ...
             WHERE pc_parent_id = '" . mysqli_real_escape_string($con, $category_p_id) . "'  
    ...
    <div class="my-container" style="width:auto!important;">
        <div class="row">
            <div class="col-md-1" style="width:4%">
                <?php if(!empty($rowMem->mst_icon)) { ?>
                <img src="admin/images/<?php echo htmlspecialchars($rowMem->mst_icon); ?>"
                    title="<?php echo htmlspecialchars($rowMem->mst_name ?? ''); ?>" width="30px" height="30px">
                <?php } ?>
            </div>
                                <div class="col-md-6">
                                    <a style=""
                                        <?php if(user_info($row_comp->bnsprof_uid,'bnsprof_compname') != ''){ ?>href="company/profile.php?c=<?php echo rand(1000,9999) . md5((string)$row_comp->bnsprof_id); ?>"
                                        <?php } ?> target="_blank">
                                        <span class="state-country"
                                            style="font-weight:900;"><?php echo htmlspecialchars($row_comp->bnsprof_compname ?? ''); ?></a><br>
                                    <?php echo htmlspecialchars($rowCountry->cn_name ?? ''); ?>,
                                    <?php echo htmlspecialchars(get_state_name($row_comp->bnsprof_state ?? 0)); ?>
                                </div>
                            </div>
                            <div class="row" style="padding-top: 10px;">
                                <div class="three-pics1">
                                    <?php 
                      $get_product = mysqli_query($con, "SELECT * FROM `products` WHERE pd_uid = '".(int)$row_comp->bnsprof_uid."' GROUP BY `pd_subcat_id` LIMIT 3");
                      
                      while ($run = mysqli_fetch_object($get_product)) { ?>
                                    <div class="col-md-2">
                                        <?php   
                        if(!empty($run->pd_image)){
                          //echo $row_comp->pd_image;die();
                        ?>
                                        <div style="width:100%;">
                                            <a href="company/products.php?c=<?php echo rand(1000,9999) . md5((string)$row_comp->bnsprof_id); ?>"
                                                target="_blank">
                                                <div class="cat_image_div zoom-box">
                                                    <img src="/upload/myproduct/<?php echo htmlspecialchars($run->pd_image); ?>"
                                                        title="<?php echo htmlspecialchars($run->pd_title ?? ''); ?>" class="cat_image">
                                                </div>
                                            </a>
                                            <br>
                                            <span
                                                style="font-size:11px;color:#37366d ;font-weight:0px;"><?php echo htmlspecialchars(wordwrap(substr($run->pd_title ?? '', 0, 30), 17, "<br/>", true)); ?></span>
                                        </div>
                                        <?php } else { ?>
                                        <div style="width:100%;">
                                            <a href="company/products.php?c=<?php echo rand(1000,9999) . md5((string)$row_comp->bnsprof_id); ?>"
                                                target="_blank">
                                                <div class="cat_image_div zoom-box"><img src="/images/noimage.jpg"
                                                        class="cat_image" title="<?php echo htmlspecialchars($row_comp->pd_title ?? ''); ?>">
                                                </div>
                                            </a>
                                            <br>
                                            <span
                                                style="font-size:11px;color:#37366d ;font-weight:0px;"><?php echo htmlspecialchars(substr($run->pd_title ?? '', 0, 15)); ?></span>
                                        </div>
                                        <?php }  ?>
                                    </div>
                                    <?php } ?>
                      ...
                      echo htmlspecialchars(stripslashes($row_wc->wc_homepage_key_desc ?? ''));
                      ?>
                                    </b>
                                    <br>
                                    <a href="company/profile.php?c=<?php echo rand(1000,9999) . md5((string)$row_comp->bnsprof_id); ?>"
                                        target="_blank" class="fc td j1 g4 bg im3 lht pb2">About Us</a> &nbsp;&nbsp;<a
                                        href="company/products.php?c=<?php echo rand(1000,9999) . md5((string)$row_comp->bnsprof_id); ?>"
                                        target="_blank" class="fc td j1 g4 bg im4 pb2 lht">View Products</a>
                                    <br>
                                    <b>Main Buisness : </b>
                                    <?php
                      $string_buisnessmain = "";
                      ...
                      while($row_user_products = mysqli_fetch_object($result_user_products)){
                         ...
                         $string_buisnessmain .= isset($rowBuisness_main) ? $rowBuisness_main->pc_name . "," : "";                      }
                      ...
                      ?>
                                    ...
                                    <a class="bg ima z1 e4 hl a2 td bo f2 wdt c6 td ajax" rel="nofollow"
                                        href="sendenquiry-form.php?id=<?php echo rand(1000,9999) . md5((string)$row_comp->bnsprof_id); ?>">Send
                                        Enquiry</a>
                                    <?php } ?>
                                    <p class="g1">
                                        <b>Address:</b>&nbsp;
                                        ...
                                        <?php if(!empty($row_comp->bnsprof_state) && $row_comp->bnsprof_state != '0'){ echo htmlspecialchars(get_state_name($row_comp->bnsprof_state)) . ", "; } ?>
                                        <?php if(!empty($row_comp->bnsprof_state) && $row_comp->bnsprof_state != '0'){ echo htmlspecialchars(get_state_name($row_comp->bnsprof_state)) . " "; } ?>
                                        <br>
                                        <?php
                        ...
                        ?>
                                        ...
                                        <?php if(!isset($_SESSION['uid_indm']) || $_SESSION['uid_indm'] == ''){
                                            ...
                                            if(!empty($rowBuisness_user_details->mobile1)){ ?>
                                        <a href="tel:+<?php echo htmlspecialchars($rowBuisness_user_details->mobile1); ?>"><span
                                                id="pns1">+</span><?php echo htmlspecialchars($rowBuisness_user_details->mobile1); ?></a><img
                                            alt="" style="position:absolute;margin-top:0px; margin-left:5px;"
                                            class="bg nr pnsPos1" src="images/zero.gif" height="20" width="20"><br>
                                        <?php } }?>
                                        <?php if(!empty($row_comp->bnsprof_mobile2)){ ?>
                                        <a href="tel:+<?php echo htmlspecialchars($row_comp->bnsprof_mobile2); ?>"><span
                                                id="pns1">+</span><?php echo htmlspecialchars($row_comp->bnsprof_mobile2); ?></a><img
                                            alt="" style="position:absolute;margin-top:0px; margin-left:5px;"
                                            class="bg nr pnsPos1" src="images/zero.gif" height="20" width="20"><br>
                                        <?php } ?>
                                        <?php if(!empty($row_comp->bnsprof_mobile3)){ ?>
                                        <a href="tel:+<?php echo htmlspecialchars($row_comp->bnsprof_mobile3); ?>"><span
                                                id="pns1">+</span><?php echo htmlspecialchars($row_comp->bnsprof_mobile3); ?></a><img
                                            alt="" style="position:absolute;margin-top:0px; margin-left:5px;"
                                            class="bg nr pnsPos1" src="images/zero.gif" height="20" width="20"><br>
                                        <?php } ?>
                                        <?php if(!empty($row_comp->bnsprof_mobile4)){ ?>
                                        <a href="tel:+<?php echo htmlspecialchars($row_comp->bnsprof_mobile4); ?>"><span
                                                id="pns1">+</span><?php echo htmlspecialchars($row_comp->bnsprof_mobile4); ?></a><img
                                            alt="" style="position:absolute;margin-top:0px; margin-left:5px;"
                                            class="bg nr pnsPos1" src="images/zero.gif" height="20" width="20">
                                        <?php } ?>
                                        <?php } ?>
                                        <span id="sms1" class="tool-tip-lg soff"><span
                                                class="bg nr toolarw"></span><span
                                                class="txt-call">&nbsp;</span></span><br>
                                        <?php if(!empty($row_comp->bnsprof_website_alt)){ ?>
                                        <b>Websites:</b>
                                        <?php if(!isset($_SESSION['uid_indm']) || $_SESSION['uid_indm'] == ''){
                                          } else { ?>
                                        <a target="_blank"
                                            href="https://<?php echo htmlspecialchars($row_comp->bnsprof_website_alt); ?>"><?php echo htmlspecialchars($row_comp->bnsprof_website_alt); ?></a><br>
                                        <?php } }?>
                                    </p>
                                </div>
                            </div>
                ...
                if (isset($_COOKIE['loc_id'])) {
                    ...
                    $sql_pd_ck = " and (
                        ...
                        (pd_preferred_buyer_location='any' and pd_uid in(select distinct usr_id from user where country='$loc_id'))
                        or
                        (pd_preferred_buyer_location='my_state' and pd_uid in(select distinct bnsprof_uid from business_profile where bnsprof_state=(select state_id FROM states where state_cn_id='$loc_id' LIMIT 1))))";
                    ...
                    $sql_so_ck = " and (
                        ...
                        or
                        (so_preferred_buyer_location='my_state' and so_usr_id in(select distinct bnsprof_uid from business_profile where bnsprof_state=(select state_id FROM states where state_cn_id='$loc_id' LIMIT 1))))";
                    ...
                    $sql_br_ck = " and ((br_preferred_supplier_location='domestic' and br_u_id in(select distinct usr_id from user where country='$loc_id'))
                        ...
                        or
                        (br_preferred_supplier_location='my_state' and br_u_id in(select distinct bnsprof_uid from business_profile where bnsprof_state=(select state_id FROM states where state_cn_id='$loc_id' LIMIT 1))))";
                ...
                { 
                    ...
                    if($totalbaneer > 2){
                        ...
                        ?>
                        <p
                            style="font-size: 16px;margin-top: 35px; font-color: blue;text-align:center; margin-bottom: 10px;">
                            <b> موردون رواد</b>
                        </p>
                        <div class="demobox">
                            ...
                            while($rowleading = mysqli_fetch_object($rsleading))
                             ...
                             ?>
                                                <a class="slick-product-wrapper"
                                                    href="company/products.php?c=<?php echo rand(1000, 9999) . md5((string)($row_bprof->bnsprof_id ?? '')); ?>&sc=<?php echo rand(10, 99999) . $rowleading->pd_subcat_id; ?>#<?php echo $rowleading->pd_id; ?>"
                                                    target=_blank>
                                                    <div class="demobox">
                                                        <div class="slick-product-image ">
                                                            <img alt=""
                                                                src="upload/myproduct/<?php echo htmlspecialchars($rowleading->pd_image); ?>"
                                                                class="black"
                                                                style="margin: auto;border: 1px solid #E9E9E9!important;"
                                                                title="<?php echo htmlspecialchars(ucwords($rowleading->pd_title ?? '')); ?>">
                                                        </div>
                                                        <div class="matterbox">
                                                            <div class="icon-pic-with-heading">
                                                                <div class="icon_pic"><img alt=""
                                                                        src="images/<?php echo htmlspecialchars($adv_icon); ?>"
                                                                        class="img-responsive" width="18"></div>
        ...
        <div class="wd6 z1 cr hide" id="hdiv">
            ...
            if(mysqli_num_rows($res_adv) > 0)
            {
                ...
                ?>
                <a href="//<?php echo htmlspecialchars($row_adv->adv_link ?? ''); ?>" target="_blank"><img
                        src="upload/advertisement/<?php echo htmlspecialchars($row_adv->adv_img ?? ''); ?>" width="300" height="300" /></a>
                <?php
            }
            else
            {
