<?php
/**
 * File: product-tempimage.php

 * Description: رفع صورة مؤقتة للمنتج (للإستخدام في إضافة منتج جديد)
...
try {
    ...
    $check_sql = "SELECT COUNT(*) as count FROM temp_products_image WHERE tmpimg_uid = ?";
    ...
    $insert_sql = "INSERT INTO temp_products_image (tmpimg_uid, tmpimg_image) VALUES (?, ?)";
    ...
    error_log("Upload Temp Product Image Error: " . $e->getMessage() . " | User ID: $uid");
    http_response_code(400);
    echo "0|" . $e->getMessage();
} catch (Exception $e) {
    error_log("Unexpected Upload Temp Product Image Error: " . $e->getMessage() . " | User ID: $uid");
