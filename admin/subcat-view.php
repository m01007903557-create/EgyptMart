...
if (!empty($search)) {
    $searchCondition = " AND (s.pc_name LIKE '%$search%' OR c.pc_name LIKE '%$search%' OR m.pc_name LIKE '%$search%')";
}

// إعدادات الصفحة
$al->limit = $limit;
$al->setsql("SELECT s.pc_id, s.pc_name, s.pc_sort_name, c.pc_name, m.pc_name 
             FROM product_category_arabyos s, product_category_arabyos c, product_category_arabyos m 
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="table-header">
                                ...
                                <!-- Display records و Search يدوياً -->
<div class="row" style="margin-bottom: 10px;">
    <div class="col-xs-6">
        <div class="dataTables_length">
            <label>
                Display 
                <select id="pageLengthSelect" class="form-control input-sm" style="width: auto; display: inline-block; width: 60px;">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // تعيين قيمة البحث الحالية في حقل الإدخال
...
<script type="text/javascript">
    $(document).ready(function() {
        // تهيئة DataTable - مع تعطيل Search الأصلي
        $('#sample-table-2').dataTable({
            ...
            "bPaginate": false,      // إخفاء Pagination الافتراضي
            "bInfo": false,          // إخفاء معلومات الصفحة
            "bFilter": false,        // تعطيل Search الأصلي (نستخدم اليدوي)
            "bLengthChange": false   // تعطيل Display الأصلي (نستخدم اليدوي)
        });
        
        // تغيير عدد العناصر في الصفحة (يدوي)
        $('#pageLengthSelect').on('change', function() {
            var limit = $(this).val();
            var url = new URL(window.location.href);
            url.searchParams.set('limit', limit);
