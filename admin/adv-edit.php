<?php
/**
 * File: admin/adv-edit.php
 * Version: PHP 8.3
 * Description: تعديل إعلان موجود في لوحة التحكم
 * 
 * تسمح هذه الصفحة للمشرف بتعديل إعلان موجود، مع إمكانية تغيير
 * الرابط والصورة (اختياري) والموضع
 */
...
class editAdvertisement
{
    ...
    private function updateWithImage(): void
    {
        ...
        $sqlImg = "SELECT * FROM advertisement WHERE adv_id = " . $this->adv_id;
        $resImg = mysqli_query($this->con, $sqlImg);
        $rowImg = mysqli_fetch_object($resImg);

        // حذف الصورة القديمة إذا كانت موجودة
        if (!empty($rowImg->adv_img)) {
            $oldImagePath = "../upload/advertisement/" . $rowImg->adv_img;
...
if (isset($_POST['btnUpdate'])) {
    ...
    $ob->adv_position = $_POST['adv_position'] ?? '';
    ...
    $_SESSION['msg'] = $ob->msg;
    header("location: adv-edit.php?aid=" . $ob->adv_id);
    exit();
}
?>
                    ...
                    <div class="col-xs-12">
                        <form class="form-horizontal" action="" id="cmp_edit" name="cmp_edit" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input name="adv_link" id="adv_link" class="col-xs-10 col-sm-5" type="text" style="width:440px;" value="<?php echo htmlspecialchars($row->adv_link ?? ''); ?>" />
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <select name="adv_position" id="adv_position" class="chosen-select">
                                        ...
                                        <option value="right" <?php echo ($row->adv_position == 'right') ? 'selected="selected"' : ''; ?>>Right</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <?php if (!empty($row->adv_img)): ?>
                                        <img src="../upload/advertisement/<?php echo htmlspecialchars($row->adv_img); ?>" alt="Advertisement" style="max-width: 300px; max-height: 200px; border: 1px solid #ddd; padding: 5px;" />
                                    <?php else: ?>
