 ...
 * Description: عرض وإدارة لوجو الموردين (Suppliers Logo)
 */
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] == 'change_status') {
    ...
    if ($id > 0 && ($stat == 0 || $stat == 1)) {
        $sql = "UPDATE supplier_logo SET adv_status = $stat WHERE adv_id = $id";
...
/**
 * Class SupplierLogoList - إدارة لوجو الموردين
 */
class SupplierLogoList {
    ...
    public function deleterecord(int $adid): void {
        ...
        $chquesql = "SELECT adv_img FROM supplier_logo WHERE adv_id = ?";
        ...
        if ($row && !empty($row['adv_img'])) {
            $path = "../upload/supplier_logo/" . $row['adv_img'];
            if (is_file($path)) {
                unlink($path);
            }
        }
        ...
        $sql_del = "DELETE FROM supplier_logo WHERE adv_id = ?";
    ...
    public function deletelink(int $id): string {
        $dellink = $_SERVER['QUERY_STRING'] == "" 
            ? "?action=del&fid=" . $id
            : "supp-view.php?" . $_SERVER['QUERY_STRING'] . "&action=del&fid=" . $id;
        return $dellink;
    }
}
...
$al->setsql("SELECT * FROM supplier_logo ORDER BY adv_updated_date DESC");
...
$query = "SELECT * FROM supplier_logo ORDER BY adv_updated_date DESC LIMIT {$al->start}, {$limit}";
$al->setsql($query);
$recObj = $al->listview();
        ...
        <div class="main-content">
            <div class="breadcrumbs" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li><a href="supp-view.php">Manage Suppliers Logo</a></li>
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="table-header">
                                ...
                                <button type="button" class="btn btn-xs btn-success" onclick="window.location='supp-add.php'">
                                    <i class="icon-pencil align-top bigger-120"></i> Add Logo
                                </button>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="row_<?php echo $row->adv_id; ?>">
                                            ...
                                            <td style="text-align:center;">
                                                <img src="https://egyptmart.shop/upload/supplier_logo/<?php echo htmlspecialchars($row->adv_img); ?>" width="100px;" height="80px;">
                                              </td>
                                            <td style="text-align:center">
                                                <a href="<?php echo htmlspecialchars($row->adv_link); ?>" target="_blank">
                                                    <?php echo htmlspecialchars(substr($row->adv_link, 0, 50)); ?>
                                                </a>
                                              </td>
                                            <td style="text-align:center">
                                                <?php echo !empty($country2show) ? htmlspecialchars($country2show) : 'Global'; ?>
                                              </td>
                                            ...
                                            <td align="center">
                                                <a href="supp-edit.php?aid=<?php echo $row->adv_id; ?>" title="edit">
