<?php
/**
 * File: admin/advcathome-edit.php
 * Version: PHP 8.3
 * Description: تعديل إعلان الفئات في الصفحة الرئيسية
 * 
 * تسمح هذه الصفحة للمشرف بتعديل إعلان الفئات الموجود،
 * مع إمكانية تغيير التصنيفات والبلدان والرابط والصورة
 */
...
class editAdvertisement
{
    ...
    private function updateWithImage(): void
    {
        ...
        if (!empty($rowImg->adv_img)) {
            $oldImagePath = "../upload/advertisementcathome/" . $rowImg->adv_img;
        ...
        try {
            ...
            $sql = "UPDATE advertisementcathome
                    SET
                        adv_cat_id = " . (int)$this->adv_cat_id . ",
                        adv_subcat_id = " . (int)$this->adv_subcat_id . ",
                        adv_subsub_cat_id = " . (int)$this->adv_subsub_cat_id . ",
...
if (isset($_POST['btnUpdate'])) {
    ...
    $ob->adv_cat_id = (int)($_POST['scat_cat_id'] ?? 0);
    $ob->adv_subcat_id = (int)($_POST['cat_id'] ?? 0);
    $ob->adv_subsub_cat_id = (int)($_POST['subcat_id'] ?? 0);
                                ...
                                <div class="col-sm-9">
                                    <select name="scat_cat_id" id="scat_cat_id" class="chosen-select" required onchange="showCategory();">
                                        ...
                                        while ($catrow = mysqli_fetch_object($catsql)):
                                            $selected = ($catrow->pc_id == $row->adv_cat_id) ? 'selected="selected"' : '';
                                        ?>
                                ...
                                <div class="col-sm-9">
                                    <select name="cat_id" id="cat_id" class="chosen-select" required onchange="showSubCategory();">
                                        ...
                                        if (!empty($row->adv_cat_id)) {
                                            $catsql = mysqli_query($con, "SELECT * FROM product_category WHERE pc_parent_id = " . (int)$row->adv_cat_id . " AND pc_status = '1'");
                                            while ($catrow = mysqli_fetch_object($catsql)):
                                                $selected = ($catrow->pc_id == $row->adv_subcat_id) ? 'selected="selected"' : '';
                                        ?>
                                                <option value="<?php echo (int)$catrow->pc_id; ?>" <?php echo $selected; ?>>
                                                    <?php echo htmlspecialchars($catrow->pc_name); ?>
                                                </option>
                                ...
                                <div class="col-sm-9">
                                    <select name="subcat_id" id="subcat_id" class="chosen-select" required>
                                        ...
                                        if (!empty($row->adv_subcat_id)) {
                                            $catsql = mysqli_query($con, "SELECT * FROM product_category WHERE pc_parent_id = " . (int)$row->adv_subcat_id . " AND pc_status = '1'");
                                            while ($catrow = mysqli_fetch_object($catsql)):
                                                $selected = ($catrow->pc_id == $row->adv_subsub_cat_id) ? 'selected="selected"' : '';
                                        ?>
                                                <option value="<?php echo (int)$catrow->pc_id; ?>" <?php echo $selected; ?>>
                                                    <?php echo htmlspecialchars($catrow->pc_name); ?>
                                                </option>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <?php if (!empty($row->adv_img)): ?>
                                        <img src="../upload/advertisementcathome/<?php echo htmlspecialchars($row->adv_img); ?>" alt="Advertisement" style="max-width: 300px; max-height: 200px; border: 1px solid #ddd; padding: 5px;" />
                                    <?php else: ?>
