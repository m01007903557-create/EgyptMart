...
if ($rfq['source_platform'] == 'saleoffer') {
    ...
    if ($offer_id > 0) {
        ...
        if ($offer_data && !empty($offer_data['so_usr_id'])) {
            ...
            $user_sql = "SELECT mobile1, fname, lname FROM user WHERE usr_id = $supplier_id LIMIT 1";
...
// ✅ رابط واتساب ويب (للجوال والمتصفح)
$wa_url = "https://api.whatsapp.com/send?phone=" . $cleanPhone . "&text=" . urlencode($message);

// ✅ رابط تطبيق واتساب (للكمبيوتر)
$wa_app_url = "whatsapp://send?phone=" . $cleanPhone . "&text=" . urlencode($message);

// ✅ ============================================================
// ✅ استخدام api.whatsapp.com لتحميل الرسالة تلقائياً
// ✅ ============================================================
$wa_url = "https://api.whatsapp.com/send?phone=" . $cleanPhone . "&text=" . urlencode($message);
...
// ✅ سجل للتصحيح
error_log("✅ WhatsApp sent: RFQ=$rfq_id, supplier=$supplier_id, phone=$supplier_phone");
