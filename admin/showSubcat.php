 ...
 * PHP Version: 8.3
 * 
 * Description: جلب التصنيفات الفرعية لعرضها في قائمة منسدلة (AJAX)
 * Get subcategories for dropdown display (AJAX)
 * 
 * Features:
...
/**
 * Class SubcategoryAjaxHandler
 * 
 * Handles fetching subcategories for AJAX dropdown
 */
class SubcategoryAjaxHandler {
    ...
    /** @var string Default option text */
    private string $defaultOption = '- Select Sub-Category -';
    ...
    public function __construct(mysqli $database) {
        $this->db = $database;
        $this->logFile = __DIR__ . '/../../logs/subcategories_ajax.log';
    ...
    public function generateOptions(array $subcategories, bool $includeDefault = true): string {
        ...
        if ($includeDefault) {
            $options[] = '<option value="0">' . htmlspecialchars($this->defaultOption, ENT_QUOTES, 'UTF-8') . '</option>';
        }
    ...
    public function logRequest(int $parentId, int $resultCount): void {
        ...
        $logEntry = sprintf(
            "[%s] Subcategory AJAX Request | Parent ID: %d | Parent: %s | Results: %d | User: %d | IP: %s\n",
            date('Y-m-d H:i:s'),
            $parentId,
            $categoryName,
            $resultCount,
    ...
    public function sendError(string $message, int $statusCode = 400): void {
        ...
        $errorOption = '<option value="0">- Error: ' . htmlspecialchars($message, ENT_QUOTES, 'UTF-8') . ' -</option>';
        echo $errorOption;
        
        error_log("Subcategory AJAX error: " . $message);
    }
}
...
try {
    // Initialize handler
    $handler = new SubcategoryAjaxHandler($con);
    
    // Validate input
    $parentId = $handler->validateCategoryId($_GET['q'] ?? null);
    
    // If ID is 0 or negative, return default option
    if ($parentId <= 0) {
        $defaultOption = '<option value="0">' . htmlspecialchars($handler->defaultOption, ENT_QUOTES, 'UTF-8') . '</option>';
        $handler->sendResponse($defaultOption);
        exit;
    }
    
    // Fetch subcategories
    $subcategories = $handler->fetchSubcategories($parentId);
    
    // Log the request
    $handler->logRequest($parentId, count($subcategories));
    
    // Generate and send response
    $response = $handler->generateOptions($subcategories, true);
    $handler->sendResponse($response);
    
} catch (InvalidArgumentException $e) {
    // Handle validation errors
    error_log("Subcategory AJAX validation error: " . $e->getMessage());
    $handler = $handler ?? new SubcategoryAjaxHandler($con);
    $handler->sendError('Invalid request');
    
} catch (RuntimeException $e) {
    // Handle runtime errors
    error_log("Subcategory AJAX runtime error: " . $e->getMessage());
    $handler = $handler ?? new SubcategoryAjaxHandler($con);
    $handler->sendError('Database error', 500);
    
} catch (Exception $e) {
    // Handle any other errors
    error_log("Subcategory AJAX unexpected error: " . $e->getMessage());
    $handler = $handler ?? new SubcategoryAjaxHandler($con);
    $handler->sendError('Unexpected error', 500);
