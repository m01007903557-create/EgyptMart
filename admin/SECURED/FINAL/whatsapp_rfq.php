...
if ($results && mysqli_num_rows($results) > 0) {
    while ($row = mysqli_fetch_assoc($results)) {
        ...
        // ✅ ============================================================
        // ✅ معالجة saleoffer (عروض البيع)
        // ✅ ============================================================
        if ($source == 'saleoffer' && $product_id > 0) {
            // ✅ جلب الصورة من sale_offer
            $offer_sql = "SELECT so_pic FROM sale_offer WHERE so_id = $product_id LIMIT 1";
            ...
            // ✅ جلب اسم المورد من sale_offer (so_usr_id)
            $sup_sql = "SELECT u.fname, u.lname, bp.bnsprof_compname 
                        FROM sale_offer so
                        LEFT JOIN user u ON so.so_usr_id = u.usr_id
                        LEFT JOIN business_profile bp ON u.usr_id = bp.bnsprof_uid
                        WHERE so.so_id = $product_id LIMIT 1";
            $sup_res = mysqli_query($con, $sup_sql);
            if ($sup_res && mysqli_num_rows($sup_res) > 0) {
                $sup_data = mysqli_fetch_assoc($sup_res);
                $row['supplier_name'] = $sup_data['bnsprof_compname'] ?? 
                                        trim(($sup_data['fname'] ?? '') . ' ' . ($sup_data['lname'] ?? ''));
...
<td class="center">
    ...
    $product_id = (int)($row['br_pc_id'] ?? 0);
    $source = $row['source_platform'] ?? '';
    $buyer_id = (int)($row['br_u_id'] ?? 0);

    if ($source == 'saleoffer' && $product_id > 0) {
        // ✅ عروض البيع: المورد هو صاحب العرض (so_usr_id)
        $sup_sql = "SELECT u.fname, u.lname, bp.bnsprof_compname 
                    FROM sale_offer so
                    LEFT JOIN user u ON so.so_usr_id = u.usr_id
                    LEFT JOIN business_profile bp ON u.usr_id = bp.bnsprof_uid
        ...
        if ($sup_res && mysqli_num_rows($sup_res) > 0) {
            $sup_data = mysqli_fetch_assoc($sup_res);
            $supplier_name = $sup_data['bnsprof_compname'] ?? 
                            trim(($sup_data['fname'] ?? '') . ' ' . ($sup_data['lname'] ?? ''));
        ...
        // ✅ المنتجات العادية: المورد هو صاحب المنتج (pd_uid)
        ...
        if ($sup_res && mysqli_num_rows($sup_res) > 0) {
            $sup_data = mysqli_fetch_assoc($sup_res);
            $supplier_name = $sup_data['bnsprof_compname'] ?? 'غير محدد';
        }
    }
...
<div id="waModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:9999;">
    <div style="background:#fff; width:450px; max-width:90%; margin:100px auto; padding:25px; border-radius:12px; direction:rtl;">
        <span onclick="closeWaModal()" style="float:left; cursor:pointer; font-size:24px;">×</span>
        <h3 style="color:#25D366;"><i class="fa fa-whatsapp"></i> إرسال إشعار للمورد</h3>
        ...
        <div style="margin:10px 0;">
            ...
            <a id="waLinkBtn" href="#" target="_blank" class="btn btn-success btn-sm" style="background:#25D366; color:white; padding:6px 12px; border-radius:4px; text-decoration:none;">
                <i class="fa fa-whatsapp"></i> فتح واتساب
            </a>
        </div>

        <div class="alert alert-info" style="margin-top:10px; padding:8px; background:#e8f4f8; border-radius:6px;">
...
<script>
// ============================================================
// ✅ فتح نافذة واتساب (Popup)
// ============================================================
function openWaModal(rfqId, phone, message) {
    console.log('📱 فتح Popup واتساب:', rfqId, phone);
    document.getElementById('waMessageText').value = message;
    document.getElementById('waLinkBtn').href = 'https://wa.me/' + phone + '?text=' + encodeURIComponent(message);
...
function closeWaModal() {
    ...
    // ✅ إعادة تحميل الصفحة بعد إغلاق الـ Popup لتحديث العداد
    location.reload();
}
...
// ============================================================
// ✅ دالة إرسال واتساب (بدون Popup تأكيد)
// ============================================================
function sendAndWhatsApp(rfqId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // ✅ فتح الرابط في تطبيق واتساب (وليس المتصفح)
            var isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
            var url = data.whatsapp_url;
            
            if (isMobile) {
                // ✅ على الجوال: استخدم wa.me (يفتح التطبيق)
                window.open(url, '_blank');
            } else {
                // ✅ على الكمبيوتر: استخدم whatasapp:// (يفتح التطبيق المثبت)
                var phone = data.phone;
                var message = data.message;
                // ✅ رابط تطبيق واتساب (للكمبيوتر)
var appUrl = 'whatsapp://send?phone=' + phone + '&text=' + encodeURIComponent(message);
window.location.href = appUrl;
                
                // ✅ محاولة فتح التطبيق أولاً
                window.location.href = appUrl;
                
                // ✅ إذا لم يفتح التطبيق، استخدم wa.me كـ fallback
                setTimeout(function() {
                    window.open(url, '_blank');
                }, 1500);
            }
...
console.log('✅ WhatsApp functions loaded');
</script>
...
<div id="waModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:9999;">
    <div style="background:#fff; width:450px; max-width:90%; margin:100px auto; padding:25px; border-radius:12px; direction:rtl;">
        ...
        <div style="margin:10px 0;">
            ...
            <!-- ✅ زر فتح واتساب - يستخدم wa.me فقط -->
            <a id="waLinkBtn" href="#" target="_blank" class="btn btn-success btn-sm" 
               style="background:#25D366; color:white; padding:6px 12px; border-radius:4px; text-decoration:none;">
               <i class="fa fa-whatsapp"></i> فتح واتساب
            </a>
        </div>
...
function copyWaMessage() {
    ...
    if (textarea) {
        ...
        textarea.setSelectionRange(0, 99999);
        document.execCommand('copy');
        alert('✓ تم نسخ الرسالة. يمكنك الآن لصقها في واتساب.');
    }
}
...
function openWaModal(phone, message) {
    // تنظيف رقم الهاتف للاستخدام في wa.me
    ...
    // ✅ استخدام wa.me فقط
    let waLink = 'https://wa.me/' + cleanPhone + '?text=' + encodeURIComponent(message);
...
// ✅ دالة إرسال واتساب (المعدلة)
// ✅ دالة إرسال واتساب (بدون confirm و alert)
// ✅ دالة إرسال واتساب (بدون إعادة تحميل)
function sendAndWhatsApp(rfqId) {
    // ✅ إظهار مؤشر تحميل على الزر
    var btn = document.querySelector('button[onclick="sendAndWhatsApp(' + rfqId + ')"]');
    if (btn) {
    ...
    .then(data => {
        if (data.success) {
            // ✅ فتح الـ Modal مباشرة (بدون alert)
            if (data.phone && data.message) {
                openWaModal(data.phone, data.message);
            }
        ...
        // ✅ إعادة تمكين الزر
        if (btn) {
            btn.innerHTML = '<i class="fa fa-whatsapp"></i> واتساب';
            btn.disabled = false;
        }
    })
    .catch(error => {
