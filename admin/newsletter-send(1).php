...
// =============================================
// معالجة النموذج (بريد إلكتروني أو واتساب)
// =============================================
if (isset($_POST['btnAdd']) || isset($_POST['btnAddWhatsApp'])) {
    // تحديد القناة بناءً على الزر المضغوط
    $channel = isset($_POST['btnAddWhatsApp']) ? 'whatsapp' : 'email';

    // 1. جلب البيانات من النموذج
    $nc_subject = trim($_POST['nc_subject'] ?? '');
    $nc_content = trim($_POST['nc_content'] ?? '');
    
    // جلب التصنيفات المحددة
    $categoryassigned = isset($_POST['categoryassigned']) && is_array($_POST['categoryassigned']) 
        ? implode(",", array_map('intval', $_POST['categoryassigned'])) 
        : '';
    ...
    // 3. حفظ النشرة في قاعدة البيانات مع تحديد القناة
    $sql = "INSERT INTO newsletter_content 
            SET nc_subject = ?, 
                nc_content = ?, 
                nc_category = ?, 
    ...
    // 4. التفرع بناءً على القناة
    if ($channel === 'whatsapp') {
        // --- إرسال عبر واتساب (Make.com) ---
        ...
        if ($http_code == 200 || $http_code == 201) {
            $_SESSION['msg'] = '<div class="alert alert-success">✅ تم إرسال طلب النشرة إلى قائمة انتظار واتساب بنجاح</div>';
        } else {
            $_SESSION['msg'] = '<div class="alert alert-warning">⚠️ تم حفظ النشرة ولكن فشل الاتصال بـ Make.com (الرمز: ' . $http_code . ')</div>';
        }
    } else {
        // --- إرسال عبر البريد الإلكتروني (الكود الأصلي) ---
        // معالجة الصور المضمنة
        $processedContent = $nc_content;
        // يمكن استدعاء دالة processInlineImages() إذا كانت موجودة
        
        // جلب المستخدمين المستهدفين وإرسال البريد
        $and = "";
        if (!empty($country)) {
            $and .= " AND country IN ($country)";
        }
        ...
        if (!empty($categoryassigned)) {
            // استخدام دالة buildCategoryCondition إذا كانت موجودة
            // $and .= $this->buildCategoryCondition($categoryassigned);
        }
        ...
        while ($row_usr = mysqli_fetch_assoc($res_usr)) {
            ...
            if (!empty($row_usr['email'])) {
                sendSMTPMail($row_usr['email'], $nc_subject, $message1);
            }
        }

        $_SESSION['msg'] = '<div class="alert alert-success">✅ تم إرسال النشرة عبر البريد الإلكتروني</div>';
    }

    // 5. تسجيل النشاط في activity_log (لكلا القناتين)
    ...
    $action = ($channel === 'whatsapp') ? 'whatsapp_newsletter' : 'email_newsletter';
    $log_title = "إرسال نشرة ($channel): " . substr($nc_subject, 0, 50);
    $ip = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';
    $admin_id = $_SESSION['ad_id_indm'] ?? 0;
    mysqli_stmt_bind_param($log_stmt, 'isiss', $admin_id, $action, $newsletter_id, $log_title, $ip);
    mysqli_stmt_execute($log_stmt);
    mysqli_stmt_close($log_stmt);

    // 6. التوجيه إلى صفحة العرض
...
<div class="main-container" id="main-container">
    ...
    <button class="btn btn-success" type="submit" name="btnAddWhatsApp" id="btnAddWhatsApp">
...
// داخل <script> في الأسفل
document.getElementById('btnAddWhatsApp').addEventListener('click', function() {
    document.getElementById('channel_type').value = 'whatsapp';
});
</script>
