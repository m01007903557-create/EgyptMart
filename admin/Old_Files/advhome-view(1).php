<?php
/**
 * File: admin/advhome-view.php
 * Version: PHP 8.3
 * Description: عرض وإدارة إعلانات الصفحة الرئيسية في لوحة التحكم
 * 
 * تعرض هذه الصفحة قائمة إعلانات الصفحة الرئيسية مع إمكانية
...
/**
 * كلاس إدارة قائمة إعلانات الصفحة الرئيسية
 */
class sliderviewlist
{
    ...
    public function deleterecord(int $adid): void
    {
        ...
        $chquesql = "SELECT * FROM advertisementhome WHERE adv_id = " . $adid;
        ...
        if ($chqueres && mysqli_num_rows($chqueres) > 0) {
            ...
            if (!empty($chquerow['adv_img'])) {
                $path = "../upload/advertisementhome/" . $chquerow['adv_img'];
                if (file_exists($path)) {
                    unlink($path);
                }
            }
        ...
        $sql_del = "DELETE FROM advertisementhome WHERE adv_id = " . $adid;
    ...
    public function deletelink(int $id): string
    {
        if ($_SERVER['QUERY_STRING'] == "") {
            ...
            $dellink = "advhome-view.php?" . $_SERVER['QUERY_STRING'] . "&action=del&fid=" . $id;
        }
        return $dellink;
    }
}
...
$al->setsql("SELECT * FROM advertisementhome ORDER BY adv_updated_date DESC");

$totalitems = $al->totalrecord();
$limit = $al->limit;
$al->start = $pagination->getStart($page, $limit, $totalitems);
$adjacents = 1;
$targetpage = "advhome-view.php";
        ...
        <div class="main-content">
            <div class="breadcrumbs" id="breadcrumbs">
                ...
                <ul class="breadcrumb">
                    ...
                    <li>
                        <a href="advhome-view.php">Manage Advertisement</a>
                    </li>
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="table-header">
                                ...
                                <button type="button" class="btn btn-xs btn-success" onclick="window.location='advhome-add.php'">
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align:center;">
                                                    <?php if (!empty($row->adv_img)): ?>
                                                        <img src="../upload/advertisementhome/<?php echo htmlspecialchars($row->adv_img); ?>" width="200px;" height="150px;" style="border: 1px solid #ddd; padding: 3px;" alt="Advertisement" />
                                                    <?php else: ?>
                                                        <span class="text-muted">No image</span>
                                                    <?php endif; ?>
                                                </td>
                                                <td style="text-align:left; padding: 10px;">
                                                    <strong>Link:</strong> <a href="<?php echo htmlspecialchars($row->adv_link ?? '#'); ?>" target="_blank"><?php echo htmlspecialchars($row->adv_link ?? ''); ?></a>
                                                    <br/><strong>Position:</strong> <?php echo htmlspecialchars($row->adv_position ?? ''); ?>
                                                    <?php
                                                    ...
                                                    if (!empty($country)):
                                                        ...
                                                        foreach ($countrylist as $snglecntry) {
                                                            $country_name = get_country_name((int)$snglecntry);
                                                            if ($country2show == '') {
                                                                $country2show = $country_name;
                                                            } else {
                                                                $country2show .= ", " . $country_name;
                                                            }
                                                        }
                                                        echo '<br/><strong>Country:</strong> ' . htmlspecialchars($country2show);
                                                    endif;
                                                    ?>
                                                </td>
                                                ...
                                                <td align="center">
                                                    <a href="advhome-edit.php?aid=<?php echo (int)$row->adv_id; ?>" title="Edit">
...
function changeStatus(stat, id) {
    ...
    $.post("ajax-file/advhome-change-status.php", {stat: stat, id: id}, function(data) {
        location.reload();
    }).fail(function() {
