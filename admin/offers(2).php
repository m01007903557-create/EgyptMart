...
$('.send-notification').click(function(e) {
    ...
    // ============================================
    // 1. إرسال طلب للخادم لحفظ العرض وإرساله لرسائل المشتري
    // ============================================
    ...
    .then(data => {
        if (data.success) {
            // ============================================
            // 2. إنشاء رسالة واتساب بالرابط السحري الصحيح
            // ============================================
            let magicLink = data.magic_link || `https://egyptmart.shop/ajax/enq-details.php?offer_id=${offerId}&rfq_id=${rfqId}`;
            
            let whatsappMsg = `📦 *عرض سعر جديد لطلبك RFQ #${rfqId}*\n\n`;
            whatsappMsg += `*المورد:* ${supplierName}\n`;
            whatsappMsg += `*السعر المقترح:* ${price} ${currency}\n`;
            whatsappMsg += `*مدة التوصيل:* ${delivery} يوم\n\n`;
            whatsappMsg += `✨ *للقبول أو الرفض، اضغط على الرابط:*\n`;
            whatsappMsg += `${magicLink}\n\n`;
            whatsappMsg += `يمكنك التواصل مع المورد مباشرة بعد القبول.`;
            
            let whatsappUrl = 'https://wa.me/' + cleanPhone + '?text=' + encodeURIComponent(whatsappMsg);
            
            // فتح واتساب
            window.open(whatsappUrl, '_blank');
            
            // تغيير الزر إلى "تم" بعد الإرسال
            $(this).html('<i class="fa fa-check"></i> تم')
                .removeClass('btn-success')
                .addClass('btn-default')
                .prop('disabled', true);
                
            alert('✅ تم إرسال عرض السعر إلى رسائل المشتري وواتساب');
        } else {
