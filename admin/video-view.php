                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-6">
    <?php
    // Generate pagination links - مع تحويل صارم للقيم إلى int
    $totalRecordsSafe = (int)($totalRecords ?? 0);
    $limitSafe = (int)($videoManager->limit ?? 20);
    
    // تجنب القسمة على صفر
    $totalPages = ($limitSafe > 0) ? (int)ceil($totalRecordsSafe / $limitSafe) : 1;
    
    if ($totalPages > 1) {
        $currentPageSafe = (int)$currentPage;
        if ($currentPageSafe < 1) $currentPageSafe = 1;
        ...
        if ($currentPageSafe > 1) {
            echo '<li class="prev"><a href="?page=' . ($currentPageSafe - 1) . '"><i class="icon-double-angle-left"></i></a></li>';
        } else {
            echo '<li class="prev disabled"><a href="#"><i class="icon-double-angle-left"></i></a></li>';
        }
        
        // Page numbers
        $startPage = max(1, $currentPageSafe - 2);
        $endPage = min($totalPages, $currentPageSafe + 2);
        
        for ($i = $startPage; $i <= $endPage; $i++) {
            $activeClass = ($i == $currentPageSafe) ? 'active' : '';
            echo '<li class="' . $activeClass . '"><a href="?page=' . $i . '">' . $i . '</a></li>';
        }
        
        // Next button
        if ($currentPageSafe < $totalPages) {
            echo '<li class="next"><a href="?page=' . ($currentPageSafe + 1) . '"><i class="icon-double-angle-right"></i></a></li>';
        } else {
