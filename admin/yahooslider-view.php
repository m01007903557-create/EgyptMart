            ...
            <tr id="row_<?php echo (int)$row->adv_id; ?>">
                <td class="center">
                 ...
                 <td>
                    ...
                    <strong>Link:</strong> <a href="<?php echo htmlspecialchars($row->adv_link ?? '#'); ?>" target="_blank"><?php echo htmlspecialchars(substr($row->adv_link ?? '', 0, 50)); ?></a><br>
                    <strong>Description:</strong> <?php echo htmlspecialchars(substr($row->adv_description ?? '', 0, 100)); ?><br>
                    <strong>Country:</strong> <?php 
                        $countries = explode(',', $row->adv_country ?? '');
                        $countryNames = [];
                        foreach ($countries as $cid) {
                            if (!empty($cid) && $cid > 0) {
                                $countryNames[] = get_country_name((int)$cid);
                            }
                        }
                        echo !empty($countryNames) ? implode(', ', $countryNames) : 'Global';
                    ?>
                 </td>
