...
if ($results && mysqli_num_rows($results) > 0) {
    while ($row = mysqli_fetch_assoc($results)) {
        ...
        if ($product_id > 0) {
            // ✅ محاولة جلب من sale_offer أولاً (لعروض البيع)
            $offer_sql = "SELECT so.so_pic, u.fname, u.lname, bp.bnsprof_compname 
                          FROM sale_offer so
            ...
            if ($offer_res && mysqli_num_rows($offer_res) > 0) {
                ...
                // ✅ وجدنا بيانات في sale_offer
                if (!empty($offer_data['so_pic'])) {
                    $row['product_image'] = $offer_data['so_pic'];
                    $row['image_path'] = '../upload/sale_offer/';
                }
                $row['supplier_name'] = $offer_data['bnsprof_compname'] ?? 
                                       ($offer_data['fname'] . ' ' . $offer_data['lname']);
            } else {
                // ✅ لم نجد في sale_offer، جرب من products
