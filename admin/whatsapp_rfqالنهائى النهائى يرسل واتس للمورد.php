...
<div class="main-container" id="main-container">
    ...
    <button class="btn btn-success send-supplier-wa" onclick="sendToSupplier(<?php echo $row['br_id']; ?>)" title="إرسال للمورد عبر واتساب"><i class="fa fa-whatsapp"></i> واتساب</button>
...
function sendToSupplier(rfqId) {
    ...
    if (!confirm('إرسال الطلب للمورد عبر واتساب؟')) return;
    ...
    // جلب رابط واتساب من الملف
    fetch('/admin/send_wa_final_working.php?rfq_id=' + rfqId)
        .then(response => response.json())
        .then(data => {
            if (data.success && data.whatsapp_url) {
                window.open(data.whatsapp_url, '_blank');
                ...
                alert('❌ خطأ: ' + (data.error || 'لم يتم إنشاء الرابط'));
            }
