...
// معالجة إرسال عرض السعر إلى رسائل المشتري (وظيفة جديدة)
if (isset($_POST['send_to_buyer_message']) && isset($_POST['offer_id'])) {
    $offer_id = (int)$_POST['offer_id'];
    
    $sql = "SELECT o.*, br.br_u_id as buyer_id, br.br_id as rfq_id FROM offers o
    ...
    if ($offer) {
        ...
        if (mysqli_query($con, $insert_sql)) {
            $success_msg = "تم إرسال العرض إلى رسائل المشتري";
            mysqli_query($con, "UPDATE offers SET status = 'notified' WHERE id = $offer_id");
        } else {
            $error_msg = "خطأ: " . mysqli_error($con);
        }
    }
}
...
<td>
    <?php 
    // ✅ جلب الصورة من المصدر الصحيح
    $img = 'noimage.jpg';
    $img_path = '../upload/myproduct/';
    
    // 1. إذا كانت الصورة في br_pic (طلبات الشراء)
    if (!empty($row['br_pic'])) {
        $img = $row['br_pic'];
        $img_path = '../upload/buy_requirement/';
        
        // ✅ التحقق من وجود الصورة في المجلد
        $full_path = $_SERVER['DOCUMENT_ROOT'] . '/' . $img_path . $img;
        if (!file_exists($full_path)) {
            // ✅ إذا لم تكن موجودة، جرب مجلد image_gallery
            $img_path = '../upload/buy_requirement/upload/image_gallery/';
            $full_path = $_SERVER['DOCUMENT_ROOT'] . '/' . $img_path . $img;
            if (!file_exists($full_path)) {
                // ✅ إذا لم تكن موجودة، استخدم noimage
                $img = 'noimage.jpg';
                $img_path = '../upload/myproduct/';
            }
        }
    } 
    // 2. إذا كانت الصورة في pd_image (منتجات عادية)
    elseif (!empty($row['pd_image'])) {
        $img = explode(',', $row['pd_image'])[0];
        $img_path = '../upload/myproduct/';
    }
    
    echo '<img src="' . $img_path . $img . '" width="50" height="50" style="object-fit:cover;" onerror="this.src=\'../upload/myproduct/noimage.jpg\'">';
    ?>
</td>                        
...
<div id="waModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:9999;">
    <div style="background:#fff; width:450px; max-width:90%; margin:100px auto; padding:25px; border-radius:12px; direction:rtl;">
        ...
        <div style="margin:10px 0;">
            ...
            <!-- ✅ زر فتح واتساب - يستخدم wa.me فقط (بدون web.whatsapp.com) -->
...
$('.send-notification').click(function(e) {
    ...
    message += "للاطلاع: https://egyptmart.shop/my-enquiries.php?rfq_id=" + rfqId;
    
    // ✅ استخدام wa.me فقط (ممنوع web.whatsapp.com)
    let waLink = 'https://wa.me/' + cleanPhone + '?text=' + encodeURIComponent(message);
    
    // عرض النافذة
    $('#waMessageText').val(message);
    $('#waLinkBtn').attr('href', waLink);
    $('#waModal').show();
    
     // تحديث حالة العرض في الخلفية
