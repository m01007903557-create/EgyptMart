...
// تنظيف رقم جوال المورد (للاتصال به)
...
// صورة المنتج (رابط مباشر قابل للعرض)
$product_image_url = '';
if (!empty($rfq['pd_image'])) {
    $images = explode(',', $rfq['pd_image']);
    $product_image_url = 'https://egyptmart.shop/upload/myproduct/' . trim($images[0]);
}
...
$buyer_mobile_raw = $buyer_data['mobile1'] ?? '';
$buyer_email = $buyer_data['email'] ?? '';
$buyer_country = get_country_name($buyer_data['country'] ?? 0);

// تنسيق رقم المشتري مع كود البلد والصفر
$buyer_mobile_formatted = '';
if (!empty($buyer_mobile_raw)) {
    $buyer_mobile_formatted = $buyer_mobile_raw; // يحتفظ بالصفر في البداية (مثال: 01033921400)
}

// =============================================
// إدراج رسالة في Dashboard المورد
// =============================================
...
if (!empty($product_image_url)) {
    $body .= "رابط صورة المنتج: " . $product_image_url . "\n";
}
...
$body .= "• الجوال: " . $buyer_mobile_formatted . "\n";
...
// =============================================
// الرابط السحري للمورد (يوجه إلى Dashboard المورد)
// =============================================
$magic_token = md5($supplier_id . $rfq_id . time() . rand(1000, 9999));
$expiry = date('Y-m-d H:i:s', strtotime('+7 days'));
mysqli_query($con, "UPDATE buy_requirement SET wa_magic_token = '$magic_token', wa_token_expiry = '$expiry' WHERE br_id = $rfq_id");

// رابط يوجه المورد إلى Dashboard الخاص به (وليس Dashboard المشتري)
$magic_link = "https://egyptmart.shop/my-enquiries.php?rfq_id=" . $rfq_id;

// =============================================
// بناء رسالة واتساب للمورد
// =============================================
$wa_message = "📦 *طلب شراء جديد #" . $rfq['br_id'] . "*\n\n";
$wa_message .= "*المنتج:* " . ($rfq['pd_title'] ?? '') . "\n";
if (!empty($product_image_url)) {
    $wa_message .= "*رابط صورة المنتج:* " . $product_image_url . "\n";
}
