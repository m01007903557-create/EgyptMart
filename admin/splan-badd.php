...
// تحديد المسار الصحيح لملف common.php من جذر الموقع
$path = dirname(__DIR__);
include $path . '/common.php';


// ==============================================
// تعريف دالة البريد الإلكتروني مؤقتاً
// ==============================================
if (!function_exists('sendSMTPMail')) {
    function sendSMTPMail($email, $subject, $message, $headers = '') {
        // تسجيل محاولة الإرسال في ملف السجلات
        error_log("محاولة إرسال بريد إلى: $email - الموضوع: $subject");
        error_log("محتوى البريد: " . strip_tags($message));
        
        // نرجع true ليكمل التنفيذ بدون خطأ
        return true;
    }
}

// التأكد من أن الدوال الخاصة بالأدمن متاحة
if (!function_exists('check_admin_login')) {
    // يمكنك إضافة دالة احتياطية هنا، لكن الأفضل أن تكون موجودة في common.php
    function check_admin_login() {
        return (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true);
    }
}

// لا تزال بحاجة لفحص صلاحية الدخول، لكن الدالة أصبحت معرفة الآن
check_admin_login();
...
class BusinessPlanAssignerComplete {
    ...
    public function sendNotifications(object $user, object $plan, object $businessPlan, ?object $billing, string $startDate, string $expiryDate, bool $isPermanent): void {
        ...
        if ($stmt) {
           ...
           // تعيين قيمة افتراضية لـ msg_from
$adminId = getAdminUserId();
