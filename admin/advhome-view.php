<?php
/**
 * File: advhome-view.php
 * Version: 2.0.0 (PHP 8.3)
 * Description: عرض وإدارة إعلانات الصفحة الرئيسية
 */
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] == 'change_status') {
    ...
    if ($id > 0 && ($stat == 0 || $stat == 1)) {
        $sql = "UPDATE advertisementhome SET adv_status = $stat WHERE adv_id = $id";
...
class AdvList {
    ...
    public function deleterecord(int $adid): void {
        ...
        $chquesql = "SELECT adv_img FROM advertisementhome WHERE adv_id = ?";
        ...
        if ($row && !empty($row['adv_img'])) {
            $path = "../upload/advertisementhome/" . $row['adv_img'];
            if (is_file($path)) {
                unlink($path);
            }
        }
        
        $sql_del = "DELETE FROM advertisementhome WHERE adv_id = ?";
    ...
    public function deletelink(int $id): string {
        $dellink = $_SERVER['QUERY_STRING'] == "" 
            ? "?action=del&fid=" . $id
            : "advhome-view.php?" . $_SERVER['QUERY_STRING'] . "&action=del&fid=" . $id;
        return $dellink;
    }
}
...
$al->setsql("SELECT * FROM advertisementhome ORDER BY adv_id DESC");

$totalitems = $al->totalrecord();
$limit = $al->limit;
$al->start = $pagination->getStart($page, $limit, $totalitems);

$query = "SELECT * FROM advertisementhome ORDER BY adv_id DESC LIMIT {$al->start}, {$limit}";
$al->setsql($query);
$recObj = $al->listview();
        ...
        <div class="main-content">
            <div class="breadcrumbs" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li class="active">Home Advertisements</li>
                </ul>
            </div>
                                            ...
                                            <tr id="row_<?php echo $row->adv_id; ?>">
                                                ...
                                                <td class="center">
                                                    <?php if (!empty($row->adv_img)): ?>
                                                        <img src="../upload/advertisementhome/<?php echo htmlspecialchars($row->adv_img); ?>" style="max-width:100px; max-height:80px;">
                                                    <?php else: ?>
                                                ...
                                                <td class="center"><?php echo $row->adv_imagewidth . ' x ' . $row->adv_imageheight; ?></td>
                                                <td class="center">
                                                ...
                                                <td class="center">
                                                    <a href="advhome-edit.php?aid=<?php echo $row->adv_id; ?>" class="btn btn-xs btn-info">Edit</a>
